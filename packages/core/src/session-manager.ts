...
import { buildAgentPath, setupPathWrapperWorkspace, PREFERRED_GH_PATH } from "./agent-workspace-hooks.js";
...
const EXEC_SHELL_OPTION = process.platform === "win32" ? ({ shell: true, windowsHide: true } as const) : ({} as const);
...
const STALE_PR_OWNERSHIP_STATUSES: ReadonlySet<string> = new Set([...PR_TRACKING_STATUSES, "merged"]);
...
async function isAgentProcessNotDefinitelyMissing(agent: Agent, handle: RuntimeHandle): Promise<boolean> {
	try {
...
export function createSessionManager(deps: SessionManagerDeps): OpenCodeSessionManager {
	...
	function findOpenCodeSessionIds(sessionsDir: string, criteria: { issueId?: string; sessionId?: string }): string[] {
	...
	async function resolveOpenCodeSessionReuse(options: {
		...
		if (strategy === "delete") {
			if (includeTitleDiscoveryForSessionId && criteria.sessionId) {
				candidateIds = [...candidateIds, ...(await discoverOpenCodeSessionIdsByTitle(criteria.sessionId))];
			}
	...
	function resolvePlugins(project: ProjectConfig, agentName?: string) {
		...
		const tracker = project.tracker?.plugin ? registry.get<Tracker>("tracker", project.tracker.plugin) : null;
		const scm = project.scm?.plugin ? registry.get<SCM>("scm", project.scm.plugin) : null;

		return { runtime, agent, workspace, tracker, scm };
	}

	function resolveSelectionForSession(project: ProjectConfig, sessionId: string, metadata: Record<string, string>) {
	...
	async function enrichSessionWithRuntimeState(
		...
		async function persistAgentSessionInfo(options?: { skipIfNativeRestoreMetadataPresent?: boolean }): Promise<void> {
			...
			if (options?.skipIfNativeRestoreMetadataPresent && hasPersistedNativeRestoreMetadata(session, plugins.agent)) {
				return;
			}
		...
		if (handleFromMetadata && session.runtimeHandle && plugins.runtime && session.status !== "spawning") {
			try {
				...
				if (!alive) {
					...
					if (session.lifecycle.session.state !== "done" && session.lifecycle.session.state !== "terminated") {
	...
	async function relaunchOrchestratorInternal(orchestratorConfig: OrchestratorSpawnConfig): Promise<Session> {
		const project = config.projects[orchestratorConfig.projectId];
		if (!project) {
			throw new Error(`Unknown project: ${orchestratorConfig.projectId}`);
		}
	...
	async function relaunchOrchestrator(orchestratorConfig: OrchestratorSpawnConfig): Promise<Session> {
		const project = config.projects[orchestratorConfig.projectId];
		if (!project) {
			throw new Error(`Unknown project: ${orchestratorConfig.projectId}`);
		}
	...
	async function list(projectId?: string): Promise<Session[]> {
		...
		const tasks = allSessions.map(async ({ sessionName, projectId: sessionProjectId, raw }) => {
			...
			const sessionListPromise =
				effectiveAgentName === "opencode" ? (openCodeSessionListPromise ??= fetchOpenCodeSessionList()) : undefined;
			...
			if (
				session.lifecycle &&
				(session.lifecycle.runtime.state === "missing" || session.lifecycle.runtime.state === "exited") &&
	...
	async function send(sessionId: SessionId, message: string): Promise<void> {
		...
		const parsedHandle = raw["runtimeHandle"] ? safeJsonParse<RuntimeHandle>(raw["runtimeHandle"]) : null;
		...
		const waitForInteractiveReadiness = async (session: Session, timeoutMs: number): Promise<void> => {
			...
			while (true) {
				...
				if (runtimeAlive && processRunning && foregroundReady && (hasQueuedMessage(output) || isStable)) {
					stablePolls += 1;
		...
		const sendWithConfirmation = async (session: Session): Promise<void> => {
			...
			for (let attempt = 1; attempt <= SEND_CONFIRMATION_ATTEMPTS; attempt++) {
				...
				const delivered =
					(baselineUpdatedAt !== undefined && updatedAt !== undefined && updatedAt > baselineUpdatedAt) ||
	...
	async function claimPR(sessionId: SessionId, prRef: string, options?: ClaimPROptions): Promise<ClaimPRResult> {
	...
	async function remap(sessionId: SessionId, force = false): Promise<string> {
		...
		const discovered = force
			? await discoverOpenCodeSessionIdByTitle(sessionId, OPENCODE_INTERACTIVE_DISCOVERY_TIMEOUT_MS)
			: (mapped ?? (await discoverOpenCodeSessionIdByTitle(sessionId, OPENCODE_INTERACTIVE_DISCOVERY_TIMEOUT_MS)));
		if (!discovered) {
	...
	async function restore(sessionId: SessionId): Promise<Session> {
		...
		const projectConfigForLaunch: ProjectConfig = {
			...
			agentConfig: {
				...
				...(session.metadata?.opencodeSessionId ? { opencodeSessionId: session.metadata.opencodeSessionId } : {}),
			},
		};
		...
		if (plugins.agent.postLaunchSetup) {
			try {
				...
				const metadataUpdates = Object.fromEntries(
					Object.entries(metadataAfterPostLaunch).filter(([key, value]) => metadataBeforePostLaunch[key] !== value),
				);

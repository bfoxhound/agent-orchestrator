...
export function isTerminalSession(session: {
	...
	return (
		TERMINAL_STATUSES.has(session.status) || (session.activity !== null && TERMINAL_ACTIVITIES.has(session.activity))
	);
}
...
export function isProcessProbeIndeterminate(result: ProcessProbeResult): result is typeof PROCESS_PROBE_INDETERMINATE {
...
export interface BatchObserver {
	...
	recordSuccess(data: { batchIndex: number; totalBatches: number; prCount: number; durationMs: number }): void;
...
export type ExternalPluginLocation =
	{ kind: "project"; projectId: string; configType: "tracker" | "scm" } | { kind: "notifier"; notifierId: string };
...
export function normalizeAgentPermissionMode(mode: string | undefined): AgentPermissionMode | undefined {
...
export interface SessionManager {
	...
	cleanup(projectId?: string, options?: { dryRun?: boolean; purgeOpenCode?: boolean }): Promise<CleanupResult>;

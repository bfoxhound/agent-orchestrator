...
export function inventoryHashDirs(aoBaseDir: string, globalConfigPath?: string): HashDirEntry[] {
	...
	for (const name of readdirSync(aoBaseDir)) {
		...
		const hasSessions =
			existsSync(sessionsDir) && readdirSync(sessionsDir).some((f) => !f.startsWith(".") && f !== "archive");
...
export async function detectActiveSessions(knownPrefixes?: string[]): Promise<string[]> {
	try {
		...
		const v2PrefixPatterns = (knownPrefixes ?? [])
			...
			.flatMap((p) => [new RegExp(`^${escapeRegExp(p)}-\\d+$`), new RegExp(`^${escapeRegExp(p)}-orchestrator-\\d+$`)]);
 ...
 */
function countPostMigrationSessions(projectDir: string, migratedDirs: Array<{ path: string }>): number {
	const sessionsDir = join(projectDir, "sessions");
	...
	if (existsSync(projectsDir)) {
		for (const projectId of safeToDeleteProjects) {
			...
			if (existsSync(v2WorktreesDir)) {
				...
				const targetHashDir = projectMigratedDirs[0] ? projectMigratedDirs[0].path.replace(/\.migrated$/, "") : null;

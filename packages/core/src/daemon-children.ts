import { type ChildProcess, type SpawnOptions, execFile as execFileCb, spawn } from "node:child_process";
import { existsSync, mkdirSync, readFileSync, rmSync, rmdirSync, statSync, unlinkSync } from "node:fs";
...
function pruneSweptDaemonChildren(sweptPids: Set<number>): void {
	const release = acquireRegistryLock();
	try {
		const next = readRawDaemonChildren().filter((entry) => !sweptPids.has(entry.pid) || isProcessAlive(entry.pid));
		writeRawDaemonChildren(next);
	} finally {
		release();
	}
...
export async function sweepDaemonChildren(options: DaemonChildSweepOptions = {}): Promise<DaemonChildSweepResult> {
	const { ownerPid, graceMs = DEFAULT_GRACE_MS } = options;
	const entries = getDaemonChildren().filter((entry) => ownerPid === undefined || entry.parentPid === ownerPid);
...
export function classifyAoOrphanCommand(command: string): string | null {
	...
	if (normalized.includes("/ao lifecycle-worker ") || normalized.includes(" ao lifecycle-worker ")) {
...
function parseLeadingUnsignedInt(value: string, start: number): { value: number; next: number } | null {
	let index = start;

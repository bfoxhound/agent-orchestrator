...
import { auditAgentReports, getReactionKeyForTrigger, REPORT_WATCHER_METADATA_KEYS } from "./report-watcher.js";
...
type WorkspaceBranchProbe = { kind: "branch"; branch: string } | { kind: "detached" } | { kind: "unavailable" };

const TRANSIENT_DETACHED_GIT_MARKERS = ["rebase-merge", "rebase-apply", "CHERRY_PICK_HEAD", "BISECT_LOG"] as const;
...
async function readWorkspaceBranch(workspacePath: string): Promise<WorkspaceBranchProbe> {
	...
	try {
		...
		if (!head.startsWith(prefix)) {
			return (await hasTransientDetachedGitState(gitDir)) ? { kind: "unavailable" } : { kind: "detached" };
		}
...
export function createLifecycleManager(deps: LifecycleManagerDeps): LifecycleManager {
	...
	async function populatePREnrichmentCache(sessions: Session[]): Promise<void> {
		...
		for (const session of sessions) {
			...
			if (session.pr && !(session.lifecycle.pr.state === "closed" && session.pr.branch !== session.branch)) {
				continue;
			}
	...
	async function refreshTrackedBranch(session: Session, siblingSessions?: Session[]): Promise<void> {
		const project = config.projects[session.projectId];
		if (!project) return;
		...
		const canRefreshTrackedBranch =
			sessionRole === "worker" && workspacePath !== null && (!session.pr || session.lifecycle.pr.state === "closed");
		...
		try {
			...
			if (
				!isBranchOwnedByAnotherActiveWorker(session, branchProbe.branch, sessionsForConflictCheck, allSessionPrefixes)
			) {
	...
	function getReactionConfigForSession(session: Session, reactionKey: string): ReactionConfig | null {
		const project = config.projects[session.projectId];
	...
	async function maybeDispatchReviewBacklog(
		...
		const hasRelevantTransition =
			transitionReaction?.key === humanReactionKey || transitionReaction?.key === automatedReactionKey;
		...
		// --- Pending (human) review comments ---
		{
			...
			if (!pendingFingerprint) {
				...
				if (
					...
					let success = false;
					if (transitionReaction?.key === humanReactionKey && reactionConfig.action === "send-to-agent") {
						try {
		...
		// --- Automated (bot) review comments ---
		{
			...
			if (!automatedFingerprint) {
				...
				if (
					...
					let success = false;
					if (transitionReaction?.key === automatedReactionKey && reactionConfig.action === "send-to-agent") {
						try {
							await sessionManager.send(session.id, enrichedMessage);
							success = true;
						} catch {
	 ...
	 */
	async function formatCIFailureMessage(scm: SCM, pr: PRInfo, failedChecks: CICheck[]): Promise<string> {
	...
	async function getFailedCIChecks(scm: SCM, pr: PRInfo, options: { allowFetch: boolean }): Promise<CICheck[] | null> {
	...
	async function maybeDispatchCIFailureDetails(
		...
		if (
			...
			(transitionReaction.messageEnriched === true || transitionReaction.result.action !== "send-to-agent")
		) {
			updateSessionMetadata(session, {
	 ...
	 */
	async function maybeDispatchMergeConflicts(session: Session, newStatus: SessionStatus): Promise<void> {
		const project = config.projects[session.projectId];
		if (!project || !session.pr) return;
	...
	async function notifyHuman(event: OrchestratorEvent, priority: EventPriority): Promise<void> {
		...
		for (const name of notifierNames) {
			...
			const notifier =
				registry.get<Notifier>("notifier", target.reference) ?? registry.get<Notifier>("notifier", target.pluginName);
			if (notifier) {
				try {
	...
	async function maybeAutoCleanupOnMerge(session: Session): Promise<void> {
		...
		const graceElapsed = Number.isFinite(pendingSinceMs) ? Date.now() - pendingSinceMs >= graceMs : false;
	...
	async function checkSession(session: Session): Promise<void> {
		...
		const oldStatus = tracked ?? ((session.metadata?.["status"] as SessionStatus | undefined) || session.status);
		...
		const isDetectingEscalated =
			...
			(assessment.detectingAttempts > DETECTING_MAX_ATTEMPTS || isDetectingTimedOut(nextDetectingStartedAt));
		...
		if (newStatus !== oldStatus) {
			...
			if (eventType) {
				...
				if (reactionKey) {
					...
					if (reactionKey === "ci-failed" && session.pr && reactionConfig?.action === "send-to-agent") {
						const project = config.projects[session.projectId];
						const scm = project?.scm?.plugin ? registry.get<SCM>("scm", project.scm.plugin) : null;
						if (scm) {
		...
		if (session.agentInfo?.summary && !session.agentInfo.summaryIsFallback && !session.metadata["pinnedSummary"]) {

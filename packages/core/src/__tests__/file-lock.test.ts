...
describe("withFileLockSync", () => {
	...
	it("serializes parallel holders across processes", async () => {
		...
		const runHolder = (label: string) =>
			new Promise<void>((resolve, reject) => {
				const child = spawn(process.execPath, ["--import", "tsx", helperPath, lockPath, logPath, label, "100"], {
					cwd: process.cwd(),
					stdio: "inherit",
				});
	...
	it("times out when another fresh lock cannot be acquired", () => {
		...
		try {
			expect(() => withFileLockSync(lockPath, () => "never", { timeoutMs: 20, staleMs: 60_000 })).toThrow(

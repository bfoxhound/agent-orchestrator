...
describe("isOrchestratorSession", () => {
	it("detects orchestrators by explicit role metadata", () => {
		expect(isOrchestratorSession({ id: "app-control", metadata: { role: "orchestrator" } }, "app")).toBe(true);
	});
	...
	it("does not false-positive when prefix ends with -orchestrator", () => {
		...
		expect(isOrchestratorSession({ id: "my-orchestrator-orchestrator-1", metadata: {} }, "my-orchestrator")).toBe(true);
	});
	...
	it("rejects bare {projectId}-orchestrator legacy ids without role metadata", () => {
		expect(isOrchestratorSession({ id: "integrator-orchestrator", metadata: {} }, "int")).toBe(false);
	});

	it("accepts bare legacy ids when role metadata is explicitly stamped", () => {
		expect(isOrchestratorSession({ id: "integrator-orchestrator", metadata: { role: "orchestrator" } }, "int")).toBe(
			true,
		);
	});
});

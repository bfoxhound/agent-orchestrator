...
function writeTerminatedSession(sessionId: string, options: { worktree: string; branch?: string }): void {
...
describe("session.spawn_failed — orchestrator path (MUST)", () => {
	...
	it("emits one terminal failure plus one stage failure when workspace.create throws", async () => {
		...
		const sm = createSessionManager({ config, registry: mockRegistry });
		await expect(sm.spawnOrchestrator({ projectId: "my-app", systemPrompt: "be helpful" })).rejects.toThrow(
			"disk full",
		);
...
describe("session.send_failed (MUST)", () => {
	...
	it("tags restore-for-delivery timeout restore_failed with ready_timeout stage", async () => {
		...
		try {
			...
			const rejection = expect(sendPromise).rejects.toThrow("restored session did not become ready for delivery");

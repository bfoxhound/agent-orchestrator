...
describe("mutateMetadata corrupt-file handling", () => {
	it("preserves corrupt JSON as `<path>.corrupt-<ts>` before overwriting", () => {
		...
		const result = mutateMetadata(dataDir, "ao-1", () => ({ branch: "feat/x", project: "myproject" }), {
	...
	it("emits metadata.corrupt_detected when JSON parse fails and file is renamed", () => {
		...
		const result = mutateMetadata(dataDir, "ao-3", (existing) => ({ ...existing, branch: "feat/x" }), {
			createIfMissing: true,
		});
	...
	it("emits a rename-failed summary when corrupt metadata cannot be renamed", () => {
		...
		const result = mutateMetadata(dataDir, "ao-rename-failed", (existing) => ({ ...existing, branch: "feat/x" }), {
			createIfMissing: true,
		});

		expect(result).not.toBeNull();
		const call = vi.mocked(recordActivityEvent).mock.calls.find((c) => c[0].kind === "metadata.corrupt_detected");
	...
	it("truncates contentSample to 200 chars in metadata.corrupt_detected", () => {
		...
		mutateMetadata(dataDir, "ao-4", (existing) => ({ ...existing, branch: "feat/y" }), { createIfMissing: true });
...
describe("status derivation from lifecycle", () => {
	...
	it("readMetadata falls back to 'unknown' when no status and no lifecycle", () => {
		writeFileSync(join(dataDir, "bare.json"), JSON.stringify({ worktree: "/tmp/w", branch: "main" }));

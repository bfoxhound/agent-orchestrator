...
function setupCheck(
	...
	const persistedStringMetadata = Object.fromEntries(
		Object.entries(persistedMetadata).filter((entry): entry is [string, string] => typeof entry[1] === "string"),
	);
...
describe("check (single session)", () => {
	...
	for (const marker of ["rebase-merge", "rebase-apply", "CHERRY_PICK_HEAD", "BISECT_LOG"] as const) {
	...
	it("keeps existing branch metadata when the current worktree HEAD cannot be read", async () => {
		...
		expect(mockSCM.detectPR).toHaveBeenCalledWith(expect.objectContaining({ branch: "fix-login" }), expect.anything());
		const meta = readMetadataRaw(env.sessionsDir, "app-1");
		expect(meta?.["branch"]).toBe("fix-login");
	});
	...
	it("serializes competing branch adoption within one poll cycle without extra session list calls", async () => {
		...
		try {
			...
			const adoptedCount = [sessionA.branch, sessionB.branch].filter((branch) => branch === "shared-branch").length;
...
describe("reactions", () => {
	it("fires report watcher reactions only once per active trigger", async () => {
		...
		try {
			...
			const reactionNotifications = vi.mocked(notifier.notify).mock.calls.filter((call) => {
				...
				return event?.type === "reaction.triggered" && event.data?.["reactionKey"] === "report-no-acknowledge";
			});
				...
				failedJobs: [
					{
						...
						logTail: "AssertionError: expected true to be false\n```\nProcess completed with exit code 1",
					},
				],
			}),
	...
	it("re-dispatches merge conflict notification after conflicts are resolved and recur", async () => {
		...
		vi.mocked(mockSCM.enrichSessionsPRBatch!).mockImplementation(mockBatchEnrichment({ hasConflicts: false }));
		await lm.check("app-1");
		expect(mockSessionManager.send).not.toHaveBeenCalled();
		...
		vi.mocked(mockSCM.enrichSessionsPRBatch!).mockImplementation(mockBatchEnrichment({ hasConflicts: true }));
		await lm.check("app-1");
		expect(mockSessionManager.send).toHaveBeenCalledTimes(1);
	});
	...
	it("notifies humans on significant transitions without reaction config", async () => {
		...
		expect(lm.getStates().get("app-1")).toBe("merged");
		expect(notifier.notify).toHaveBeenCalled();
		expect(notifier.notify).toHaveBeenCalledWith(expect.objectContaining({ type: "merge.completed" }));
	});
	...
	it("prefers alias-specific notifier instances over shared plugin instances", async () => {
		...
		expect(opsNotifier.notify).toHaveBeenCalledWith(expect.objectContaining({ type: "merge.completed" }));
...
describe("pollAll terminal status accounting", () => {
	...
	it("treats all TERMINAL_STATUSES as inactive for all-complete", async () => {
		...
		expect(notifier.notify).toHaveBeenCalledWith(expect.objectContaining({ type: "reaction.triggered" }));
	...
	it("does not fire all-complete when a session is in non-terminal status like done is missing", async () => {
		...
		const allCompleteNotifications = vi.mocked(notifier.notify).mock.calls.filter((call: unknown[]) => {

...
describe("inventoryHashDirs", () => {
	...
	it("detects bare 12-hex hash directories", () => {
		...
		writeFileSync(join(testDir, "aaaaaa000000", "sessions", "ao-1"), "project=myproject\nstatus=working\n");
	...
	it("derives bare hash projectId from global config storageKey", () => {
		...
		writeFileSync(
			configPath,
			["projects:", "  my-app:", "    path: /home/user/my-app", "    storageKey: aaaaaa000000", ""].join("\n"),
		);
...
describe("convertKeyValueToJson", () => {
	...
	it("converts statePayload to lifecycle object", () => {
		...
		const kv = ["project=myproject", `statePayload=${JSON.stringify(lifecycle)}`, "stateVersion=2"].join("\n");
...
describe.skipIf(process.platform === "win32")("migrateStorage", () => {
	...
	it("writes orchestrator sessions to sessions/ alongside workers", async () => {
		...
		writeFileSync(
			...
			["project=myproject", "role=orchestrator", "agent=claude-code", "createdAt=2026-04-21T12:00:00.000Z"].join("\n"),
		);
	...
	it("migrates bare 12-hex hash directories", async () => {
		...
		writeFileSync(
			configPath,
			["projects:", "  myproject:", "    path: /home/user/myproject", "    storageKey: aaaaaa000000", ""].join("\n"),
		);
...
describe.skipIf(process.platform === "win32")("rollbackStorage", () => {
	...
	it("restores .migrated directories and removes migrated projects", async () => {
		...
		writeFileSync(join(aoBaseDir, "aaaaaa000000-myproject.migrated", "sessions", "ao-1"), "project=myproject");
		mkdirSync(join(aoBaseDir, "projects", "myproject", "sessions"), { recursive: true });
		writeFileSync(join(aoBaseDir, "projects", "myproject", "sessions", "ao-1.json"), '{"project":"myproject"}');

		// Config without storageKey
		writeFileSync(
			configPath,
			["projects:", "  myproject:", "    path: /home/user/myproject", "    defaultBranch: main", ""].join("\n"),
		);
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			writeFileSync(
				join(aoBaseDir, "aaaaaa000000-myproject.migrated", "sessions", "archive", "ao-83_2026-04-20T14:30:52.000Z"),
				"project=myproject",
			);
			mkdirSync(join(aoBaseDir, "projects", "myproject", "sessions"), { recursive: true });
			writeFileSync(
			...
			writeFileSync(configPath, ["projects:", "  myproject:", "    path: /home/user/myproject", ""].join("\n"));
	...
	it("writes storageKey in original directory name format", async () => {
		...
		writeFileSync(configPath, ["projects:", "  myapp:", "    path: /home/user/myapp", ""].join("\n"));
	...
	it("restores config storageKey for bare hash directories", async () => {
		...
		writeFileSync(join(aoBaseDir, "aaaaaa000000.migrated", "sessions", "ao-1"), "project=myproject");
		mkdirSync(join(aoBaseDir, "projects", "myproject", "sessions"), { recursive: true });
		writeFileSync(join(aoBaseDir, "projects", "myproject", "sessions", "ao-1.json"), '{"project":"myproject"}');
	...
	it("moves worktrees back to restored hash dir before deleting project dir", async () => {
		...
		writeFileSync(join(aoBaseDir, "projects", "myproject", "worktrees", "ao-1", "README.md"), "# test");
		...
		expect(readFileSync(join(aoBaseDir, "aaaaaa000000-myproject", "worktrees", "ao-1", "README.md"), "utf-8")).toBe(
			"# test",
		);
...
describe.skipIf(process.platform === "win32")("migration edge cases", () => {
	...
	it("blocks migration when an active session uses a custom sessionPrefix", async () => {
		...
		writeFileSync(
			configPath,
			["projects:", "  backend-service:", "    path: /home/user/backend-service", "    sessionPrefix: be", ""].join(
				"\n",
			),
		);
	...
	it("duplicate sessions across hash dirs are renamed to a hash-suffixed alias, not archived", async () => {
		...
		const alias = JSON.parse(readFileSync(join(sessionsDir, "ao-1__from-aaaaaaaaaaaa.json"), "utf-8"));
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			try {
				...
				writeFileSync(
					join(hashDir, "sessions", "ao-1"),
					["project=myproject", "agent=claude-code", "branch=session/ao-1", `worktree=${oldWorktreePath}`].join("\n"),
				);
				...
				writeFileSync(join(oldClaudeDir, "session-uuid.jsonl"), '{"type":"user","message":{"content":"hello"}}\n');
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			try {
				...
				writeFileSync(
					join(hashDir, "sessions", "ao-1"),
					["project=myproject", "agent=codex", "branch=session/ao-1", `worktree=${oldWorktreePath}`].join("\n"),
				);

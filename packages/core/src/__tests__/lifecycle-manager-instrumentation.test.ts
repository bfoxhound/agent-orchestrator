...
import type { OpenCodeSessionManager, OrchestratorConfig, PRInfo, PluginRegistry, SessionMetadata } from "../types.js";
import {
	createMockNotifier,
	createMockPlugins,
	createMockRegistry,
...
describe("lifecycle.poll_failed", () => {
	...
	it("REGRESSION: summary must not interpolate raw error text (credential leak via FTS)", async () => {
		...
		vi.mocked(mockSessionManager.list).mockRejectedValue(new Error(`fatal: unable to access '${credentialUrl}/'`));

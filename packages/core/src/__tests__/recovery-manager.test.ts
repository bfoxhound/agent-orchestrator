...
import { DEFAULT_RECOVERY_CONFIG, type RecoveryAssessment, type RecoveryResult } from "../recovery/types.js";
...
describe("runRecovery activity events", () => {
	...
	it("emits recovery.session_failed for each session that recovery couldn't fix", async () => {
		...
		vi.spyOn(actionsModule, "executeAction").mockResolvedValueOnce(successResult).mockResolvedValueOnce(failedResult);

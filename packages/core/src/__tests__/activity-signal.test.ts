...
describe("summarizeActivityFreshness", () => {
	...
	it("classifies exact strong and weak boundaries correctly", () => {
		expect(summarizeActivityFreshness(new Date(now.getTime() - ACTIVITY_STRONG_WINDOW_MS), now)).toBe("strong");
		expect(summarizeActivityFreshness(new Date(now.getTime() - ACTIVITY_WEAK_WINDOW_MS), now)).toBe("weak");
	});
	...
	it("returns stale when the timestamp is older than the weak window", () => {
		expect(summarizeActivityFreshness(new Date(now.getTime() - ACTIVITY_WEAK_WINDOW_MS - 1), now)).toBe("stale");
	});
});
...
describe("activity signal helpers", () => {
	...
	it("detects positive idle evidence only for valid idle states with timestamps", () => {
		...
		expect(hasPositiveIdleEvidence(createActivitySignal("valid", { activity: "idle", source: "native" }))).toBe(false);
		expect(
			hasPositiveIdleEvidence(
	...
	it("treats all non-valid signals as weak evidence", () => {
		expect(isWeakActivityEvidence(createActivitySignal("valid", { activity: "active" }))).toBe(false);
		expect(isWeakActivityEvidence(createActivitySignal("stale", { activity: "active" }))).toBe(true);

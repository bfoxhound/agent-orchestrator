		...
		recordActivityEvent({
			...
			data: {
				...
				nested: {
					...
					attempts: [`xoxb webhook rejected: ${sentinels.slack}`, `s3 PutObject failed for ${sentinels.aws}`],
				},
			},
		});

		// 1. Direct row read — none of the sentinels survive in stored data.
		const rows = db.prepare("SELECT data FROM activity_events WHERE type = 'lifecycle.poll_failed'").all() as Array<{
			data: string;
		}>;

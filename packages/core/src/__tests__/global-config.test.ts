...
describe("global-config storage identity", () => {
	...
	it("generates deterministic external IDs from path and origin", () => {
		const repoPath = createRepo("agent-orchestrator", "https://github.com/OpenAI/agent-orchestrator.git");
	...
	it("throws if a generated external ID is already registered for a different path", () => {
		...
		expect(() => registerProjectInGlobalConfig("collision-source", "Collision", repoA, undefined, configPath)).toThrow(
	...
	it("allocates a suffixed generated session prefix for same-basename projects", () => {
		...
		const idA = registerProjectInGlobalConfig("agent-orchestrator", "AO A", repoA, undefined, configPath);
		const idB = registerProjectInGlobalConfig("agent-orchestrator", "AO B", repoB, undefined, configPath);
	...
	it("repairs wrapped local .yml configs without creating a .yaml sibling", () => {
		...
		writeFileSync(
			configPathYml,
			["projects:", "  wrapped-local-yml:", `    path: ${repoPath}`, "    agent: codex", "    runtime: tmux", ""].join(
				"\n",
			),
		);

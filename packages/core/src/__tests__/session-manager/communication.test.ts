...
describe("send", () => {
	...
	it("restores a dead session before sending the message", async () => {
		...
		vi.mocked(mockAgent.isProcessRunning).mockImplementation(async (handle) => handle.id !== "rt-old");
		...
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith(makeHandle("rt-restored"), "Please fix the review comments");
	});
	...
	it("throws when a killed session cannot be restored to a ready state for delivery", async () => {
		...
		try {
			...
			vi.mocked(mockAgent.isProcessRunning).mockImplementation(async (handle) => handle.id !== "rt-restored");
			vi.mocked(mockRuntime.create).mockResolvedValue(makeHandle("rt-restored"));
			...
			const rejection = expect(sendPromise).rejects.toThrow("Cannot send to session app-1: session is not running");
	...
	it("waits for spawning sessions to become interactive before considering restore", async () => {
		...
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith(makeHandle("rt-1"), "wait until interactive");
	});
	...
	it("resolves on restored session when confirmation never flips (soft success)", async () => {
		...
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith(makeHandle("rt-restored"), "retry after restore");
	});
	...
	it("falls back to session ID as runtime handle when no runtimeHandle stored", async () => {
		...
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith({ id: "app-1", runtimeName: "mock", data: {} }, "hello");
	});
	...
	it("confirms OpenCode delivery from session updated timestamps", async () => {
		...
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith(makeHandle("rt-1"), "confirm via updated timestamp");
	});
	...
	it("does not confirm OpenCode delivery from timestamp visibility alone", async () => {
		...
		expect(readFileSync(listLogPath, "utf-8").trim().split("\n").length).toBeGreaterThanOrEqual(2);
		expect(mockRuntime.sendMessage).toHaveBeenCalledWith(makeHandle("rt-1"), "do not confirm on visibility");
	}, 10000);
});

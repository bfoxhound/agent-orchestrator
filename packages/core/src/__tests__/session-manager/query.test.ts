...
describe("list", () => {
	...
	it("uses tmuxName fallback handle for runtime liveness checks when runtimeHandle is missing", async () => {
		...
		const deadRuntime: Runtime = {
			...mockRuntime,
			isAlive: vi.fn().mockImplementation(async (handle: RuntimeHandle) => handle.id !== expectedTmuxName),
		};
...
describe("get", () => {
	...
	it("reuses a single OpenCode session list lookup when multiple unmapped sessions are listed", async () => {
		...
		expect(sessions).toHaveLength(2);
		expect(readMetadataRaw(sessionsDir, "app-1")?.["opencodeSessionId"]).toBe("ses_get_discovered_1");
		expect(readMetadataRaw(sessionsDir, "app-2")?.["opencodeSessionId"]).toBe("ses_get_discovered_2");

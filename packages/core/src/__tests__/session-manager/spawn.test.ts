...
import { buildLifecycleMetadataPatch, createInitialCanonicalLifecycle } from "../../lifecycle-state.js";
...
import type { OrchestratorConfig, PluginRegistry, Runtime, Agent, Workspace, Tracker } from "../../types.js";
...
describe("spawn", () => {
	...
	it("forwards configured subagent to spawn launch when no override is provided", async () => {
		...
		expect(mockAgent.getLaunchCommand).toHaveBeenCalledWith(expect.objectContaining({ subagent: "oracle" }));
	});
	...
	it("prefers spawn subagent override over configured subagent", async () => {
		...
		expect(mockAgent.getLaunchCommand).toHaveBeenCalledWith(expect.objectContaining({ subagent: "librarian" }));
	});
	...
	it("fails on tracker auth errors", async () => {
		...
		await expect(sm.spawn({ projectId: "my-app", issueId: "INT-100" })).rejects.toThrow("Failed to fetch issue");
	...
	describe("rollback on failure", () => {
		it("cleans up reserved metadata when workspace creation fails", async () => {
			(mockWorkspace.create as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("workspace creation failed"));
			const sm = createSessionManager({ config, registry: mockRegistry });
		...
		it("destroys the worktree and cleans metadata when runtime creation fails", async () => {
			...
			(mockRuntime.create as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("runtime creation failed"));
			const sm = createSessionManager({ config, registry: mockRegistry });
		...
		it("still cleans subsequent resources when one cleanup step throws", async () => {
			...
			(mockWorkspace.destroy as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("destroy failed"));
			(mockRuntime.create as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("runtime creation failed"));
			const sm = createSessionManager({ config, registry: mockRegistry });
	...
	describe("spawnOrchestrator", () => {
		...
		it("ensureOrchestrator replaces the canonical session for delete strategy", async () => {
			...
			expect(mockWorkspace.create).toHaveBeenCalledWith(expect.objectContaining({ sessionId: "app-orchestrator" }));
		...
		it("cleans up reserved metadata on workspace creation failure", async () => {
			...
			await expect(sm.spawnOrchestrator({ projectId: "my-app" })).rejects.toThrow("workspace creation failed");
		...
		it("destroys the worktree and metadata when runtime creation fails", async () => {
			...
			await expect(sm.spawnOrchestrator({ projectId: "my-app" })).rejects.toThrow("runtime creation failed");
		...
		it("destroys the worktree when post-launch setup fails", async () => {
			...
			await expect(sm.spawnOrchestrator({ projectId: "my-app" })).rejects.toThrow("post-launch setup failed");
		...
		it("discovers OpenCode mapping by title when no archived mapping exists for new session id", async () => {
			...
			const mockBin = installMockOpencode(
				tmpDir,
				JSON.stringify([{ id: "ses_existing", title: "AO:app-orchestrator", updated: 1_772_777_000_000 }]),
				deleteLogPath,
			);
			process.env.PATH = `${mockBin}${PATH_SEP}${originalPath ?? ""}`;
		...
		it("reuses OpenCode session by title when orchestrator mapping is missing", async () => {
			...
			const mockBin = installMockOpencode(
				tmpDir,
				JSON.stringify([{ id: "ses_title_match", title: "AO:app-orchestrator", updated: 1_772_777_000_000 }]),
				deleteLogPath,
			);
			process.env.PATH = `${mockBin}${PATH_SEP}${originalPath ?? ""}`;
		...
		it("uses orchestratorModel when configured", async () => {
			...
			expect(mockAgent.getLaunchCommand).toHaveBeenCalledWith(expect.objectContaining({ model: "orchestrator-model" }));
		});
		...
		it("persists displayName derived from the orchestrator system prompt", async () => {
			...
			const meta = readMetadataRaw(sessionsDir, "app-orchestrator");
			expect(meta?.["displayName"]).toBe("Audit test coverage for session-manager and open PRs for gaps");
		});
		...
		it("throws for unknown project", async () => {
			...
			await expect(sm.spawnOrchestrator({ projectId: "nonexistent" })).rejects.toThrow("Unknown project");
		});

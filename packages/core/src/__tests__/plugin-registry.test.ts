...
describe("loadBuiltins", () => {
	...
	it("registers gitlab tracker and scm plugins from importFn", async () => {
		...
		expect(registry.list("tracker")).toContainEqual(expect.objectContaining({ name: "gitlab", slot: "tracker" }));
		expect(registry.list("scm")).toContainEqual(expect.objectContaining({ name: "gitlab", slot: "scm" }));
	});
	...
	it("registers alias-specific notifier instances for the same plugin", async () => {
		...
		expect(registry.list("notifier")).toContainEqual(expect.objectContaining({ name: "slack", slot: "notifier" }));
...
describe("loadFromConfig", () => {
	...
	it("loads external package plugins from config.plugins", async () => {
		...
		expect(registry.list("agent")).toContainEqual(expect.objectContaining({ name: "goose", slot: "agent" }));
	...
	it("loads local plugins relative to the config file", async () => {
		...
		writeFileSync(join(localPluginDir, "package.json"), JSON.stringify({ name: "role-qa", main: "dist/index.js" }));
...
describe("External plugin manifest validation", () => {
	it("accepts matching manifest.name and expectedPluginName", async () => {
		...
		const config = makeOrchestratorConfig({
			configPath: "/test/config.yaml",
			plugins: [{ name: "jira", source: "npm", package: "@acme/ao-plugin-tracker-jira", enabled: true }],
			projects: {
				proj1: {
					path: "/repos/test",
					repo: "org/test",
	...
	it("updates config with actual manifest.name for notifiers", async () => {
		...
		const config = makeOrchestratorConfig({
			configPath: "/test/config.yaml",
			plugins: [{ name: "teams", source: "npm", package: "@acme/ao-plugin-notifier-teams", enabled: true }],
			projects: {
				proj1: {
					path: "/repos/test",
					repo: "org/test",

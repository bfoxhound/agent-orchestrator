...
function appendAgentReportAuditEntry(dataDir: string, sessionId: SessionId, entry: AgentReportAuditEntry): void {
...
export function readAgentReportAuditTrail(dataDir: string, sessionId: SessionId): AgentReportAuditEntry[] {
	const auditFilePath = buildAuditFilePath(dataDir, sessionId);
	if (!existsSync(auditFilePath)) {
		return [];
	}
...
export function applyAgentReport(
	...
	if (trimmedPrUrl && parsedPrNumber !== undefined && urlPrNumber !== undefined && parsedPrNumber !== urlPrNumber) {
	...
	const existingPrNumber =
		existingPrNumberRaw && /^\d+$/.test(existingPrNumberRaw) ? Number.parseInt(existingPrNumberRaw, 10) : undefined;
	...
	const nextMetadata = mutateMetadata(
		...
		(existing) => {
			...
			if (isPRWorkflowReport(input.state)) {
				...
				const effectivePrNumber = prNumber ?? current.pr.number ?? existingPrNumber ?? parsedPrFromUrl?.number;
				const canAdvancePrState =
					effectivePrUrl !== undefined || effectivePrNumber !== undefined || current.pr.state !== "none";
...
export function readAgentReport(meta: Record<string, string> | null | undefined): AgentReport | null {
	...
	const prNumber = rawPrNumber && /^\d+$/.test(rawPrNumber) ? Number.parseInt(rawPrNumber, 10) : undefined;

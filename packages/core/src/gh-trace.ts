...
async function resolveGhBinary(): Promise<string> {
	...
	const dirs = (process.env["PATH"] ?? "").split(delimiter).filter((entry) => entry && entry !== aoBinDir);
...
function buildTraceEntry(args: string[], ctx: GhTraceContext, result: GhTraceResult, durationMs: number): GhTraceEntry {
...
export async function execGhObserved(args: string[], ctx: GhTraceContext, timeout: number = 30_000): Promise<string> {

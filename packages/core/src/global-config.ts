...
export const LocalProjectConfigSchema = z
	.object({
		...
		agentConfig: z
			.object({
				permissions: z.enum(["permissionless", "default", "auto-edit", "suggest", "skip"]).optional(),
		...
		orchestrator: z
			...
			.optional(),
		worker: z.object({ agent: z.string().optional(), agentConfig: z.object({}).passthrough().optional() }).optional(),
 ...
 */
export function loadGlobalConfig(configPath?: string, options: { alreadyLocked?: boolean } = {}): GlobalConfig | null {
...
export function writeLocalProjectConfig(
	...
	const validated = LocalProjectConfigSchema.parse(stripLocalIdentityFields(config as Record<string, unknown>));
...
function normalizeLegacyRepoValue(repoValue: unknown): z.infer<typeof GlobalRepoIdentitySchema> | undefined {
...
export function registerProjectInGlobalConfig(
	...
	return withFileLockSync(globalConfigLockPath(configPath), () => {
		...
		let existing = globalConfig.projects[projectId] as (GlobalProjectEntry & Record<string, unknown>) | undefined;

		if (!existing) {
			const hashedId = generateExternalId(normalizedProjectPath, originUrl);
			const hashedExisting = globalConfig.projects[hashedId] as
				(GlobalProjectEntry & Record<string, unknown>) | undefined;
		...
		const requestedSessionPrefix =
			existing?.sessionPrefix ?? localConfig?.sessionPrefix ?? generateSessionPrefix(basename(requestedProjectPath));
...
export function resolveProjectIdentity(
	...
	const defaultBranch =
		typeof entry.defaultBranch === "string" && entry.defaultBranch.length > 0 ? entry.defaultBranch : "main";
	const repoString =
	...
	if (localConfigResult.kind === "loaded" && localConfigResult.config) {
		return {
			...applyBehaviorDefaults(stripLocalIdentityFields(localConfigResult.config as Record<string, unknown>)),
			...identityFields,
		};
	}
...
export function migrateToGlobalConfig(oldConfigPath: string, globalConfigPath?: string): string {
	...
	for (const [projectId, project] of Object.entries(oldProjects)) {
		...
		const repoIdentity =
			typeof project["originUrl"] === "string" ? normalizeRepoIdentity(project["originUrl"] as string) : undefined;
		newGlobal.projects[projectId] = {
			projectId,
			path: projectPath,
			...(repoIdentity ? { repo: repoIdentity } : {}),
			...(typeof project["defaultBranch"] === "string" ? { defaultBranch: project["defaultBranch"] as string } : {}),
			source: "migrated",
			registeredAt: Math.floor(Date.now() / 1000),
			displayName: (project["name"] as string | undefined) ?? projectId,
			...(typeof project["sessionPrefix"] === "string" ? { sessionPrefix: project["sessionPrefix"] as string } : {}),
		};
	}

...
export function readMetadata(dataDir: string, sessionId: SessionId): SessionMetadata | null {
	...
	return {
		...
		prAutoDetect:
			raw["prAutoDetect"] === "off" || raw["prAutoDetect"] === "false" || raw["prAutoDetect"] === false
				? false
				: raw["prAutoDetect"] === "on" || raw["prAutoDetect"] === "true" || raw["prAutoDetect"] === true
					? true
					: undefined,
 ...
 */
export function readMetadataRaw(dataDir: string, sessionId: SessionId): Record<string, string> | null {
	const path = metadataPath(dataDir, sessionId);
 ...
 */
export function writeMetadata(dataDir: string, sessionId: SessionId, metadata: SessionMetadata): void {
	const path = metadataPath(dataDir, sessionId);
	...
	if (metadata.displayNameUserSet !== undefined) data["displayNameUserSet"] = metadata.displayNameUserSet;
 ...
 */
export function updateMetadata(dataDir: string, sessionId: SessionId, updates: Partial<Record<string, string>>): void {
	mutateMetadata(
		dataDir,
		sessionId,
		(existing) => {
				...
				if (content) {
					...
					if (raw) {
						...
						try {
							...
							console.warn(`[metadata] corrupt JSON at ${path}; preserved as ${corruptPath} before rewriting`);
						} catch {
...
export function readCanonicalLifecycle(dataDir: string, sessionId: SessionId): CanonicalSessionLifecycle | null {

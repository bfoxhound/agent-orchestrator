...
export function getActivityEventStats(): ActivityEventStats | null {
	...
	try {
		const totalRow = db.prepare("SELECT COUNT(*) as cnt FROM activity_events").all() as Record<string, unknown>[];
		const total = (totalRow[0]?.["cnt"] as number) ?? 0;

		const byKindRows = db.prepare("SELECT type, COUNT(*) as cnt FROM activity_events GROUP BY type").all() as Record<
			string,
			unknown
		>[];
		...
		const rangeRow = db.prepare("SELECT MIN(ts) as oldest, MAX(ts) as newest FROM activity_events").all() as Record<
			string,
			unknown
		>[];

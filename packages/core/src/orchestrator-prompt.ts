...
function removeOptionalSectionBlocks(template: string, data: OrchestratorPromptRenderData): string {
	...
	for (const [startKey, endKey, section] of templates) {
		...
		while (true) {
			...
			interpolated = replacement ? before + replacement + after : collapseOptionalGap(before, after);
		}
	}
...
function collapseOptionalGap(before: string, after: string): string {
	...
	return before.slice(0, before.length - trailingNewlines.length) + boundary + after.slice(leadingNewlines.length);
}

function hasRenderDataKey(data: OrchestratorPromptRenderData, key: string): key is OrchestratorPromptRenderKey {
...
function renderTemplate(template: string, data: OrchestratorPromptRenderData): string {
	const unresolvedPlaceholder = template.replace(/\{\{([a-zA-Z0-9_]+)\}\}/g, "").match(/\{\{[^}]+\}\}/);

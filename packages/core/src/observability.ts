	...
	if (typeof value === "object") {
		return Object.fromEntries(
			Object.entries(value)
				.slice(0, 25)
				.map(([key, entry]) => [key, shouldRedactKey(key) ? REDACTED_VALUE : sanitizeUnknown(entry, depth + 1)]),
		);
	}
	return String(value);
}
...
function appendObservabilityFailure(config: OrchestratorConfig, payload: Record<string, unknown>): void {
	try {
...
function mergeCounter(
	...
	if (source.lastSuccessAt && (!merged.lastSuccessAt || source.lastSuccessAt > merged.lastSuccessAt)) {
		merged.lastSuccessAt = source.lastSuccessAt;
	}
	if (source.lastFailureAt && (!merged.lastFailureAt || source.lastFailureAt > merged.lastFailureAt)) {
...
export function createProjectObserver(config: OrchestratorConfig, component: string): ProjectObserver {

...
export function withConfigSchema<T extends Record<string, unknown>>(config: T): T & { $schema: string } {
	const { $schema: providedSchema, ...rest } = config as {
		$schema?: unknown;
	};
	const schema =
		typeof providedSchema === "string" && providedSchema.trim().length > 0 ? providedSchema : CONFIG_SCHEMA_URL;
...
export function detectScmPlatform(host: string): ScmPlatform {
	...
	if (lower === "bitbucket.org" || lower.endsWith(".bitbucket.org") || lower.endsWith(".bitbucket.com"))
		return "bitbucket";
	return "unknown";
}
		...
		const pm = hasFile("pnpm-lock.yaml")
			...
			: hasFile("yarn.lock")
				...
				: hasFile("bun.lockb") || hasFile("bun.lock")
					? "bun"
					: "npm";
		const lang = hasFile("tsconfig.json") || hasFile("tsconfig.base.json") ? "typescript" : "javascript";
...
export function generateConfigFromUrl(options: GenerateConfigOptions): Record<string, unknown> {
	...
	const installCmd = projectInfo.packageManager ? JS_PACKAGE_MANAGERS[projectInfo.packageManager] : undefined;
	if (installCmd) {

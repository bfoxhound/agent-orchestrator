...
export async function validateSession(
	...
	if (agent && runtimeHandle) {
		...
		try {
			...
			if (processProbeSucceeded && !agentProcessRunning && indicatesLiveAgentActivity(agentActivity)) {
	...
	const recoveryRule = determineRecoveryRule(classification, signalDisagreement, metadataStatus, recoveryConfig);

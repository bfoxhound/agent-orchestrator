...
const RoleAgentSpecificConfigSchema = z
	.object({
		permissions: z.union([z.enum(["permissionless", "default", "auto-edit", "suggest"]), z.literal("skip")]).optional(),
		model: z.string().optional(),
		orchestratorModel: z.string().optional(),
		opencodeSessionId: z.string().optional(),
	})
...
const OrchestratorConfigSchema = z.object({
	...
	projects: z.record(
		z
			.string()
			.regex(/^[a-zA-Z0-9_-]+$/, "Project ID must match [a-zA-Z0-9_-]+ (no dots, slashes, or special characters)"),
		ProjectConfigSchema,
	),
	...
	for (const entry of externalEntries) {
		...
		if (seen.has(key)) {
			...
			const existingPlugin = plugins.find(
				(p) => (entry.package && p.package === entry.package) || (entry.path && p.path === entry.path),
			);
...
function buildEffectiveConfigFromFlatLocalPath(configPath: string, _localParsed: unknown): LoadedConfig | null {
	const globalConfigPath = getGlobalConfigPath();
...
export function loadConfig(configPath?: string): LoadedConfig {
	...
	const normalizedParsed =
		!isCanonicalGlobalConfig && shape === "wrapped" ? applyWrappedLocalStorageKeys(path, parsed) : parsed;

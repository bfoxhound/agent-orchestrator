...
function collectNotifierRegistrations(
	...
	if (exactMatch && typeof exactMatch === "object" && matchesNotifierPlugin(pluginName, pluginName, exactMatch)) {
	...
	return [...orderedMatches.entries()].map(([registrationName, rawConfig]) => ({
		registrationName,
		config: prepareConfig("notifier", pluginName, registrationName, rawConfig, config.configPath, isExternalLoad),
	}));
}
 ...
 */
function validateManifestName(manifest: PluginManifest, entry: ExternalPluginEntryRef, specifier: string): void {
...
function resolvePluginSpecifier(plugin: InstalledPluginConfig, config: OrchestratorConfig): string | null {
...
export function createPluginRegistry(): PluginRegistry {
	...
	function registerInstance(slot: PluginSlot, name: string, manifest: PluginManifest, instance: unknown): void {
		plugins.set(makeKey(slot, name), { manifest, instance });
	}

	function registerNotifier(plugin: PluginModule, config: OrchestratorConfig, isExternalLoad = false): void {
	...
	return {
			...
			for (const builtin of BUILTIN_PLUGINS) {
				...
				if (mod) {
					try {
						...
						process.stderr.write(`[plugin-registry] Failed to load built-in plugin "${builtin.name}": ${error}\n`);
						recordActivityEvent({
		...
		async loadFromConfig(config: OrchestratorConfig, importFn?: (pkg: string) => Promise<unknown>): Promise<void> {

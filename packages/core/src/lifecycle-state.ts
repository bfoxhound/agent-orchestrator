...
const CanonicalSessionLifecycleSchema = z.object({
	...
	session: z
		.object({
			...
			state: z.enum(["not_started", "working", "idle", "needs_input", "stuck", "detecting", "done", "terminated"]),
			reason: z.enum([
				"spawn_requested",
				"agent_acknowledged",
				"task_in_progress",
...
const TERMINAL_CANONICAL_SESSION_STATES: ReadonlySet<CanonicalSessionState> = new Set(["done", "terminated"]);

export function clearTerminalMarkersForNonTerminalState(lifecycle: CanonicalSessionLifecycle): void {
...
export function createInitialCanonicalLifecycle(kind: SessionKind, now = new Date()): CanonicalSessionLifecycle {
...
function synthesizeRuntimeState(
	...
	const handle = runtimeHandle ?? (meta["runtimeHandle"] ? safeJsonParse<RuntimeHandle>(meta["runtimeHandle"]) : null);
	if (handle || tmuxName) {
		return {
			state: "unknown",
			reason: "spawn_incomplete",
...
function synthesizeCanonicalLifecycle(
	...
	const sessionKind: SessionKind =
		options.sessionKind ??
		(meta["role"] === "orchestrator" || options.sessionId?.endsWith("-orchestrator") ? "orchestrator" : "worker");
	const now =
	...
	return {
		...
		session: {
			...
			terminatedAt: status === "killed" || status === "terminated" || status === "cleanup" ? now : null,
	...
	return {
		...
		runtime: {
			...
			reason: hasPayloadRuntimeReason
				? ((payloadRuntime?.reason as CanonicalRuntimeReason | undefined) ?? synthesized.runtime.reason)
			...
			handle: hasPayloadRuntimeHandle ? normalizeRuntimeHandle(payloadRuntime?.handle) : synthesized.runtime.handle,
 ...
 */
export function buildLifecycleMetadataPatch(lifecycle: CanonicalSessionLifecycle): Partial<Record<string, string>> {
	return {

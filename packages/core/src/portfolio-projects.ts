...
import { buildEffectiveProjectConfig, getGlobalConfigPath, loadGlobalConfig } from "./global-config.js";

const CACHE_TTL_MS = 30_000;
const configCache = new Map<string, { config: LoadedConfig; expiresAt: number }>();

export function resolveProjectConfig(entry: PortfolioProject): { config: LoadedConfig; project: ProjectConfig } | null {
	try {
			...
			const config = validateConfig({
				...
				projects: {
					[entry.configProjectKey]: {
						...effectiveProject,
						...(typeof effectiveProject["repo"] === "string" ? { repo: effectiveProject["repo"] } : {}),
					},
				},
			});

 ...
 */
export function isDetectingTimedOut(detectingStartedAt: string | undefined, now: Date = new Date()): boolean {
...
function resolveTerminalPRStateDecision(prState: PREnrichmentData["state"] | "open"): LifecycleDecision | null {
...
export function resolveProbeDecision(input: ProbeDecisionInput): LifecycleDecision | null {
	...
	if (input.runtimeProbe.state === "dead" && input.processProbe.state === "unknown" && input.canProbeRuntimeIdentity) {
		return createDetectingDecision({
			currentAttempts: input.currentAttempts,
			idleWasBlocked: input.idleWasBlocked,
	...
	if (input.runtimeProbe.state === "dead" && input.processProbe.state === "dead" && !recentActivitySupportsLiveness) {
		return createLifecycleDecision({

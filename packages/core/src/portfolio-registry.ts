...
import { getPortfolioDir, getPreferencesPath, getRegisteredPath, generateSessionPrefix } from "./paths.js";
import {
	getGlobalConfigPath,
	loadGlobalConfig,
...
function parsePortfolioRegistered(value: unknown): PortfolioRegistered | null {
	...
	for (const project of value.projects) {
		...
		projects.push({
			...
			...(typeof project.configProjectKey === "string" ? { configProjectKey: project.configProjectKey } : {}),
		});
	}
...
function parsePortfolioPreferences(value: unknown): PortfolioPreferences | null {
	...
	if ("projects" in value && value.projects !== undefined) {
		...
		for (const prefs of Object.values(value.projects)) {
			...
			if ("displayName" in prefs && prefs.displayName !== undefined && typeof prefs.displayName !== "string") {
				return null;
			}
		}
	}
			...
			? {
					projects: Object.fromEntries(
						Object.entries(value.projects).map(([id, prefs]) => [
							id,
							{
								...(isRecord(prefs) && typeof prefs.pinned === "boolean" ? { pinned: prefs.pinned } : {}),
								...(isRecord(prefs) && typeof prefs.enabled === "boolean" ? { enabled: prefs.enabled } : {}),
								...(isRecord(prefs) && typeof prefs.displayName === "string" ? { displayName: prefs.displayName } : {}),
							},
						]),
					),
				}
...
function applyPreferences(projects: PortfolioProject[], preferences: PortfolioPreferences): PortfolioProject[] {

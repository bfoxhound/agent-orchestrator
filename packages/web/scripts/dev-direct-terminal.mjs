...
function shouldFallback(code, stderr) {
	...
	return stderr.includes("createIpcServer") && stderr.includes("EPERM") && stderr.includes("operation not permitted");
}
...
function runTsx(args, { allowFallback }) {
	...
	child.on("close", (code, signal) => {
		...
		if (allowFallback && shouldFallback(code ?? 1, stderrBuffer)) {
			console.warn("[dev:direct-terminal] tsx watch IPC setup failed; falling back to non-watch mode.");

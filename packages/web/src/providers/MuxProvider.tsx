...
interface MuxContextValue {
	subscribeTerminal: (id: string, callback: (data: string) => void, projectId?: string) => () => void;
...
export function MuxProvider({ children }: { children: ReactNode }) {
	...
	const openedTerminalsRef = useRef(new Map<string, { id: string; projectId?: string; tmuxName?: string }>());
	...
	const resizeTerminal = useCallback((id: string, cols: number, rows: number, projectId?: string) => {
		if (wsRef.current?.readyState === WebSocket.OPEN) {
			const msg: ClientMessage = {
				ch: "terminal",
				id,

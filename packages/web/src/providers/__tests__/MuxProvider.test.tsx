...
describe("MuxProvider message handling", () => {
	...
	it("tracks opened terminals on 'opened' message", async () => {
		...
		try {
			...
			act(() => ws1.simulateMessage({ ch: "terminal", id: "s1", projectId: "project-a", type: "opened" }));
	...
	it("removes exited terminal from opened set", async () => {
		...
		try {
			...
			act(() => ws1.simulateMessage({ ch: "terminal", id: "s-x", projectId: "project-a", type: "opened" }));
			act(() =>
				ws1.simulateMessage({
					ch: "terminal",
					id: "s-x",
			...
			const reopened = ws2.sentMessages.some((m) => {
				const p = JSON.parse(m) as Record<string, unknown>;
				return p.ch === "terminal" && p.id === "s-x" && p.projectId === "project-a" && p.type === "open";
			});
...
describe("MuxProvider terminal operations", () => {
	...
	it("closeTerminal sends close message", async () => {
		...
		expect(
			ws.sentMessages.some((m) => {
				const p = JSON.parse(m) as Record<string, unknown>;
				return p.ch === "terminal" && p.type === "close" && p.id === "session-abc" && p.projectId === "project-a";
			}),
		).toBe(true);
	});
	...
	it("subscribeTerminal sends open for untracked terminal", async () => {
		...
		expect(
			ws.sentMessages.some((m) => {
				const p = JSON.parse(m) as Record<string, unknown>;
				return p.ch === "terminal" && p.type === "open" && p.id === "session-new" && p.projectId === "project-a";
			}),
		).toBe(true);
	});
	...
	it("keeps terminal subscribers scoped by project id", async () => {
		...
		expect(
			ws.sentMessages.some((m) => {
				const p = JSON.parse(m) as Record<string, unknown>;
				return p.ch === "terminal" && p.type === "open" && p.id === "app-1" && p.projectId === "alpha";
			}),
		).toBe(true);
		expect(
			ws.sentMessages.some((m) => {
				const p = JSON.parse(m) as Record<string, unknown>;
				return p.ch === "terminal" && p.type === "open" && p.id === "app-1" && p.projectId === "beta";
			}),
		).toBe(true);
	});
});

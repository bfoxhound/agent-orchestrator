...
describe("useMediaQuery", () => {
	...
	it("cleans up the event listener on unmount", async () => {
		...
		const addedListener = (currentMQL.addEventListener as ReturnType<typeof vi.fn>).mock.calls[0][1];
		const removedListener = (currentMQL.removeEventListener as ReturnType<typeof vi.fn>).mock.calls[0][1];

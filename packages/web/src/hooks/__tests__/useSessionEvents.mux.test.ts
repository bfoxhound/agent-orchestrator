...
describe("useSessionEvents - mux", () => {
	...
	it("triggers refresh when mux patch contains unknown id", async () => {
		...
		await waitFor(() => {
			expect(fetch).toHaveBeenCalledWith("/api/sessions?project=proj", expect.objectContaining({ cache: "no-store" }));
		});
	});
	...
	it("does not warn when an in-flight refresh is aborted on unmount", async () => {
		...
		expect(warnSpy).not.toHaveBeenCalledWith("[useSessionEvents] refresh failed:", expect.anything());
		vi.useRealTimers();
	});
});

...
import { GET as sessionDetailGET, PATCH as sessionDetailPATCH } from "@/app/api/sessions/[id]/route";
...
describe("API Routes", () => {
	...
	describe("GET /api/sessions", () => {
		...
		it("activeOnly keeps working sessions with stale terminatedAt annotations", async () => {
			const staleLifecycle = createInitialCanonicalLifecycle("worker", new Date("2026-05-13T19:13:20.146Z"));
		...
		it("returns per-project orchestrators and excludes them from worker sessions", async () => {
			(mockSessionManager.listCached as ReturnType<typeof vi.fn>).mockResolvedValueOnce(multiProjectSessions);
			...
			expect(data.sessions.map((session: { id: string }) => session.id)).toEqual(["backend-3", "docs-2"]);
			expect(data.stats.totalSessions).toBe(2);
		});

		it("supports project-scoped session queries for orchestrator detail views", async () => {
			(mockSessionManager.listCached as ReturnType<typeof vi.fn>).mockImplementationOnce(async (projectId?: string) =>
				multiProjectSessions.filter((session) => !projectId || session.projectId === projectId),
			);

			const res = await sessionsGET(makeRequest("http://localhost:3000/api/sessions?project=docs-app"));
			expect(res.status).toBe(200);
			const data = await res.json();

			expect(data.orchestratorId).toBe("docs-orchestrator");
			expect(data.orchestrators).toEqual([{ id: "docs-orchestrator", projectId: "docs-app", projectName: "Docs App" }]);
		...
		it("uses the live session list when fresh=true is requested", async () => {
			(mockSessionManager.list as ReturnType<typeof vi.fn>).mockImplementationOnce(async (projectId?: string) =>
				multiProjectSessions.filter((session) => !projectId || session.projectId === projectId),
			);

			const res = await sessionsGET(makeRequest("http://localhost:3000/api/sessions?project=docs-app&fresh=true"));
			expect(res.status).toBe(200);
			const data = await res.json();
		...
		it("keeps dead orchestrators as the fallback project-scoped payload when none are live", async () => {
			...
			expect(data.orchestrators).toEqual([{ id: "my-app-orchestrator-0", projectId: "my-app", projectName: "My App" }]);
			expect(data.sessions).toEqual([]);
		});
		...
		it("enriches all PRs concurrently, not sequentially", async () => {
			...
			const metadataSpy = vi.spyOn(serialize, "enrichSessionsMetadata").mockResolvedValue(undefined);
	...
	describe("GET /api/sessions/[id]", () => {
		it("returns partial session data when metadata and PR enrichment stall", async () => {
			const metadataSpy = vi
				.spyOn(serialize, "enrichSessionsMetadata")
				.mockImplementation(() => new Promise<void>(() => {}));
			const prSpy = vi.spyOn(serialize, "enrichSessionPR").mockImplementation(() => new Promise<boolean>(() => {}));

			const responsePromise = sessionDetailGET(makeRequest("http://localhost:3000/api/sessions/backend-7"), {
	...
	describe("PATCH /api/sessions/[id]", () => {
		...
		it("persists a sanitized displayName and sets the user-set flag", async () => {
			const res = await sessionDetailPATCH(patchRequest("backend-7", { displayName: "  PR 432 review  " }), {
				params: Promise.resolve({ id: "backend-7" }),
			});
	...
	describe("GET /api/runtime/terminal", () => {
		...
		it("returns null proxyWsPath when TERMINAL_WS_PATH is absent", async () => {
			await withEnv({ TERMINAL_WS_PATH: undefined, NEXT_PUBLIC_TERMINAL_WS_PATH: undefined }, async () => {
				const res = await runtimeTerminalGET();
				expect(res.status).toBe(200);
				const data = await res.json();
	...
	describe("POST /api/orchestrators", () => {
		...
		it("returns 500 when orchestrator spawn fails", async () => {
			(mockSessionManager.spawnOrchestrator as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("boom"));
	...
	describe("POST /api/sessions/:id/remap", () => {
		...
		it("returns 404 when session is missing", async () => {
			(mockSessionManager.remap as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new SessionNotFoundError("missing"));
	...
	describe("GET /api/sessions/patches", () => {
		...
		it("filters by project query param", async () => {
			const res = await patchesGET(makeRequest("http://localhost:3000/api/sessions/patches?project=my-app"));
			expect(res.status).toBe(200);
			const data = await res.json();
			expect(Array.isArray(data.sessions)).toBe(true);
		});

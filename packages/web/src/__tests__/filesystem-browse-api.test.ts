...
describe("/api/filesystem/browse", () => {
	...
	it("returns 400 when the requested path contains ..", async () => {
		const response = await browseGET(makeRequest("/api/filesystem/browse?path=projects/../secrets"));
	...
	it("returns 400 for an absolute path outside HOME", async () => {
		...
		const outsidePath = process.platform === "win32" ? encodeURIComponent("C:\\Windows") : "/etc";
	...
	it("returns minimal metadata only for a valid path inside HOME", async () => {
		...
		for (const entry of body.entries) {
			...
			expect(Object.keys(entry).sort()).toEqual(["hasLocalConfig", "isDirectory", "isGitRepo", "name"]);
		}
	...
	it("redirects the legacy browse endpoint to the new route", async () => {
		...
		expect(response.headers.get("location")).toBe("http://localhost:3000/api/filesystem/browse?path=~/repo");
	});
});

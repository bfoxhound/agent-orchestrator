...
describe("API mutation routes emit activity events (api source)", () => {
	describe("MUST emits — session mutations", () => {
		...
		it("POST /api/sessions/:id/send does NOT include the raw message in data", async () => {
			...
			const calls = recorded.mock.calls.filter((c) => (c[0] as { kind: string }).kind === "api.session_message_sent");
	...
	describe("SHOULD emits — failure paths", () => {
		...
		it("POST /api/spawn does not emit api.session_spawn_failed when core spawn throws", async () => {
			(mockSessionManager.spawn as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("runtime failed"));
			const req = makeRequest("/api/spawn", {
				method: "POST",
			...
			expect(
				recorded.mock.calls.some(([event]) => (event as { kind: string }).kind === "api.session_spawn_failed"),
			).toBe(false);
		});
		...
		it.each([
			...
			async (_name, clean, method) => {
				...
				expect(res.status).toBe(500);
				expect(
					recorded.mock.calls.some(([event]) => (event as { kind: string }).kind === "api.orchestrator_spawn_failed"),
				).toBe(false);
			},
		);

		it("POST /api/sessions/:id/send emits api.session_message_failed on unexpected error", async () => {
			(mockSessionManager.send as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("write failed"));
			const req = makeRequest("/api/sessions/backend-3/send", {
				method: "POST",
				body: JSON.stringify({ message: "hi" }),
				headers: { "Content-Type": "application/json" },

...
import { generateExternalId, loadGlobalConfig, registerProjectInGlobalConfig } from "@aoagents/ao-core";
...
describe("POST /api/projects", () => {
	...
	it("registers a project and invalidates services cache", async () => {
		...
		expect(
			saved?.projects[generateExternalId(realpathSync(repoDir), "git@github.com:acme/demo.git")]?.defaultBranch,
		).toBe("trunk");
	});
	...
	it("migrates the current local config into the global registry before adding a new project", async () => {
		...
		const { POST } = await import("@/app/api/projects/route");
		const response = await POST(makeRequest({ projectId: "added", name: "Added", path: addedRepoDir }));
	...
	it("rejects non-repository directories", async () => {
		...
		const { POST } = await import("@/app/api/projects/route");
		const response = await POST(makeRequest({ projectId: "downloads", name: "Downloads", path: repoDir }));
	...
	it("reconnects to the existing project ID when the same path is registered again", async () => {
		...
		const { POST } = await import("@/app/api/projects/route");
		const firstResponse = await POST(makeRequest({ projectId: "existing-app", name: "Existing", path: repoDir }));
		const firstBody = (await firstResponse.json()) as { projectId: string };

		const response = await POST(makeRequest({ projectId: "second-app", name: "Second", path: aliasDir }));
	...
	it("registers same-basename projects with distinct hashed IDs", async () => {
		...
		const response = await POST(makeRequest({ projectId: "agent-orchestrator", name: "AO B", path: repoB }));

		const idA = generateExternalId(realpathSync(repoA), "git@github.com:acme/agent-orchestrator-a.git");
		const idB = generateExternalId(realpathSync(repoB), "git@github.com:acme/agent-orchestrator-b.git");
		expect(response.status).toBe(201);
		await expect(response.json()).resolves.toMatchObject({
			projectId: idB,
		});

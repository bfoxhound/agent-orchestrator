...
describe("SessionCard", () => {
	...
	it("stops propagation when clicking a passing CI chip link", () => {
		...
		const pr = makePR({
			state: "open",
			ciStatus: "passing",
			ciChecks: [{ name: "build", status: "passed", url: "https://github.com/owner/repo/runs/333" }],
			reviewDecision: "approved",
			mergeability: {
				mergeable: true,
				ciPassing: true,
	...
	it("hides quick reply presets for terminal sessions", () => {
		...
		expect(screen.queryByRole("textbox", { name: /type a reply to the agent/i })).not.toBeInTheDocument();
	...
	it("does not show sent state or clear reply text when quick reply send fails", async () => {
		...
		await waitFor(() => {
			...
			expect(screen.getByRole("textbox", { name: /type a reply to the agent/i })).toHaveValue("please continue");

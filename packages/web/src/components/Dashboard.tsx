...
function DoneCard({ session, onRestore }: { session: DashboardSession; onRestore: (id: string) => void }) {
	const title = (!session.summaryIsFallback && session.summary) || session.issueTitle || session.summary || session.id;
...
function DashboardInner({
	...
	const allSessionPrefixes = useMemo(() => projects.map((p) => p.sessionPrefix ?? p.id), [projects]);
	...
	const connectionStatus: "connected" | "reconnecting" | "disconnected" =
		mux?.status === "disconnected" ? "disconnected" : mux?.status === "connected" ? "connected" : "reconnecting";
	...
	const [activeOrchestrators, setActiveOrchestrators] = useState<DashboardOrchestratorLink[]>(orchestratorLinks);
	...
	const showDebugBundleButton =
		!isMobile && (process.env.NODE_ENV === "development" || debugParam === "1" || debugParam === "true");
	...
	const currentProjectOrchestrator = useMemo(
		() =>
			projectId ? (activeOrchestrators.find((orchestrator) => orchestrator.projectId === projectId) ?? null) : null,
	...
	const projectOverviews = useMemo(() => {
		...
		return projects.map((project) => {
			...
			return {
				project,
				orchestrator: activeOrchestrators.find((orchestrator) => orchestrator.projectId === project.id) ?? null,
	...
	const handleSend = useCallback(
		async (sessionId: string, message: string) => {
			try {
				...
				const toastShown =
					error instanceof Error && "toastShown" in error && (error as Error & { toastShown?: boolean }).toastShown;
				if (!toastShown) {
	...
	const handleSpawnOrchestrator = async (project: ProjectInfo) => {
		setSpawningProjectIds((current) => (current.includes(project.id) ? current : [...current, project.id]));
		...
		<div className="dashboard-main--desktop">
								...
								<div className="topbar-session-pills">
									{grouped.working.length > 0 ? (
										<div className="topbar-status-pill topbar-status-pill--active">
											...
											<span className="topbar-status-pill__label">{grouped.working.length} working</span>
										</div>
									) : null}
									{grouped.merge.length + grouped.action.length + grouped.respond.length + grouped.review.length > 0 ? (
										<div className="topbar-status-pill topbar-status-pill--waiting-for-input">
											...
											<span className="topbar-status-pill__label">
												{grouped.merge.length + grouped.action.length + grouped.respond.length + grouped.review.length}{" "}
												need attention
											</span>
										</div>
									) : null}
			...
			<main className="dashboard-main flex-1 min-h-0 overflow-hidden">
				...
				<div className="dashboard-main__subhead">
					...
					<p className="dashboard-main__subtitle">Live agent sessions, pull requests, and merge status.</p>
				</div>
				...
				<div className="dashboard-main__body">
					...
					{anyRateLimited && !rateLimitDismissed && (
						<div className="dashboard-alert mb-4 flex items-center gap-2.5 border border-[color-mix(in_srgb,var(--color-status-attention)_25%,transparent)] bg-[var(--color-tint-yellow)] px-3.5 py-2.5 text-[11px] text-[var(--color-status-attention)]">
							...
							<span className="flex-1">
								GitHub API rate limited — PR data (CI status, review state, sizes) may be stale. Will retry
					...
					{!allProjectsView && currentProjectSpawnError ? (
						<p className="mt-3 text-[11px] text-[var(--color-status-error)]">{currentProjectSpawnError}</p>
					) : null}
	...
	return (
		<SidebarContext.Provider value={{ onToggleSidebar: handleToggleSidebar, mobileSidebarOpen }}>
			...
			<div className="dashboard-app-shell">
				<div className={`dashboard-shell--desktop${sidebarCollapsed ? " dashboard-shell--sidebar-collapsed" : ""}`}>
					...
					{mobileSidebarOpen && <div className="sidebar-mobile-backdrop" onClick={() => setMobileSidebarOpen(false)} />}
					{mainPanel}
				</div>
			</div>
			{bottomSheet}
					...
					return (
						...
						>
							<div className="mb-4 flex items-start justify-between gap-3">
								<div>
									<h2 className="text-[14px] font-semibold text-[var(--color-text-primary)]">{project.name}</h2>
							...
							<div className="border-t border-[var(--color-border-subtle)] pt-3">
								...
								{spawnErrors[project.id] ? (
									<p className="mt-2 text-[11px] text-[var(--color-status-error)]">{spawnErrors[project.id]}</p>
								) : null}
							</div>
						</section>
					);
...
function ProjectMetric({ label, value, tone }: { label: string; value: number; tone: string }) {
	return (
		<div className="min-w-[78px] border border-[var(--color-border-subtle)] px-2.5 py-2">
			<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">{label}</div>

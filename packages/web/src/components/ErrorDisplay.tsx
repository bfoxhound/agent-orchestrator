				...
				<div className="flex flex-col gap-6">
					<div className="flex flex-col gap-4 sm:flex-row sm:items-start">
						...
						<div className="min-w-0 flex-1">
							...
							<h1 className="text-[24px] font-semibold tracking-[-0.04em] text-[var(--color-text-primary)]">{title}</h1>
							<p className="mt-2 max-w-[34rem] text-[14px] leading-6 text-[var(--color-text-secondary)]">{message}</p>
						</div>
					</div>
					...
					{hasDetails ? (
						...
						>
							...
							<div className="border-t px-4 py-4" style={{ borderColor: "var(--color-border-subtle)" }}>
								...
								{error?.message ? (
									<p className="mb-3 text-[12px] leading-5 text-[var(--color-text-secondary)]">{error.message}</p>
								) : null}
								{error?.stack ? (

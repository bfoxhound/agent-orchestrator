...
function SessionCardView({ session, onSend, onKill, onMerge, onRestore }: SessionCardProps) {
	...
	const visiblePassingChecks =
		!rateLimited && pr && !prUnenriched ? pr.ciChecks.filter((check) => check.status === "passed").slice(0, 3) : [];
					...
					<div className="done-expand-section px-3.5 py-3">
						{session.summary && pr?.title && session.summary !== pr.title && (
							<div className="mb-3">
								...
								<p className="text-[12px] leading-relaxed text-[var(--color-text-secondary)]">{session.summary}</p>
							</div>
						)}
									...
									{prUnenriched ? (
										...
										<>
											...
											<span className="mt-1 inline-flex items-center gap-2">
												...
												<span className="text-[var(--color-text-muted)]">·</span>
												<span className="text-[10px] text-[var(--color-text-muted)]">review: {pr.reviewDecision}</span>
											</span>
										</>
									)}
								</p>
	...
	return (
		...
		>
			...
			<div className="session-card__body flex min-h-0 flex-1 flex-col">
				...
				{truthLine && (
					<div className="px-[10px] pb-[5px]">
						<p className="text-[10px] leading-relaxed text-[var(--color-text-tertiary)]">{truthLine}</p>
					</div>
				)}
						...
						{alerts.slice(0, 3).map((alert) => (
							<div key={alert.key} className={cn("alert-row", `alert-row--${alert.type}`)}>
								...
								<span className="alert-row__text">
									<a href={alert.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}>
								...
								{alert.actionLabel && (
									...
									>
										{sendingAction === alert.key ? "sent!" : failedAction === alert.key ? "failed" : alert.actionLabel}
									</button>
								)}
							</div>
						))}
							...
							<>
								<div className="card__presets">
									...
									<button
										...
										disabled={sendingQuickReply !== null}
									>
										{sendingQuickReply === "abort" ? "Sending..." : sentQuickReply === "abort" ? "Sent" : "Abort"}
									</button>
									<button
										className="card__preset"
										...
										disabled={sendingQuickReply !== null}
									>
										{sendingQuickReply === "skip" ? "Sending..." : sentQuickReply === "skip" ? "Sent" : "Skip"}
									</button>
								</div>
...
function getAlerts(session: DashboardSession): Alert[] {
	...
	const ciIsFailing =
		pr.ciStatus === CI_STATUS.FAILING || lifecyclePrReason === "ci_failing" || lifecycleStatus === "ci_failed";

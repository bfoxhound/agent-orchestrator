			...
			.then(([Terminal, FitAddon, WebLinksAddon]) => {
				...
				const handleFontsLoadingDone = () => {
					...
					try {
						...
						fitAddon.current.fit();
						resizeTerminalMux(sessionId, terminalInstance.current.cols, terminalInstance.current.rows, projectId);
					} catch {
						// Ignore fit errors
					}
				};
				...
				const fontsListenerAttached = !!fontsFace && typeof fontsFace.addEventListener === "function";

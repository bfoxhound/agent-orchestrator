...
export function TerminalControls({
	...
	async function handleReload(): Promise<void> {
		...
		try {
			...
			if (!commandToSend) {
				...
				if (typeof remapData.opencodeSessionId !== "string" || remapData.opencodeSessionId.length === 0) {
	...
	const fontSizeControls = (
		<div className="flex items-center">
			...
			<span className="w-9 text-center text-xs font-medium text-[var(--color-text-secondary)]">{fontSize}px</span>
			<button

...
describe("ProjectSidebar", () => {
	...
	it("renders the empty-state header with the + button when no projects are configured", () => {
		render(<ProjectSidebar projects={[]} sessions={[]} activeProjectId={undefined} activeSessionId={undefined} />);
	...
	it("renders a collapsed empty rail when collapsed with no projects", () => {
		const { container } = render(
			<ProjectSidebar projects={[]} sessions={[]} activeProjectId={undefined} activeSessionId={undefined} collapsed />,
		);
	...
	it("renders the compact sidebar header and project rows", () => {
		render(
			<ProjectSidebar projects={projects} sessions={[]} activeProjectId="project-1" activeSessionId={undefined} />,
		);
	...
	it("marks the active project row as the current page", () => {
		render(
			<ProjectSidebar projects={projects} sessions={[]} activeProjectId="project-2" activeSessionId={undefined} />,
		);

		expect(screen.getByRole("button", { name: /^Project Two 0$/ })).toHaveAttribute("aria-current", "page");
		expect(screen.getByRole("button", { name: /^Project One 0$/ })).not.toHaveAttribute("aria-current");
	});
	...
	it("renders degraded projects distinctly and navigates them to the project page", () => {
		render(
			<ProjectSidebar
				projects={[...projects, { id: "broken-project", name: "Broken Project", resolveError: "Bad config" }]}
				sessions={[]}
	...
	it("keeps killed sessions visible when they still need attention", () => {
		...
		expect(screen.getByRole("link", { name: "Open Runtime missing but needs review" })).toBeInTheDocument();

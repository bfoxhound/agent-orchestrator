...
describe("Dashboard unified layout (mobile viewport)", () => {
	...
	it("shows hamburger toggle button in topbar on mobile", () => {
		render(<Dashboard initialSessions={[makeSession()]} projects={[{ id: "my-app", name: "My App" }]} />);
	...
	it("opens the done bar and restores completed sessions", async () => {
		...
		expect(screen.getByRole("link", { name: "#209" })).toHaveAttribute("href", "https://github.com/acme/app/pull/100");
	...
	it("kill button requires a two-click confirmation before firing", async () => {
		const fetchSpy = vi.fn(() => Promise.resolve({ ok: true, text: () => Promise.resolve("") } as Response));

...
describe("DirectTerminal render", () => {
	...
	it("renders the shared accent chrome for orchestrator terminals", async () => {
		render(<DirectTerminal sessionId="ao-orchestrator" tmuxName="ao-orchestrator" variant="orchestrator" />);
	...
	it("keeps restart and fullscreen actions available in chromeless mode", async () => {
		render(<DirectTerminal sessionId="ao-opencode" tmuxName="ao-opencode" chromeless isOpenCodeSession />);

		await waitFor(() => expect(screen.getByRole("button", { name: "Restart OpenCode session" })).toBeInTheDocument());

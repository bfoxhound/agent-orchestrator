...
export function SessionDetailPRCard({ pr, metadata, lifecyclePrReason, onAskAgentToFix }: SessionDetailPRCardProps) {
	...
	return (
		<div className={cn("session-detail-pr-card", allGreen && "session-detail-pr-card--green")}>
			<div className="session-detail-pr-card__row">
				<a href={pr.url} target="_blank" rel="noopener noreferrer" className="session-detail-pr-card__title-link">
				...
				{pr.state === "merged" ? <span className="session-detail-pr-card__diff-label">Merged</span> : null}
			</div>

			{showConflictActions ? (
				<div className="session-detail-pr-card__merge-actions" role="group" aria-label="Resolve merge conflicts">
					<a href={compareUrl} target="_blank" rel="noopener noreferrer" className="session-detail-pr-merge-action">
				...
				{allGreen ? (
					...
					blockerIssues.map((issue) => (
						...
						>
							...
							{issue.notified ? <span className="session-detail-blocker-chip__note">· notified</span> : null}
						</span>
					))
				)}

...
import { type DashboardSession, isDashboardSessionRestorable, isDashboardSessionTerminal } from "@/lib/types";
...
const DirectTerminal = dynamic(() => import("./DirectTerminal").then((m) => ({ default: m.DirectTerminal })), {
	ssr: false,
	...
	loading: () => <div className="h-full w-full animate-pulse rounded bg-[var(--color-bg-primary)]" />,
});
...
export function SessionDetail({
	...
	const opencodeSessionId =
		typeof session.metadata["opencodeSessionId"] === "string" && session.metadata["opencodeSessionId"].length > 0
			? session.metadata["opencodeSessionId"]
			: undefined;
	const reloadCommand = opencodeSessionId ? `/exit\nopencode --session ${opencodeSessionId}\n` : undefined;
	...
	const handleRelaunchClean = useCallback(async () => {
		const confirmed = window.confirm("This will discard the current orchestrator's conversation and state. Continue?");
		if (!confirmed) return;
			...
			<main className="session-detail-page flex-1 min-h-0 flex flex-col bg-[var(--color-bg-base)]">
				...
				<div className="flex-1 min-h-0 flex flex-col">
					{!showTerminal ? (
						...
						<SessionEndedSummary session={session} headline={headline} pr={pr} dashboardHref={dashboardHref} />
					) : (
						<DirectTerminal

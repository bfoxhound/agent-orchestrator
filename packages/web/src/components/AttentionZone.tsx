...
function AttentionZoneView({
	...
	const visibleSessions = isAccordion && compactMobile && !showAll ? sessions.slice(0, 5) : sessions;
					...
					{sessions.length > 0 ? (
						<div className={compactMobile ? "mobile-session-list" : "flex flex-col gap-2 p-3"}>
							{visibleSessions.map((session) =>
								compactMobile ? (
									<MobileSessionRow key={session.id} session={session} level={level} onPreview={onPreview} />
								) : (
									<SessionCard
										key={session.id}
										session={session}
							...
							{compactMobile && hiddenCount > 0 ? (
								<button type="button" className="mobile-session-list__view-all" onClick={() => setShowAll(true)}>
...
function MobileSessionRow({
	...
	const meta = [session.branch, session.pr ? `PR #${session.pr.number}` : null, session.issueLabel].filter(Boolean);
...
function SessionStateChip({ session, level }: { session: DashboardSession; level: AttentionLevel }) {

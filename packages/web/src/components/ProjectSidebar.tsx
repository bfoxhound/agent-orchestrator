...
const SessionRow = memo(function SessionRow({
	...
	const effectiveDisplayName =
		pendingRename !== undefined ? pendingRename : session.displayNameUserSet ? (session.displayName ?? "") : "";
	const title = effectiveDisplayName !== "" ? effectiveDisplayName : (session.branch ?? getSessionTitle(session));
	const sessionHref = projectSessionPath(session.projectId, session.id);

	return (
		<div className={cn("project-sidebar__sess-row group", isActive && "project-sidebar__sess-row--active")}>
			<a
				href={sessionHref}
				onClick={(e) => {
			...
			>
				...
				<div className="flex-1 min-w-0">
					<span className={cn("project-sidebar__sess-label", isActive && "project-sidebar__sess-label--active")}>
						{title}
					</span>
					{showSessionId ? (
...
function ProjectSidebarInner({
	...
	const [expandedProjects, setExpandedProjects] = useState<Set<string>>(
		() => loadExpandedProjects() ?? new Set(activeProjectId && activeProjectId !== "all" ? [activeProjectId] : []),
	);
	...
	useEffect(() => {
		setRemovedProjectIds((prev) => {
			...
			const next = new Set([...prev].filter((projectId) => !projects.some((project) => project.id === projectId)));
	...
	const allPrefixes = useMemo(() => visibleProjects.map((p) => p.sessionPrefix ?? p.id), [visibleProjects]);
		...
		<aside className="project-sidebar flex h-full flex-col">
			...
			{error && sessions && sessions.length > 0 ? (
				...
				>
					...
					{onRetry ? (
						<button type="button" onClick={onRetry} className="font-medium text-[var(--color-link)] hover:underline">
							Retry
						</button>
					) : null}
				</div>
				...
				{visibleProjects.map((project) => {
					...
					return (
						<div key={project.id} className="project-sidebar__project">
							...
							<div className="project-sidebar__proj-row flex items-center">
								{isDegraded ? (
									...
									<button
										...
										className={cn("project-sidebar__proj-toggle", isActive && "project-sidebar__proj-toggle--active")}
								...
								{!isDegraded && orchestratorLink && (
									<a
										...
										onClick={(e) => {
											...
											e.preventDefault();
											e.stopPropagation();
											navigate(projectSessionPath(project.id, orchestratorLink.id), orchestratorSession ?? undefined);
										}}
										className="project-sidebar__proj-action"
							...
							{isDegraded ? <div className="project-sidebar__degraded-note">Config needs repair</div> : null}
			...
			<div className="project-sidebar__footer">
				<div className="flex items-center gap-1 border-t border-[var(--color-border-subtle)] px-2 py-2">
					...
					<button
						...
						className={cn("project-sidebar__footer-btn", showKilled && "project-sidebar__footer-btn--active")}
					...
					<button
						...
						className={cn("project-sidebar__footer-btn", showDone && "project-sidebar__footer-btn--active")}
					...
					<div className="project-sidebar__settings-wrap" ref={settingsRef}>
						<button
							...
							className={cn("project-sidebar__footer-btn", settingsOpen && "project-sidebar__footer-btn--active")}
						...
						{settingsOpen ? (
							...
							>
								<label className="project-sidebar__settings-row">
									<input type="checkbox" checked={showSessionId} onChange={(e) => setShowSessionId(e.target.checked)} />

...
export async function GET(request: NextRequest) {
	...
	try {
		...
		for (const [projectId, project] of Object.entries(config.projects)) {
			...
			try {
				const issues = await tracker.listIssues({ state, labels: label ? [label] : undefined, limit: 50 }, project);
		...
		return NextResponse.json({ error: err instanceof Error ? err.message : "Failed to fetch issues" }, { status: 500 });
	}
}

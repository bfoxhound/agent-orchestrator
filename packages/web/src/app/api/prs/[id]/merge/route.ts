...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	...
	try {
		...
		if (state !== "open") {
			...
			return jsonWithCorrelation({ error: `PR is ${state}, not open` }, { status: 409 }, correlationId);
		}
		...
		return jsonWithCorrelation({ ok: true, prNumber, method: "squash" }, { status: 200 }, correlationId);
	} catch (err) {
		const config = configForObservation ?? (await getServices().catch(() => ({ config: undefined }))).config;
		if (config) {
			recordApiObservation({
				config,
				method: "POST",

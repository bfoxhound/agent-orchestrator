...
import { sessionToDashboard, resolveProject, enrichSessionPR, enrichSessionsMetadata } from "@/lib/serialize";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	...
	if (!Object.prototype.hasOwnProperty.call(body, "displayName")) {
		return jsonWithCorrelation({ error: "displayName is required" }, { status: 400 }, correlationId);
	}
	const raw = body["displayName"];
	if (raw !== null && typeof raw !== "string") {
		return jsonWithCorrelation({ error: "displayName must be a string or null" }, { status: 400 }, correlationId);
	}
	...
	const cleaned =
		raw === null ? "" : stripControlChars(raw).replace(/\s+/g, " ").trim().slice(0, DISPLAY_NAME_MAX_LENGTH);
	...
	try {
		...
		const dashboardSession = updated ? sessionToDashboard(updated) : sessionToDashboard(coreSession);

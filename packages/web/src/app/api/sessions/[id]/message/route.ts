...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	...
	try {
		...
		if (typeof rawMessage !== "string") {
			return jsonWithCorrelation({ error: "message must be a string" }, { status: 400 }, correlationId);
		}
		...
		try {
			...
			return jsonWithCorrelation({ error: `Failed to send message: ${errorMsg}` }, { status: 500 }, correlationId);
		}
	} catch (error) {

...
function deriveSlug(pathname: string): string {
	return pathname.startsWith(WEBHOOK_PATH_PREFIX) ? pathname.slice(WEBHOOK_PATH_PREFIX.length) : pathname;
}
...
export async function POST(request: Request): Promise<Response> {
	...
	try {
		...
		if (candidates.length === 0) {
			return NextResponse.json({ error: "No SCM webhook configured for this path" }, { status: 404 });
		}

		const rawContentLength = request.headers.get("content-length");
		const contentLength = rawContentLength ? Number(rawContentLength) : NaN;
		const candidateMaxBodyBytes = candidates.map((candidate) => candidate.project.scm?.webhook?.maxBodyBytes);
		const allCandidatesBounded = candidateMaxBodyBytes.every((value) => typeof value === "number");
		const maxBodyBytes = allCandidatesBounded ? Math.max(...(candidateMaxBodyBytes as number[])) : undefined;
		if (maxBodyBytes !== undefined && Number.isFinite(contentLength) && contentLength > maxBodyBytes) {
			recordActivityEvent({
				source: "api",
			...
			return NextResponse.json({ error: "Webhook payload exceeds configured maxBodyBytes" }, { status: 413 });
		}
		...
		if (!verified) {
			...
			recordActivityEvent({
				...
				data: {
					...
					reason: unsupportedOnly ? "verification_unsupported" : (errors[0] ?? "verification_failed"),
				},
			});

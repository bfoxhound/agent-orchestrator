...
describe("SessionPage project polling", () => {
	...
	it("resolves orchestrator nav once for non-orchestrator pages and skips repeated project polling", async () => {
		...
		expect(
			vi
				.mocked(fetch)
				.mock.calls.filter(([url]) => url === "/api/sessions?project=my-app&orchestratorOnly=true&fresh=true"),
		).toHaveLength(1);
		...
		expect(vi.mocked(fetch).mock.calls.filter(([url]) => url === "/api/sessions?fresh=true")).toHaveLength(3);
	});
		...
		global.fetch = vi.fn(async (input: RequestInfo | URL) => {
			...
			if (url === "/api/sessions/worker-1") {
				...
				return {
					...
					status: 200,
					json: async () =>
						sessionFetchCount >= 3 ? { ...workerSession, metadata: { role: "orchestrator" } } : workerSession,
				} as Response;
			}
	...
	it("revalidates projects and sidebar sessions on remount even when cache exists", async () => {
		...
		expect(fetchMock.mock.calls.filter(([url]) => url === "/api/sessions?fresh=true")).toHaveLength(2);
	});
	...
	it("silences aborted sidebar refreshes during unmount", async () => {
		...
		expect(consoleErrorSpy).not.toHaveBeenCalledWith("Failed to fetch sidebar sessions:", expect.anything());
	});

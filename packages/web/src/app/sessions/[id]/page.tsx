...
function areSidebarSessionsEqual(previous: DashboardSession[] | null, next: DashboardSession[]): boolean {
	if (!previous || previous.length !== next.length) {
		return false;
	}
...
export default function SessionPage() {
	...
	const [sidebarOrchestrators, setSidebarOrchestrators] = useState<ProjectSidebarOrchestrator[] | undefined>(undefined);
	...
	useEffect(() => {
		...
		if (pathname?.startsWith("/projects/") && expectedProjectId && session.projectId !== expectedProjectId) {
			router.replace(projectSessionPath(session.projectId, session.id));
		}
		...
		try {
			...
			if (!hasLoadedSessionRef.current && isTransientSessionLoadError(err)) {
				...
				const shouldKeepRetrying =
					failureCount < SESSION_LOAD_MAX_CONSECUTIVE_FAILURES && elapsedMs < SESSION_LOAD_MAX_RETRY_ELAPSED_MS;

				if (shouldKeepRetrying) {
					const delay =
						SESSION_LOAD_RETRY_BACKOFF_MS[Math.min(failureCount - 1, SESSION_LOAD_RETRY_BACKOFF_MS.length - 1)];
	...
	const fetchSidebarSessions = useCallback(async () => {
		...
		try {
			...
			const nextSessions = applyMuxSessionPatches(restSessions, pendingMuxSessionsRef.current ?? []) ?? restSessions;
			cachedSidebarSessions = nextSessions;
			setSidebarOrchestrators(body?.orchestrators);
			setSidebarError(false);
			setSidebarSessions((current) => (areSidebarSessionsEqual(current, nextSessions) ? current : nextSessions));
		} catch (err) {
			if (pageUnloadingRef.current || controller.signal.aborted || isAbortLikeError(err)) {
				return;
			}

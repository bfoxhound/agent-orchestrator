...
const Terminal = nextDynamic(() => import("@/components/Terminal").then((m) => ({ default: m.Terminal })), {
	ssr: false,
});
...
function TerminalTestPageContent() {
			...
			<div className="mx-auto max-w-7xl">
				...
				<section className="mb-8 rounded-[2px] border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] p-6">
					...
					<div className="space-y-3 text-sm text-[var(--color-text-secondary)]">
						<p>
							<strong className="text-[var(--color-text-primary)]">Issue:</strong> Browser clipboard (Cmd+C/Ctrl+C) only
							worked when an iTerm2 client was attached to the tmux session.
						</p>
						<p>
					...
					<div className="space-y-4 text-sm">
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									Format: <code className="rounded-[2px] bg-black px-1 py-0.5">\x1b]52;c;&lt;base64&gt;\x07</code>
								</li>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									XDA query: <code className="rounded-[2px] bg-black px-1 py-0.5">CSI &gt; q</code> (also called
									XTVERSION)
								</li>
								<li>
						...
						<div>
							<h3 className="mb-2 font-semibold text-[var(--color-text-primary)]">4. Why iTerm2 "Fixed" It</h3>
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								<li>
					...
					<div className="space-y-3 text-sm">
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									Error on Node 25.6.1: <code className="rounded-[2px] bg-black px-1 py-0.5">posix_spawnp failed</code>
								</li>
								<li>
				...
				<div className="mb-4">
					...
					{oldSessionId === newSessionId && (
						<div className="mt-2 rounded-[2px] border border-[var(--color-accent-orange)] bg-[var(--color-bg-tertiary)] p-2 text-xs text-[var(--color-text-secondary)]">
							...
							<code className="ml-1 rounded-[2px] bg-black px-1">?old_session=ao-orchestrator&new_session=ao-20</code>
						</div>
					)}
				</div>
				...
				{showComparison && (
					<section className="mb-8">
						<div className="grid gap-6 lg:grid-cols-2">
							...
							<div className="rounded-[2px] border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] p-4">
								<div className="mb-3 flex items-center gap-2">
									...
									<h3 className="text-sm font-semibold text-[var(--color-text-primary)]">ttyd iframe (no XDA)</h3>
								</div>
							...
							<div className="rounded-[2px] border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] p-4">
								<div className="mb-3 flex items-center gap-2">
									...
									<h3 className="text-sm font-semibold text-[var(--color-text-primary)]">DirectTerminal (with XDA)</h3>
								</div>
								<div className="mb-2 rounded bg-[var(--color-bg-tertiary)] p-2 text-xs text-[var(--color-text-secondary)]">
						...
						<div className="mt-4 rounded-[2px] border border-[var(--color-accent-blue)] bg-[var(--color-bg-secondary)] p-4">
							<h3 className="mb-2 text-sm font-semibold text-[var(--color-text-primary)]">🧪 How to Test Clipboard</h3>
					...
					<div className="space-y-4 text-sm text-[var(--color-text-secondary)]">
						...
						<div>
							...
							<ol className="ml-6 list-decimal space-y-2">
								<li>
									<strong>Suspected ttyd clipboard handling</strong> - Spent hours analyzing ttyd source code, checking
									OSC 52 passthrough. ttyd was innocent - it passes escape sequences correctly.
								</li>
								<li>
								...
								<li>
									<strong>Investigated macOS clipboard permissions</strong> - Checked browser permissions, sandbox
									attributes, navigator.clipboard API. All were correct.
								</li>
								<li>
								...
								<li>
									...
									<code className="rounded-[2px] bg-black px-1 py-0.5">set-option -s set-clipboard on</code> in
									tmux.conf. Didn't help - tmux needs the terminal to declare support.
								</li>
							</ol>
						</div>
						...
						<div>
							...
							<div className="space-y-2">
								<p>
									<strong>What finally worked:</strong> Registering an XDA (Extended Device Attributes) handler in
									xterm.js using{" "}
									<code className="rounded-[2px] bg-black px-1 py-0.5">terminal.parser.registerCsiHandler()</code>
								</p>
  ...
  () => {
    ...
    return true;
								...
								<p className="mt-2">
									This single handler made tmux recognize our terminal as clipboard-capable. Clipboard immediately
									started working.
								</p>
							</div>
						</div>
						...
						<div>
							...
							<ol className="ml-6 list-decimal space-y-2">
								<li>
									<strong>Deep-dive into tmux source code</strong> - Used DeepWiki.com to analyze tmux's terminal
									capability detection logic in <code className="rounded-[2px] bg-black px-1 py-0.5">tty-keys.c</code>{" "}
									and <code className="rounded-[2px] bg-black px-1 py-0.5">tty.c</code>
								</li>
								<li>
									<strong>Discovered XDA queries</strong> - Found that tmux sends{" "}
									<code className="rounded-[2px] bg-black px-1 py-0.5">CSI &gt; q</code> (XTVERSION) to detect terminal
									type
								</li>
								<li>
						...
						<div className="rounded-[2px] border border-[var(--color-accent-orange)] bg-[var(--color-bg-tertiary)] p-4">
							...
							<ol className="ml-6 list-decimal space-y-2 text-[var(--color-text-secondary)]">
								<li>
									<strong>Start with tmux source code first</strong> - Instead of debugging xterm.js and ttyd, we should
									have immediately checked how tmux detects terminal capabilities
								</li>
								<li>
									<strong>Monitor escape sequences</strong> - Running{" "}
									<code className="rounded-[2px] bg-black px-1 py-0.5">tmux -vvv</code> or using a terminal protocol
									analyzer would have revealed the XDA queries being sent
								</li>
								<li>
									<strong>Compare working vs broken scenarios</strong> - Should have captured and diffed the escape
									sequences when iTerm2 was attached vs not attached earlier
								</li>
								<li>
								...
								<li>
									<strong>Read tmux documentation on terminal types</strong> - tmux man page mentions terminal
									capability detection, but we focused on clipboard settings instead
								</li>
							</ol>
						</div>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1">
								...
								<li>
									tmux <code className="rounded-[2px] bg-black px-1 py-0.5">tty-keys.c</code>: Terminal type detection
									logic
								</li>
							</ul>
						</div>
				...
				<section className="mt-6 rounded-[2px] border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] p-6">
					...
					<ul className="ml-6 list-disc space-y-1 text-sm text-[var(--color-text-secondary)]">
						<li>
							<code className="rounded-[2px] bg-black px-1 py-0.5">packages/web/src/components/DirectTerminal.tsx</code>{" "}
							- Main component with XDA handler
						</li>
						<li>
							<code className="rounded-[2px] bg-black px-1 py-0.5">packages/web/server/direct-terminal-ws.ts</code> -

...
async function labelIssuesForVerification(
	...
	const mergedSessions = sessions.filter(
		(s) => s.lifecycle.pr.state === "merged" && s.issueId && !processedIssues.has(`${s.projectId}:${s.issueId}`),
	);
 ...
 */
async function relabelReopenedIssues(config: LoadedConfig, registry: PluginRegistry): Promise<void> {
	for (const [, project] of Object.entries(config.projects)) {
		...
		let reopened: Issue[];
		try {
			reopened = await tracker.listIssues({ state: "open", labels: ["agent:done"], limit: 20 }, project);
		} catch {
			continue;
		}
...
export async function pollBacklog(): Promise<void> {
	try {
		...
		const allSessionPrefixes = Object.entries(config.projects).map(([id, p]) => p.sessionPrefix ?? id);
		...
		for (const [projectId, project] of Object.entries(config.projects)) {
			...
			try {
				backlogIssues = await tracker.listIssues({ state: "open", labels: [BACKLOG_LABEL], limit: 10 }, project);
			} catch {
...
export async function getBacklogIssues(): Promise<Array<Issue & { projectId: string }>> {
	...
	try {
		...
		for (const [projectId, project] of Object.entries(config.projects)) {
			...
			try {
				const issues = await tracker.listIssues({ state: "open", labels: [BACKLOG_LABEL], limit: 20 }, project);
...
export async function getVerifyIssues(): Promise<Array<Issue & { projectId: string }>> {
	...
	try {
		...
		for (const [projectId, project] of Object.entries(config.projects)) {
			...
			try {
				const issues = await tracker.listIssues({ state: "open", labels: ["merged-unverified"], limit: 20 }, project);
				for (const issue of issues) {
					results.push({ ...issue, projectId });
				}
			} catch {

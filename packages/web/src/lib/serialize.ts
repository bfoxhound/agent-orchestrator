...
export function resolveProject(core: Session, projects: Record<string, ProjectConfig>): ProjectConfig | undefined {
	// Try explicit projectId first
	const direct = projects[core.projectId];
	if (direct) return direct;

	// Match by session prefix
	const entry = Object.entries(projects).find(([, p]) => matchesSessionPrefix(core.id, p.sessionPrefix));
 ...
 */
export function readPREnrichmentFromMetadata(metadata: Record<string, string>): PREnrichmentData | null {
...
export function enrichSessionIssue(dashboard: DashboardSession, tracker: Tracker, project: ProjectConfig): void {
...
export async function enrichSessionsMetadataFast(
	...
	const { summaryPromises } = prepareSessionMetadataEnrichment(coreSessions, dashboardSessions, config, registry);

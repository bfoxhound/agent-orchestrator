...
async function readResponseJson<T>(response: Response, signal: AbortSignal | undefined): Promise<T> {
	throwIfAborted(signal);
...
async function readErrorMessage(response: Response, signal: AbortSignal | undefined): Promise<string> {
	try {
		const payload = await readResponseJson<{ error?: string; message?: string } | null>(response, signal);

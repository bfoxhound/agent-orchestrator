...
export interface DashboardLifecycleFacet<TState extends string = string, TReason extends string = string> {
	state: TState;
	reason: TReason;
	label: string;
	reasonLabel: string;
...
export function isDashboardRuntimeEnded(session: DashboardSession): boolean {
	if (session.lifecycle) {
		return session.lifecycle.runtimeState === "missing" || session.lifecycle.runtimeState === "exited";
	}
	return (
		TERMINAL_STATUSES.has(session.status) || (session.activity !== null && TERMINAL_ACTIVITIES.has(session.activity))
	);
...
export function isDashboardSessionRestorable(session: DashboardSession): boolean {
	if (session.lifecycle) {
		const terminalByCoreTruth =
			...
			session.lifecycle.runtimeState === "missing" ||
			session.lifecycle.runtimeState === "exited";
		return terminalByCoreTruth && session.lifecycle.prState !== "merged" && session.status !== "merged";
	}

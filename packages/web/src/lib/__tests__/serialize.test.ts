...
describe("readPREnrichmentFromMetadata", () => {
	...
	it("should include review comments when present", () => {
		const data = readPREnrichmentFromMetadata({
			...
			prReviewComments: createReviewCommentsMetadata({
				unresolvedThreads: 3,
				unresolvedComments: [{ url: "https://example.com", path: "src/app.ts", author: "alice", body: "Please fix" }],
			}),
		});
...
describe("enrichSessionAgentSummary", () => {
	function createMockAgent(info: Partial<Awaited<ReturnType<Agent["getSessionInfo"]>>> | null = null): Agent {
		return {
			name: "mock",
			processName: "mock",
...
describe("enrichSessionsMetadata", () => {
	...
	it("starts issue-title fetches before agent summaries finish", async () => {
		let resolveSummary:
			((value: { summary: string; summaryIsFallback: false; agentSessionId: string }) => void) | null = null;

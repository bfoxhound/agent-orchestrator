...
export function eventMatchesProject(event: SCMWebhookEvent, project: ProjectConfig): boolean {
	...
	return `${event.repository.owner}/${event.repository.name}`.toLowerCase() === project.repo.toLowerCase();
}

export function findAffectedSessions(sessions: Session[], projectId: string, event: SCMWebhookEvent): Session[] {
...
export function buildWebhookRequest(request: Request, body: string, rawBody: Uint8Array): SCMWebhookRequest {

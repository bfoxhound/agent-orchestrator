...
describe("handleWindowsPipeMessage", () => {
	...
	it("rejects malformed session ids before touching the pipe path", () => {
		...
		handleWindowsPipeMessage({ id: "../../../etc/passwd", type: "open" }, ws, new Map(), new Map(), deps);
	...
	it("throws when pipe path cannot be resolved", () => {
		...
		expect(() => handleWindowsPipeMessage({ id: "bad", type: "open" }, ws, new Map(), new Map(), deps)).toThrow(
			"No PTY host pipe found",
		);
	});
	...
	it("relays terminal data (0x01) from pipe to WebSocket", () => {
		...
		expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ ch: "terminal", id: "s1", type: "data", data: "hello" }));
	});
	...
	it("sends exited on PTY status (0x07) alive=false", () => {
		...
		expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ ch: "terminal", id: "s1", type: "exited", code: 0 }));
	});
	...
	it("scopes pipe maps and resolution by projectId so two projects can share a sessionId", () => {
		...
		const resolvePipePath = vi.fn((id: string, projectId?: string) => `\\\\.\\pipe\\ao-pty-${projectId}-${id}`);
	...
	it("handles partial frames across multiple data chunks", () => {
		...
		expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ ch: "terminal", id: "s1", type: "data", data: "split" }));
	});
});

...
describe("resolveTmuxSession", () => {
	...
	describe("exact match", () => {
		...
		it("passes correct tmux path and timeout to has-session", () => {
			...
			expect(mockExec).toHaveBeenCalledWith(customTmux, ["has-session", "-t", "=my-session"], { timeout: 5000 });
		});
		...
		it("does not call list-sessions when exact match succeeds", () => {
			...
			expect(mockExec).toHaveBeenCalledTimes(1);
			expect(mockExec).toHaveBeenCalledWith(TMUX, ["has-session", "-t", "=ao-15"], { timeout: 5000 });
		});
	});
	...
	describe("hash-prefix resolution", () => {
		...
		it("matches the correct session among many", () => {
			const mockExec = vi
				...
				.mockImplementationOnce(() => {
					return (
						["aabbccddeef0-ao-1", "112233445566-ao-15", "ffeeddccbbaa-ao-2", "a0b1c2d3e4f5-ao-orchestrator"].join(
							"\n",
						) + "\n"
					);
				});
		...
		it("passes list-sessions format flag correctly", () => {
			...
			expect(mockExec).toHaveBeenNthCalledWith(2, TMUX, ["list-sessions", "-F", "#{session_name}"], {
				timeout: 5000,
				encoding: "utf8",
			});
		});
		...
		it("does NOT false-match when wrapped session belongs to a different project", () => {
			...
			const fs = {
				...
				exists: (p: string) => p.replace(/\\/g, "/").endsWith("/112233445566-my-project/sessions/app-1"),
				homedir: () => "/home/user",
			};
			const mockExec = vi
				.fn()
		...
		it("probes later candidates when earlier storageKey has no live tmux session", () => {
			...
			expect(mockExec).toHaveBeenNthCalledWith(2, TMUX, ["has-session", "-t", "=aaaaaaaaaaaa-stale-project-app-1"], {
				timeout: 5000,
			});
			expect(mockExec).toHaveBeenNthCalledWith(3, TMUX, ["has-session", "-t", "=bbbbbbbbbbbb-live-project-app-1"], {
				timeout: 5000,
			});
		});
		...
		it("prefers the project-scoped storageKey when session IDs collide", () => {
			const fs = {
				...
				exists: (p: string) => {
					const u = p.replace(/\\/g, "/");
					return u.endsWith("/aaaaaaaaaaaa-alpha/sessions/app-1") || u.endsWith("/bbbbbbbbbbbb-beta/sessions/app-1");
				},
				homedir: () => "/home/user",
			};
			const mockExec = vi
			...
			expect(mockExec).toHaveBeenNthCalledWith(2, TMUX, ["has-session", "-t", "=bbbbbbbbbbbb-beta-app-1"], {
				timeout: 5000,
			});
		});
		...
		it("does not treat suffix project names as exact project matches", () => {
			const fs = {
				...
				exists: (p: string) => {
					const u = p.replace(/\\/g, "/");
					return u.endsWith("/aaaaaaaaaaaa-my-app/sessions/app-1") || u.endsWith("/bbbbbbbbbbbb-app/sessions/app-1");
				},
				homedir: () => "/home/user",
			};
			const mockExec = vi
			...
			expect(mockExec).toHaveBeenNthCalledWith(2, TMUX, ["has-session", "-t", "=bbbbbbbbbbbb-app-app-1"], {
				timeout: 5000,
			});
		});
		...
		it("accepts wrapped storageKeys with spaces/unicode in the project name", () => {
			...
			expect(resolveTmuxSession("ao-15", TMUX, mockExec, fs)).toBe("aabbccddeef0-My App (v2)-ao-15");
		});
...
describe("resolvePipePath", () => {
	...
	it("reads V2 JSON metadata under projects/{projectId}/sessions/{sessionId}.json", () => {
		...
		const fs = {
			readdir: (p: string) => (p.replace(/\\/g, "/").endsWith("/.agent-orchestrator/projects") ? ["my-app"] : []),
			exists: (p: string) => {
				const u = p.replace(/\\/g, "/");
	...
	it("uses projectId to disambiguate when two projects share a sessionId", () => {
		...
		const fs = {
			...
			homedir: () => "/home/u",
			readFile: (p: string) => (p.replace(/\\/g, "/").includes("/projects/projA/") ? metaFor(PIPE_A) : metaFor(PIPE_B)),
		};

...
function httpGet(path: string): Promise<{ status: number; body: string }> {
	return new Promise((resolve, reject) => {
		const req = request({ hostname: "localhost", port, path, method: "GET", timeout: 3000 }, (res: IncomingMessage) => {
			let body = "";
...
function waitForMessage(ws: WebSocket, predicate: (msg: MuxMessage) => boolean, timeoutMs = 3000): Promise<MuxMessage> {
	return new Promise((resolve, reject) => {
...
describeWithTmux("mux terminal open", () => {
	it("sends 'opened' response for a valid tmux session", async () => {
		...
		ws.send(JSON.stringify({ ch: "terminal", id: "test-session", tmuxName: TEST_SESSION, type: "open" }));
	...
	it("uses an explicit exact tmuxName for hash-prefixed sessions", async () => {
		...
		ws.send(JSON.stringify({ ch: "terminal", id: hashOnlyId, tmuxName: TEST_HASH_SESSION, type: "open" }));
	...
	it("retry budget recovers after PTY survives the grace period (issue #1639)", async () => {
		...
		try {
			...
			const exitedMsg = await waitForMessage(ws, (m) => m.ch === "terminal" && m.type === "exited", 2000);

...
describe("mux WebSocket connection events", () => {
	...
	function findEvent(kind: string): { data: Record<string, unknown> } | undefined {
		const found = recordActivityEvent.mock.calls.find(([e]) => (e as { kind: string }).kind === kind);
	...
	it("emits ui.terminal_heartbeat_lost once on 3 missed pongs and terminates", () => {
		...
		vi.advanceTimersByTime(15_000);
		expect(
			recordActivityEvent.mock.calls.filter(([e]) => (e as { kind: string }).kind === "ui.terminal_heartbeat_lost")
				.length,
		).toBe(1);
	});
...
describe("Windows pipe ui.terminal_pty_lost activity events", () => {
	...
	function openPipe() {
		...
		handleWindowsPipeMessage({ id: "app-1", type: "open", projectId: "proj-1" }, ws, winPipes, winPipeBuffers, deps);
		pipe.emit("connect");
		recordActivityEvent.mockClear();
		ws.send.mockClear();
	...
	it("does not emit ui.terminal_pty_lost for an intentional client close", () => {
		...
		handleWindowsPipeMessage({ id: "app-1", type: "close", projectId: "proj-1" }, ws, winPipes, winPipeBuffers, deps);
...
describe("TerminalManager.open — tmux target args (regression for #1714)", () => {
	...
	it("invokes set-option mouse on with the bare session id (no = prefix)", () => {
		...
		const mouseCall = mockSpawn.mock.calls.find((call) => Array.isArray(call[1]) && call[1].includes("mouse"));
	...
	it("invokes set-option status off with the bare session id (no = prefix)", () => {
		...
		const statusCall = mockSpawn.mock.calls.find((call) => Array.isArray(call[1]) && call[1].includes("status"));

...
import { banner, header, formatAge, activityIcon, ciStatusIcon, reviewDecisionIcon, padCol } from "../lib/format.js";
...
async function gatherSessionInfo(
	...
	const allSessionPrefixes = Object.entries(projectConfig.projects).map(([id, p]) => p.sessionPrefix ?? id);
...
function printOrchestratorRow(info: SessionInfo): void {
	const lastActivity = info.lastActivity === "-" ? chalk.dim("unknown") : chalk.dim(info.lastActivity);
	console.log(
	...
	program
		...
		.action(async (opts: StatusOptions) => {
			...
			const renderStatus = async (refreshing = false): Promise<void> => {
				...
				const hiddenTerminatedCount = opts.includeTerminated ? 0 : allSessions.filter(isTerminalSession).length;
				const sessions = opts.includeTerminated ? allSessions : allSessions.filter((s) => !isTerminalSession(s));

				if (!opts.json) {
					console.log(banner("AGENT ORCHESTRATOR STATUS"));
					if (opts.watch) {
						console.log(chalk.dim(`Refreshing every ${watchIntervalSeconds}s. Press Ctrl+C to exit.`));
						console.log();
					} else {
						console.log();
					}
			...
			const watchTimer = setInterval(() => {
				...
				void renderStatus(true)
					.catch((err) => {
						console.error(chalk.red(`Watch refresh failed: ${err instanceof Error ? err.message : String(err)}`));
					})
					.finally(() => {
						rendering = false;
					});
...
async function showFallbackStatus(): Promise<void> {
	...
	console.log(banner("AGENT ORCHESTRATOR STATUS"));
	console.log();
	console.log(chalk.dim(`  ${allTmux.length} tmux session${allTmux.length !== 1 ? "s" : ""} found\n`));
	...
	const details = await Promise.all(
		sortedSessions.map(async (session) => {
			...
			const [activityTs, introspection] = await Promise.all([activityTsPromise, introspectionPromise]);

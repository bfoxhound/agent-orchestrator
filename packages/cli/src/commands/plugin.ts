...
async function rollbackManagedPackageInstall(packageName: string, previousVersion: string | null): Promise<void> {
	if (previousVersion) {
...
async function installOrVerifyPlugin(
	...
	try {
		...
		return await verifyPluginDescriptor(config, { ...descriptor, version: installedVersion }, descriptor.package);
	} catch (err) {
		if (installCompleted) {
			try {
...
async function resolveTargetVersion(plugin: InstalledPluginConfig): Promise<string> {
	...
	const marketplacePlugin = findMarketplacePlugin(plugin.package) ?? findMarketplacePlugin(plugin.name);
...
async function verifyPluginDescriptor(
	...
	if (!registered) {
		throw new Error(`Plugin ${imported.manifest.name} imported successfully but did not load through the registry`);
	}
...
function printPluginListFromCatalog(
	...
	if (installedOnly) {
		...
		if (filtered.length === 0) {
			console.log(
				chalk.dim(
					slotFilter ? `No installed plugins with type "${slotFilter}".` : "No plugins are installed in this config.",
				),
			);
			return;
		}
	...
	if (filteredCatalog.length === 0) {
		console.log(
			chalk.dim(slotFilter ? `No marketplace plugins with type "${slotFilter}".` : "No marketplace plugins found."),
		);
		return;
	}

	for (const plugin of filteredCatalog) {
		const isInstalled = installed.some((entry) => entry.package === plugin.package || entry.name === plugin.id);
	...
	plugin
		...
		.action(
			async (
				...
				const slotOptions: PluginSlot[] = ["runtime", "agent", "workspace", "tracker", "scm", "notifier", "terminal"];

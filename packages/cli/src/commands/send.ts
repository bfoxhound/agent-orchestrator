...
import { type Agent, type OpenCodeSessionManager, type Session, loadConfig } from "@aoagents/ao-core";
...
export function registerSend(program: Command): void {
	program
		...
		.action(
			async (session: string, messageParts: string[], opts: { file?: string; wait?: boolean; timeout?: string }) => {
				...
				if (!canUseTmux && !delegatesToSessionManager) {
					console.error(
						chalk.red(`Session '${session}' is not tmux-backed and cannot be sent without lifecycle routing`),
					);
					process.exit(1);
				}

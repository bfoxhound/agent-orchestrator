...
async function checkPluginResolution(config: OrchestratorConfig, fail: (msg: string) => void): Promise<PluginRegistry> {
	console.log("");
	...
	const slots: CheckedPluginSlot[] = ["runtime", "agent", "workspace", "tracker", "scm", "notifier"];
...
async function checkOpenClawNotifier(config: OrchestratorConfig, fail: (msg: string) => void): Promise<void> {
	...
	const url =
		(typeof openclawConfig["url"] === "string" ? openclawConfig["url"] : undefined) ?? "http://127.0.0.1:18789";
...
async function checkNotifierConnectivity(config: OrchestratorConfig, fail: (msg: string) => void): Promise<void> {
...
function checkVersionFreshness(): void {
	...
	const isOutdated =
		installMethod === "git" ? cached.isOutdated === true : isVersionOutdated(current, cached.latestVersion);

...
export function registerSession(program: Command): void {
	const session = program.command("session").description("Session management (ls, kill, cleanup, restore, claim-pr)");
	...
	session
		...
		.action(async (opts: { project?: string; all?: boolean; includeTerminated?: boolean; json?: boolean }) => {
			...
			const hiddenTerminatedCount = opts.includeTerminated ? 0 : withoutOrchestrators.filter(isTerminalSession).length;
				...
				const pipePath =
					typeof dataPipePath === "string" && dataPipePath
						...
						: `\\\\.\\pipe\\ao-pty-${
								...
								(config.configPath ? `${generateConfigHash(config.configPath)}-${sessionName}` : sessionName)
							}`;
	...
	session
		...
		.action(async (prRef: string, sessionName: string | undefined, opts: { assignOnGithub?: boolean }) => {
			const config = loadConfig();
			...
			try {
				...
				console.log(
					chalk.dim(`  Checkout: ${result.branchChanged ? "switched to PR branch" : "already on PR branch"}`),
				);

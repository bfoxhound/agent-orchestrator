	...
	program
		...
		.action(async (target: string | undefined, opts: { newWindow?: boolean; browser?: boolean }) => {
			...
			if (!running && !opts.browser) {
				console.log(chalk.dim("Note: AO daemon does not appear to be running — dashboard URL fallback may not load."));
			}

			console.log(chalk.bold(`Opening ${sessionsToOpen.length} session${sessionsToOpen.length > 1 ? "s" : ""}...\n`));
			...
			for (const session of sorted) {
				...
				if (opts.browser || dead) {
					...
					if (dead) {
						...
						console.log(`  ${chalk.yellow(session.id)} ${chalk.dim("(terminated)")} — opened ${chalk.dim(url)}`);

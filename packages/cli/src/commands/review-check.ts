			...
			for (const result of results) {
				...
				if (!opts.dryRun) {
					try {
						...
						const projectReaction = config.projects[result.projectId]?.reactions?.["changes-requested"];
						const globalReaction = config.reactions["changes-requested"];
						const reviewFixPrompt = projectReaction?.message ?? globalReaction?.message ?? DEFAULT_REVIEW_FIX_PROMPT;

...
async function runSpawnPreflight(
	...
	for (const plugin of candidates) {
		const preflight = (plugin as { preflight?: (ctx: PreflightContext) => Promise<void> } | null)?.preflight;
...
export function registerBatchSpawn(program: Command): void {
	program
		...
		.action(async (issues: string[], opts: { open?: boolean }) => {
			...
			const needsFallback = issues.some((issue) => resolveSpawnTarget(config.projects, issue) === null);
			if (needsFallback) {
				try {
			...
			for (const issue of issues) {
				...
				if (!config.projects[target.projectId]) {
					console.error(
						chalk.red(`Unknown project: ${target.projectId}\nAvailable: ${Object.keys(config.projects).join(", ")}`),
					);
					process.exit(1);
				}
			...
			for (const [groupProjectId, items] of groups) {
				...
				for (const { original, resolved } of items) {
					...
					if (existingSessionId) {
						console.log(chalk.yellow(`  Skip ${original} — already has session ${existingSessionId}`));
					...
					try {
						...
						console.log(chalk.red(`  Failed ${original} — ${err instanceof Error ? err.message : String(err)}`));
					}
				}
			}

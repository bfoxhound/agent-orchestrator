 ...
 */
const ACTIVE_SESSION_STATUSES = new Set<Session["status"]>(["working", "idle", "needs_input", "stuck"]);
...
async function handleNpmUpdate(method: InstallMethod): Promise<void> {
	...
	const isFirstChannelOptIn =
		!info.isOutdated && !isChannelSwitch && previousChannel === undefined && info.currentVersion !== info.latestVersion;

	const needsInstall = info.isOutdated || isChannelSwitch || isFirstChannelOptIn;

	if (!needsInstall) {
		console.log(
			chalk.green(`Already on latest ${channel === "nightly" ? "nightly" : "version"} (${info.currentVersion}).`),
		);
		return;
	}
	...
	if (isChannelSwitch) {
		...
		console.log(
			chalk.dim("  The version compare says you're current, but the install command picks a different dist-tag."),
		);

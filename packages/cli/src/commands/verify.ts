...
function resolveProject(
	...
	if (ids.length > 1) {
		console.error(chalk.red(`Multiple projects found. Specify one with --project: ${ids.join(", ")}`));
		process.exit(1);
	}
		...
		.action(
			async (
				...
				if (opts.list) {
					...
					const issues = await tracker.listIssues({ state: "open", labels: ["merged-unverified"] }, project);

...
async function cloneRepo(parsed: ParsedRepoUrl, targetDir: string, cwd: string): Promise<void> {
	...
	if (parsed.host === "github.com") {
		...
		if (ghAvailable) {
			try {
				await runInteractiveCommand("gh", ["repo", "clone", parsed.ownerRepo, targetDir, "--", "--depth", "1"], {
					cwd,
					action: "clone repository via gh",
				});
				return;
			} catch {
 ...
 */
async function addProjectToConfig(config: OrchestratorConfig, projectPath: string): Promise<string> {
...
async function runStartup(
	...
	if (opts?.dashboard !== false) {
		...
		if (opts?.rebuild) {
			await rebuildDashboardProductionArtifacts(webDir, [...new Set([requestedDashboardPort, port])]);
			...
			async (
				...
				try {
					...
					if (running) {
						...
						if (projectArgIsUrlOrPath && !resolvedProject.justCreated && running.projects.includes(projectId)) {
							console.log(chalk.cyan(`\nℹ AO is already running.`));
							console.log(`  Dashboard: ${chalk.cyan(`http://localhost:${running.port}`)}`);
		...
		.action(async (projectArg?: string, opts: { purgeSession?: boolean; all?: boolean } = {}) => {
			...
			try {
				...
				try {
					...
					const allSessions = stopAll ? rawSessions : rawSessions.filter((s) => s.projectId === _projectId);
					...
					if (activeSessions.length > 0) {
						...
						const killedTarget = targetActive.filter((s) => killedSessionIds.includes(s.id)).map((s) => s.id);
					...
					if (killedSessionIds.length > 0) {
						...
						try {
							...
							console.log(
								chalk.yellow(`  Could not write last-stop state: ${err instanceof Error ? err.message : String(err)}`),
							);
						}
					}
				} catch (err) {

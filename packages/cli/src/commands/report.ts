...
async function writeReport(
	...
	try {
		...
		console.log(`${chalk.green("✓")} ${chalk.bold(sessionName)} reported ${chalk.cyan(state)} ${label}`);
		if (prUrl || prNumber !== undefined) {
			const details = [prNumber !== undefined ? `#${prNumber}` : null, prUrl ?? null].filter((value): value is string =>
				Boolean(value),
			);
...
export function registerAcknowledge(program: Command): void {
	program
		.command("acknowledge")
		.description("Acknowledge session pickup — agents run this once after reading the initial prompt (Stage 3).")
...
export function registerReport(program: Command): void {
	...
	program
		.command("report")
		.description(`Declare a workflow transition (Stage 3). Allowed states: ${allowed} (hyphenated aliases accepted).`)
		.argument(
			"<state>",
		...
		.option("--pr-url <url>", "Attach a PR URL to pr-created / draft-pr-created / ready-for-review reports")
		.option("--pr-number <number>", "Attach a PR number to pr-created / draft-pr-created / ready-for-review reports")
		.action(async (state: string, opts: { session?: string; note?: string; prUrl?: string; prNumber?: string }) => {
			...
			const prWorkflowState =
				canonical === "pr_created" || canonical === "draft_pr_created" || canonical === "ready_for_review";
			if (!prWorkflowState && (opts.prUrl || opts.prNumber)) {
				console.error(
					chalk.red("PR metadata flags are only valid with pr-created, draft-pr-created, or ready-for-review."),
				);
				process.exit(1);
			}
			const prNumber = opts.prNumber !== undefined ? Number.parseInt(opts.prNumber, 10) : undefined;
			if (opts.prNumber !== undefined && (!Number.isInteger(prNumber) || prNumber === undefined || prNumber <= 0)) {

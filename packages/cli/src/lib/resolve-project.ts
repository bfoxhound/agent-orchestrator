...
async function fromUrlIntoGlobal(arg: string, deps: ResolveDeps): Promise<Resolved> {
	...
	if (isRepoAlreadyCloned(targetDir, parsed.cloneUrl)) {
		...
		try {
			...
			throw new Error(`Failed to clone ${parsed.ownerRepo}: ${err instanceof Error ? err.message : String(err)}`, {
				cause: err,
			});
		}
	}
...
async function fromCwdOrId(arg: string | undefined, deps: ResolveDeps, opts: ResolveOptions): Promise<Resolved> {

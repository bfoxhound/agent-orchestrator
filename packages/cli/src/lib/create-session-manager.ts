 ...
 */
export async function getSessionManager(config: OrchestratorConfig): Promise<OpenCodeSessionManager> {
 ...
 */
export async function getLifecycleManager(config: OrchestratorConfig, projectId?: string): Promise<LifecycleManager> {
	const registry = await getPluginRegistry(config);

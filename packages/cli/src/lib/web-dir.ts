...
export async function buildDashboardEnv(
	...
	const explicitDirect =
		directTerminalPort ?? (env["DIRECT_TERMINAL_PORT"] ? parseInt(env["DIRECT_TERMINAL_PORT"], 10) : undefined);
...
export function findWebDir(): string {
	...
	try {
		...
		const candidates = [resolve(__dirname, "../../../web"), resolve(__dirname, "../../../../packages/web")];

...
import { ensureLifecycleWorker, listLifecycleWorkers, stopLifecycleWorker } from "./lifecycle-service.js";
...
async function projectHasNonTerminalSession(config: OrchestratorConfig, projectId: string): Promise<boolean> {
	const sm = await getSessionManager(config);
	const sessions = await sm.list(projectId);
	return sessions.some((session) => !isTerminalSession(session));
}

export async function reconcileProjectSupervisor(options: ReconcileProjectSupervisorOptions = {}): Promise<void> {
	...
	for (const projectId of configuredProjectIds) {
		try {
			...
			reportProjectSupervisorError(observer, projectId, "Failed to reconcile lifecycle worker for project", error);
			// Best-effort per project: a broken project must not block others from reconciling.
		}
	}
}

export async function startProjectSupervisor(options: StartProjectSupervisorOptions = {}): Promise<SupervisorHandle> {

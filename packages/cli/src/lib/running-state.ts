import { readFileSync, writeFileSync, mkdirSync, unlinkSync, openSync, closeSync, constants, statSync } from "node:fs";
 ...
 */
async function acquireLock(lockFile: string, timeoutMs = 5000, resourceName = "lock"): Promise<() => void> {
	ensureDir();
...
export async function readLastStop(): Promise<LastStopState | null> {
	...
	try {
		...
		if (!state || typeof state.stoppedAt !== "string" || !state.projectId || !Array.isArray(state.sessionIds))
			return null;
		return state;
	} catch {
		return null;

...
export function resolveDefaultRepoRootFromPath(modulePath: string): string {
	...
	return layout === "package-install" ? resolve(moduleDir, "../..") : resolve(moduleDir, "../../../../");
}
...
function isValidRepoRootForLayout(root: string, layout: ScriptLayout): boolean {
	return layout === "source-checkout" ? isAgentOrchestratorRepoRoot(root) : isAoCliPackageRoot(root);
}
...
export function resolveScriptLayout(): ScriptLayout {
	...
	if (process.env["AO_DEV"] === "1" && (override === "package-install" || override === "source-checkout")) {
		return override;
	}

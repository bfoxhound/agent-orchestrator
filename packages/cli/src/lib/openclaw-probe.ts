...
export async function detectOpenClawInstallation(
	...
	return {
		state: probe.reachable ? "running" : binaryPath || hasConfig ? "installed-but-stopped" : "missing",
		gatewayUrl,
		binaryPath,
...
export async function validateToken(
	...
	try {
		...
		if (response.status === 401 || response.status === 403) {
			return {
				valid: false,
				error: "Token rejected by OpenClaw. Check that hooks.token in your OpenClaw config matches this token.",
			};
		}

...
export function detectProjectType(dir: string): ProjectType {
	...
	if (hasFile("package.json")) {
		...
		if (hasFile("tsconfig.json") || hasFile("tsconfig.base.json") || pkg?.devDependencies?.typescript) {
...
function generateTestCommands(projectType: ProjectType): string {
	...
	if (projectType.languages.includes("typescript") || projectType.languages.includes("javascript")) {

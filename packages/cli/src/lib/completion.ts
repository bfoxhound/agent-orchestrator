...
function getArgumentAction(node: CompletionCommandNode, argumentIndex: number): string | undefined {
	...
	const argumentName = argumentIndex >= 0 ? (node.arguments[argumentIndex]?.name()?.toLowerCase() ?? "") : "";
	if (argumentName.includes("path") || argumentName.includes("file") || argumentName.includes("directory")) {
		return "_files";
	}
...
function getOptionAction(node: CompletionCommandNode, option: Option): string | undefined {
	...
	if (key === "session cleanup" && (optionMatches(option, "-p") || optionMatches(option, "--project"))) {
		return "_ao_complete_projects";
	}
	...
	if (attributeName.includes("file") || attributeName.includes("path") || attributeName.includes("directory")) {
		return "_files";
	}
...
function renderSubcommandCase(node: CompletionCommandNode): string[] {
	...
	const lines = ["  case $state in", "    subcommand)", "      local -a subcommands", "      subcommands=("];

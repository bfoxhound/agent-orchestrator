...
vi.mock("../../src/lib/dashboard-rebuild.js", () => ({
	isInstalledUnderNodeModules: (path: string) => path.includes("/node_modules/") || path.includes("\\node_modules\\"),
}));
...
describe("preflight.checkBuilt", () => {
	...
	it("throws npm hint when ao-core not found in global install", async () => {
		...
		await expect(preflight.checkBuilt("/usr/local/lib/node_modules/@aoagents/ao-web")).rejects.toThrow(
	...
	it("throws pnpm hint when ao-core not found in monorepo", async () => {
		mockExistsSync.mockReturnValue(false);
		await expect(preflight.checkBuilt("/home/user/agent-orchestrator/packages/web")).rejects.toThrow(

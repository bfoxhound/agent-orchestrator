...
describe("clearStaleCacheIfNeeded", () => {
	...
	it("clears cache and updates stamp when version differs", async () => {
		...
		await clearStaleCacheIfNeeded("/web");
		expect(mockRmSync).toHaveBeenCalledWith(expect.stringContaining("cache"), { recursive: true, force: true });
		expect(mockWriteFileSync).toHaveBeenCalledWith(expect.stringContaining("AO_VERSION"), "0.3.0", "utf8");
	});

...
const { mockExistsSync, mockReadFileSync, mockWriteFileSync, mockUnlinkSync, mockMkdirSync } = vi.hoisted(() => ({
...
describe("update-check", () => {
	...
	describe("classifyInstallPath", () => {
		it("returns 'npm-global' for /usr/local/lib/node_modules path", () => {
			expect(classifyInstallPath("/usr/local/lib/node_modules/@aoagents/ao-cli/dist/lib/update-check.js")).toBe(
				"npm-global",
			);
		});
		...
		it("returns 'unknown' for local project node_modules (npx)", () => {
			mockExistsSync.mockReturnValue(false);
			expect(classifyInstallPath("/home/user/my-project/node_modules/@aoagents/ao-cli/dist/lib/update-check.js")).toBe(
				"unknown",
			);
		});
		...
		it("returns 'git' when repo root has .git", () => {
			...
			expect(classifyInstallPath("/home/user/agent-orchestrator/packages/cli/src/lib/update-check.ts")).toBe("git");
		});
		...
		it("returns 'unknown' for a non-AO repo even when .git exists four levels up", () => {
			...
			expect(classifyInstallPath("/home/user/other-monorepo/packages/cli/src/lib/update-check.ts")).toBe("unknown");
		});
	...
	describe("classifyInstallPath — bun + homebrew (Section F)", () => {
		it("returns 'bun-global' for ~/.bun/install/global/ paths", () => {
			expect(
				classifyInstallPath("/home/user/.bun/install/global/node_modules/@aoagents/ao-cli/dist/lib/update-check.js"),
			).toBe("bun-global");
		});

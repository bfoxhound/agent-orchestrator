...
const { mockUnregister, mockWaitForExit, mockKillProcessTree, mockSweepDaemonChildren } = vi.hoisted(() => ({
...
describe("killExistingDaemon", () => {
	...
	it("throws when SIGKILL also fails to exit, and does not unregister", async () => {
		mockWaitForExit.mockResolvedValueOnce(false);
		mockWaitForExit.mockResolvedValueOnce(false);
		await expect(killExistingDaemon(fakeRunning)).rejects.toThrow(/Failed to stop AO process \(PID 12345\)/);

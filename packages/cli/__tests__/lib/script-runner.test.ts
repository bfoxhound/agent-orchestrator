...
describe("script-runner", () => {
	...
	it.skipIf(process.platform === "win32")("uses the package root for packaged installs inside node_modules", () => {
		...
		expect(resolveDefaultRepoRootFromPath(modulePath)).toBe("/usr/local/lib/node_modules/@aoagents/ao-cli");
	});
	...
	it("includes the expected scripts path in missing-script errors", () => {
		const expectedScriptsDir = resolve(dirname(fileURLToPath(import.meta.url)), "../../src/assets/scripts");
	...
	it.skipIf(process.platform !== "win32")("does not rewrite to .ps1 for scripts that do not end in .sh", async () => {
		...
		await expect(runRepoScript("ao-doctor.nope", [])).rejects.toThrowError(/Script not found: ao-doctor\.nope/);

...
function mockFailure(message = "command failed"): void {
	mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: unknown, cb: (err: Error) => void) => {
...
describe("tmux", () => {
	it("delegates to execSilent with tmux command", async () => {
		...
		expect(mockExecFile).toHaveBeenCalledWith("tmux", ["list-sessions"], expect.any(Object), expect.any(Function));

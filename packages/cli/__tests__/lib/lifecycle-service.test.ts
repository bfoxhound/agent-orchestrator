...
describe("lifecycle-service", () => {
	...
	it("throws on unknown projects", async () => {
		await expect(ensureLifecycleWorker(makeConfig(["app"]), "missing")).rejects.toThrow(/Unknown project/);
	});
	...
	it("stopLifecycleWorker stops only the requested project", async () => {
		...
		mockGetLifecycleManager.mockImplementation(async (_cfg, projectId: string) => (projectId === "a" ? a : b));

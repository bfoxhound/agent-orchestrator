...
beforeEach(() => {
	tmpDir = makeTempRoot("ao-dashboard-test-");
	mockExec.mockReset();
...
describe("cleanNextCache", () => {
	it("deletes .next directory when it exists", async () => {
		...
		writeFileSync(join(webDir, ".next", "server", "vendor-chunks", "xterm@5.3.0.js"), "module.exports = {}");
...
describe("isInstalledUnderNodeModules", () => {
	...
	it("returns false for source paths containing node_modules as plain text", async () => {
		...
		expect(isInstalledUnderNodeModules("/home/user/node_modules_backup/agent-orchestrator/packages/web")).toBe(false);
	});
});
...
describe("assertDashboardRebuildSupported", () => {
	it("passes for a source checkout", async () => {
		...
		expect(() => assertDashboardRebuildSupported("/home/user/agent-orchestrator/packages/web")).not.toThrow();
	});

	it("throws for an npm-installed package path", async () => {
		const { assertDashboardRebuildSupported } = await import("../../src/lib/dashboard-rebuild.js");

		expect(() => assertDashboardRebuildSupported("/usr/local/lib/node_modules/@aoagents/ao-web")).toThrow(
...
describe("rebuildDashboardProductionArtifacts", () => {
	...
	it("throws when called from an npm-installed path", async () => {
		...
		await expect(rebuildDashboardProductionArtifacts("/usr/local/lib/node_modules/@aoagents/ao-web")).rejects.toThrow(
			"Dashboard rebuild is only available from a source checkout",
		);
	});
});
...
describe("findRunningDashboardPidsForWebDir", () => {
	...
	it.skipIf(process.platform === "win32")("returns only listeners whose cwd matches the web directory", async () => {
		...
		const { findRunningDashboardPidsForWebDir } = await import("../../src/lib/dashboard-rebuild.js");
	...
	it.runIf(process.platform === "win32")("deduplicates dashboard pids found on multiple ports on Windows", async () => {
		const webDir = join(tmpDir, "packages", "web");
		mkdirSync(webDir, { recursive: true });

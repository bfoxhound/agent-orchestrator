...
import { mkdtempSync, mkdirSync, writeFileSync, rmSync, existsSync, readdirSync, readFileSync, realpathSync } from "node:fs";
import { join } from "node:path";
import { tmpdir } from "node:os";
import { type Session, type SessionManager, getSessionsDir, sessionFromMetadata } from "@aoagents/ao-core";

const { mockTmux, mockExec, mockGh, mockConfigRef, mockSessionManager, sessionsDirRef } = vi.hoisted(() => ({
	mockTmux: vi.fn(),
	mockExec: vi.fn(),
	mockGh: vi.fn(),
...
beforeEach(() => {
	tmpDir = makeTempRoot("ao-review-test-");
...
describe("review-check command", () => {
	...
	it("skips sessions with non-matching prefix", async () => {
		writeFileSync(join(sessionsDir, "other-1"), "branch=feat/fix\npr=https://github.com/org/my-app/pull/10\n");
	...
	it("sends fix prompt when not in dry-run mode", async () => {
		...
		expect(mockSessionManager.send).toHaveBeenCalledWith("app-1", expect.stringContaining("review comments"));
	...
	it("rejects unknown project ID", async () => {
		await expect(program.parseAsync(["node", "test", "review-check", "nonexistent"])).rejects.toThrow(
			"process.exit(1)",
		);
	});
});

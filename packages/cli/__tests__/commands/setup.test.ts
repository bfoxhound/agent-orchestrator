...
vi.mock("@aoagents/ao-core", () => ({
	CONFIG_SCHEMA_URL: "https://raw.githubusercontent.com/ComposioHQ/agent-orchestrator/main/schema/config.schema.json",
...
`);
		...
		it("merges existing allowedSessionKeyPrefixes in openclaw.json", async () => {
			...
			const openclawWrite = mockWriteFileSync.mock.calls.find(([path]) => path === openclawConfigPath);
	...
	describe("error handling", () => {
		...
		it("exits when --url missing and gateway unreachable in non-interactive mode", async () => {
			...
			await expect(
				program.parseAsync(["node", "test", "setup", "openclaw", "--token", "tok", "--non-interactive"]),
			).rejects.toThrow("process.exit");

...
describe("session ls", () => {
	...
	it("marks metadata-based orchestrators correctly in JSON output", async () => {
		writeFileSync(join(sessionsDir, "app-control"), "branch=control\nstatus=working\nrole=orchestrator\n");
...
describe("session kill", () => {
	it("rejects unknown session (no matching project)", async () => {
		...
		await expect(program.parseAsync(["node", "test", "session", "kill", "unknown-1"])).rejects.toThrow(
			"process.exit(1)",
		);
	});
...
describe("session attach", () => {
	...
	it("fails when tmux session does not exist", async () => {
		...
		await expect(program.parseAsync(["node", "test", "session", "attach", "unknown-1"])).rejects.toThrow(
			"process.exit(1)",
		);
	});
...
describe("session claim-pr", () => {
	...
	it("claims a PR for an explicit session", async () => {
		await program.parseAsync(["node", "test", "session", "claim-pr", "42", "app-2", "--assign-on-github"]);
	...
	it("fails when no session can be resolved", async () => {
		await expect(program.parseAsync(["node", "test", "session", "claim-pr", "42"])).rejects.toThrow("process.exit(1)");
	});
});
...
describe("session remap", () => {
	...
	it("fails with exit code when remap errors", async () => {
		...
		await expect(program.parseAsync(["node", "test", "session", "remap", "app-1"])).rejects.toThrow("process.exit(1)");
	});
});

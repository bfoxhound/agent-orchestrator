...
describe("report commands", () => {
	...
	it("rejects unknown states before touching the session manager", async () => {
		await expect(program.parseAsync(["node", "test", "report", "bogus-state"])).rejects.toThrow("process.exit(1)");
	...
	it("rejects invalid PR numbers", async () => {
		await expect(program.parseAsync(["node", "test", "report", "pr-created", "--pr-number", "abc"])).rejects.toThrow(
			"process.exit(1)",
		);
	...
	it("rejects PR metadata flags for non-PR workflow states", async () => {
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("PR metadata flags are only valid"));
	});

	it("surfaces session-not-found errors", async () => {
		mockSessionManager.get.mockResolvedValue(null);

		await expect(program.parseAsync(["node", "test", "report", "working", "--session", "app-1"])).rejects.toThrow(
			"process.exit(1)",
		);
	...
	it("surfaces project-not-found errors", async () => {
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining("Project not found for session"));
	});

...
describe("send command", () => {
	describe("session existence check", () => {
		it("exits with error when session does not exist", async () => {
			...
			await expect(program.parseAsync(["node", "test", "send", "nonexistent", "hello"])).rejects.toThrow(
				"process.exit(1)",
			);
	...
	describe("busy detection", () => {
		it("detects idle session via agent plugin", async () => {
			...
			expect(mockExec).toHaveBeenCalledWith("tmux", ["send-keys", "-t", "my-session", "-l", "hello world"]);
			// Should have sent Enter
			expect(mockExec).toHaveBeenCalledWith("tmux", ["send-keys", "-t", "my-session", "Enter"]);
			expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Message sent and processing"));
		});
		...
		it("detects busy session and waits via agent plugin", async () => {
			...
			expect(mockExec).toHaveBeenCalledWith("tmux", ["send-keys", "-t", "my-session", "-l", "fix the bug"]);
		}, 30_000);
	...
	describe("auto-prefix from AO_SESSION_ID", () => {
		...
		it("auto-prefixes when delivering through SessionManager.send too", async () => {
			...
			expect(mockSessionManager.send).toHaveBeenCalledWith("app-1", "[from app-orchestrator] fix the build");
		});
	});
	...
	describe("session manager integration", () => {
		...
		it("routes AO sessions through SessionManager.send", async () => {
			...
			expect(mockExec).not.toHaveBeenCalledWith("tmux", expect.arrayContaining(["send-keys", "-l", "hello opencode"]));
			expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Message sent and processing"));
		});
		...
		it("skips tmux busy detection when lifecycle send handles delivery", async () => {
			...
			expect(consoleSpy).not.toHaveBeenCalledWith(expect.stringContaining("Waiting for app-1 to become idle"));
			expect(mockTmux).not.toHaveBeenCalledWith("capture-pane", "-t", "tmux-target-1", "-p", "-S", expect.any(String));
		});
		...
		it("fails loudly when lifecycle delivery fails for an AO session", async () => {
			...
			await expect(program.parseAsync(["node", "test", "send", "app-1", "hello"])).rejects.toThrow("process.exit(1)");

			expect(consoleErrorSpy).toHaveBeenCalledWith(
				expect.stringContaining("Cannot send to session app-1: session is not running"),
			);
			expect(consoleSpy).not.toHaveBeenCalledWith(expect.stringContaining("Message sent and processing"));
		});

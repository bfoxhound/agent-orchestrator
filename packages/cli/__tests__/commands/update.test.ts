...
describe("update command", () => {
	...
	it("rejects conflicting smoke flags", async () => {
		await expect(program.parseAsync(["node", "test", "update", "--skip-smoke", "--smoke-only"])).rejects.toThrow(
			"process.exit(1)",
		);
	...
	describe("git-only flags rejected on non-git installs", () => {
		it.each(["npm-global", "pnpm-global", "bun-global", "homebrew", "unknown"])(
			...
			async (method) => {
				...
				await expect(program.parseAsync(["node", "test", "update", "--skip-smoke"])).rejects.toThrow("process.exit(1)");
		...
		it("rejects --smoke-only on npm installs with an actionable message", async () => {
			...
			const errSpy = vi.mocked(console.error);
			await expect(program.parseAsync(["node", "test", "update", "--smoke-only"])).rejects.toThrow("process.exit(1)");
			const messages = errSpy.mock.calls.map((c) => String(c[0])).join("\n");
		...
		it("still accepts --skip-smoke on git installs", async () => {
			...
			expect(mockRunRepoScript).toHaveBeenCalledWith("ao-update.sh", expect.arrayContaining(["--skip-smoke"]));
		});
	});
	...
	describe("git install", () => {
		...
		it("shows an actionable error when the bundled update script is missing", async () => {
			mockRunRepoScript.mockRejectedValue(new Error("Script not found: ao-update.sh. Expected at: /tmp/ao-update.sh"));

			await expect(program.parseAsync(["node", "test", "update"])).rejects.toThrow("process.exit(1)");
	...
	describe("npm-global install", () => {
		...
		it("exits non-zero when registry is unreachable", async () => {
			mockCheckForUpdate.mockResolvedValue(makeNpmUpdateInfo({ latestVersion: null, isOutdated: false }));

			await expect(program.parseAsync(["node", "test", "update"])).rejects.toThrow("process.exit(1)");
			expect(vi.mocked(console.error)).toHaveBeenCalledWith(expect.stringContaining("Could not reach npm registry"));
		});
		...
		it("prints exit code when npm install fails", async () => {
			...
			expect(vi.mocked(console.error)).toHaveBeenCalledWith(expect.stringContaining("exited with code 1"));
		});
		...
		it("does not print a null exit code when npm install is killed by a signal", async () => {
			...
			expect(vi.mocked(console.error)).not.toHaveBeenCalledWith(expect.stringContaining("exited with code null"));
			expect(mockInvalidateCache).not.toHaveBeenCalled();
		});
	...
	describe("active-session guard", () => {
		...
		it("always loads global config (never project-local), so sessions in OTHER projects fire the guard", async () => {
			...
			mockSessions.value = [{ id: "other-1", status: "working" }];
		...
		it("refuses when sessions exist in a locally-registered project not in global config (Dhruv edge-case)", async () => {
			...
			mockSessions.value = [{ id: "local-feat-1", status: "working", projectId: "local-only" }];
	...
	describe("channel-switch detection", () => {
		...
		it("forces an explicit prompt when active channel differs from cached.channel and !isOutdated", async () => {
			...
			expect(mockPromptConfirm).toHaveBeenCalledWith(expect.stringMatching(/Switch to nightly/), false);
			expect(mockSpawn).toHaveBeenCalled();
		});

...
describe("start command — project resolution", () => {
	...
	it("uses explicit project arg when given", async () => {
		...
		await program.parseAsync(["node", "test", "start", "backend", "--no-dashboard", "--no-orchestrator"]);
	...
	it("errors when explicit project not found", async () => {
		...
		await expect(
			program.parseAsync(["node", "test", "start", "nonexistent", "--no-dashboard", "--no-orchestrator"]),
	...
	it("errors when multiple projects and no arg", async () => {
		...
		await expect(program.parseAsync(["node", "test", "start", "--no-dashboard", "--no-orchestrator"])).rejects.toThrow(
			"process.exit(1)",
		);
...
describe("start command — URL argument", () => {
	...
	it("clones repo via gh when gh auth is available", async () => {
		...
		mockSpawn.mockImplementation((cmd: string, args: string[], _opts?: { cwd?: string; env?: NodeJS.ProcessEnv }) => {
	...
	it("fails on clone error with descriptive message", async () => {
		mockCwd(tmpDir);
		mockSpawn.mockImplementation(() => createSpawnChild({ error: new Error("fatal: repository not found") }));
...
describe("start command — orchestrator session strategy display", () => {
	...
	it("passes the requested and reassigned dashboard ports to rebuild", async () => {
		...
		expect(dashboardRebuild.rebuildDashboardProductionArtifacts).toHaveBeenCalledWith(tmpDir, [3000, 3001]);
	});
	...
	it("reports startup lock acquisition failures through the normal CLI error path", async () => {
		mockConfigRef.current = makeConfig({ "my-app": makeProject() });
		mockAcquireStartupLock.mockRejectedValueOnce(new Error("Could not acquire startup lock (/tmp/startup.lock)"));
	...
	it("attributes other-project restore failures to the owning project", async () => {
		...
		const restoreFailedEvents = recordedEvents().filter((e) => e.kind === "cli.restore_session_failed");
...
describe("stop command", () => {
	...
	it.skipIf(process.platform === "win32")("finds orphaned dashboard on a reassigned port via port scan", async () => {
		mockConfigRef.current = makeConfig({ "my-app": makeProject() });
		mockSessionManager.list.mockResolvedValue([]);
		// Port 3000 has nothing; port 3001 has the orphaned dashboard
		mockFindPidByPort.mockImplementation(async (port: number) => (port === 3001 ? "99999" : null));
...
describe("start command — platform-aware runtime fallback", () => {
	...
	it("ao start <project> while daemon alive but project removed: attaches to existing daemon (no second dashboard)", async () => {
		...
		try {
			...
			await expect(
				program.parseAsync(["node", "test", "start", "project-2", "--no-dashboard", "--no-orchestrator"]),
			).rejects.toThrow("process.exit(1)");
...
describe("start command — already-running detection", () => {
	...
	it("path arg unregistered + AO running: registers in global config and spawns orchestrator without showing the menu", async () => {
		...
		try {
			...
			vi.mocked(shell.git).mockImplementation(async (args: string[], workingDir?: string) => {
				if (args[0] === "rev-parse" && args[1] === "--git-dir" && workingDir === repoDir) return ".git";
				if (args[0] === "remote" && args[1] === "get-url" && args[2] === "origin" && workingDir === repoDir) {
					return "https://github.com/org/new-repo.git";
				}
				if (args[0] === "symbolic-ref" && workingDir === repoDir) return "refs/remotes/origin/main";
				if (args[0] === "rev-parse" && args[1] === "--verify" && workingDir === repoDir) return "abc";
				return null;
			});
	...
	it("offers to add cwd when AO is running and cwd is an unregistered git repo", async () => {
		...
		const options = mockPromptSelect.mock.calls[0]?.[1] as Array<{ value: string; label: string }> | undefined;
		expect(options?.some((option) => option.value === "add" && option.label.includes("Add"))).toBe(true);
	});
...
describe("start command — path-based deduplication in addProjectToConfig", () => {
	it("skips addProjectToConfig when path arg matches an existing project", async () => {
		...
		try {
			...
			await program.parseAsync(["node", "test", "start", repoDir, "--no-dashboard", "--no-orchestrator"]);
...
describe("start command — global registry mutations", () => {
	it("adds a project to the global registry and writes behavior to the repo-local config", async () => {
		...
		try {
			try {
				await program.parseAsync(["node", "test", "start", addedRepoDir, "--no-dashboard", "--no-orchestrator"]);
			} catch (error) {
				const loggedErrors = vi
					.mocked(console.error)
					.mock.calls.map((call) => call.join(" "))
					.join("\n");
				throw new Error(`${error instanceof Error ? error.message : String(error)}\n${loggedErrors}`, { cause: error });
			}
	...
	it("writes interactive agent overrides to the repo-local config when using the global registry", async () => {
		...
		try {
			await program.parseAsync(["node", "test", "start", "--interactive", "--no-dashboard", "--no-orchestrator"]);

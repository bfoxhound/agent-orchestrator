...
const { mockExec, mockSpawn, mockConfigRef, mockListRef, mockOpenUrl, mockIsMacRef, mockIsWindowsRef, mockRunningRef } =
	vi.hoisted(() => ({
		mockExec: vi.fn(),
		mockSpawn: vi.fn(),
		...
		mockRunningRef: {
			current: { pid: 1, port: 3000, projects: [] } as { pid: number; port: number; projects: string[] } | null,
		},
	}));
...
describe("open command (macOS)", () => {
	...
	it("rejects unknown target", async () => {
		...
		await expect(program.parseAsync(["node", "test", "open", "nonexistent"])).rejects.toThrow("process.exit(1)");
	});
	...
	it("falls back gracefully when open-iterm-tab fails", async () => {
		...
		expect(mockOpenUrl).toHaveBeenCalledWith("http://localhost:3000/projects/my-app/sessions/app-1");
	});

	it("excludes terminated sessions from aggregate targets", async () => {
		mockListRef.current = [makeSession("app-1", "my-app"), makeSession("app-dead", "my-app", "terminated")];
	...
	it("includes a terminated session when looked up by name (opens dashboard with death reason)", async () => {
		...
		expect(mockOpenUrl).toHaveBeenCalledWith("http://localhost:3000/projects/my-app/sessions/app-dead");
		const output = consoleSpy.mock.calls.map((c) => String(c[0])).join("\n");
	...
	it("uses the live daemon's port from running-state, not config", async () => {
		...
		expect(mockOpenUrl).toHaveBeenCalledWith("http://localhost:4173/projects/my-app/sessions/app-1");
	});
...
describe("open command (Windows)", () => {
	...
	it("falls back to dashboard URL when both terminal launchers fail", async () => {
		...
		expect(mockOpenUrl).toHaveBeenCalledWith("http://localhost:3000/projects/test-repo/sessions/tr-orchestrator");
	});

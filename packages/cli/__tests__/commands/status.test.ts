 ...
 */
function buildSessionsFromDir(dir: string, projectId: string, activityOverride?: ActivityState | null): Session[] {
...
describe("status command", () => {
	...
	it("ignores --interval entirely when --watch is not set", async () => {
		...
		await expect(program.parseAsync(["node", "test", "status", "--interval", "0"])).resolves.not.toThrow();

		// Valid value should also be silently ignored without --watch
		await expect(program.parseAsync(["node", "test", "status", "--interval", "10"])).resolves.not.toThrow();
	});
	...
	it("includes orchestrators in JSON output with explicit roles", async () => {
		...
		expect(parsed.find((entry: { name: string }) => entry.name === "app-orchestrator")).toMatchObject({
			role: "orchestrator",
			project: "my-app",
		});
	...
	it("exits with error when --project refers to an unknown project", async () => {
		...
		await expect(program.parseAsync(["node", "test", "status", "--project", "no-such-project"])).rejects.toThrow(
			"process.exit(1)",
		);

...
describe("events command", () => {
	...
	it("filters list output by source and --kind alias", async () => {
		...
		expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('"kind": "metadata.corrupt_detected"'));
	});

	it("keeps --type as the existing event-kind filter", async () => {
		await program.parseAsync(["node", "test", "events", "list", "--type", "recovery.session_failed", "--json"]);

...
describe("spawn command", () => {
	...
	it("rejects more than one positional arg with replacement usage", async () => {
		...
		await expect(program.parseAsync(["node", "test", "spawn", "my-app", "INT-100"])).rejects.toThrow("process.exit(1)");
	...
	it("rejects --assign-on-github without --claim-pr", async () => {
		await expect(program.parseAsync(["node", "test", "spawn", "--assign-on-github"])).rejects.toThrow(
			"process.exit(1)",
		);
	...
	it("reports claim failures after creating the session", async () => {
		...
		await expect(program.parseAsync(["node", "test", "spawn", "--claim-pr", "123"])).rejects.toThrow("process.exit(1)");

		const errors = vi
			.mocked(console.error)
			.mock.calls.map((c) => String(c[0]))
			.join("\n");
		expect(errors).toContain("Session app-1 was created, but failed to claim PR 123: already tracked by app-9");
	});
});
	...
	it("surfaces a plugin's preflight error and aborts before sm.spawn", async () => {
		mockRegistryGet.mockImplementation((slot: string) => {
			if (slot === "runtime") {
				return {
					name: "tmux",
					preflight: vi.fn().mockRejectedValue(new Error("tmux is not installed. Install it: brew install tmux")),
				};
			}
			return null;
		});
...
describe("batch-spawn command", () => {
	...
	it("groups cross-project issues and routes each to the correct project", async () => {
		...
		await program.parseAsync(["node", "test", "batch-spawn", "agent-orchestrator/10", "x402-identity/20"]);
	...
	it("skips a prefixed issue that already has an active session in the target project", async () => {
		...
		mockSessionManager.spawn.mockResolvedValueOnce(makeFakeSession({ id: "ao-2", projectId: "agent-orchestrator" }));
...
describe("batch-spawn daemon-polling enforcement", () => {
	...
	it("refuses to batch-spawn when no AO daemon is running", async () => {
		...
		await expect(batchProgram.parseAsync(["node", "test", "batch-spawn", "INT-1", "INT-2"])).rejects.toThrow(
			"process.exit(1)",
		);

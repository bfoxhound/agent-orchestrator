...
describe("ao project add", () => {
	it("registers a valid project path", async () => {
		...
		expect(mockRegisterProject).toHaveBeenCalledWith(expect.stringContaining("my-project"), "my-project", "my-project");
	...
	it("exits with error when no config found at path", async () => {
		...
		await expect(program.parseAsync(["node", "ao", "project", "add", "/tmp/no-config"])).rejects.toThrow();
	...
	it("passes --key option to registerProject", async () => {
		...
		await program.parseAsync(["node", "ao", "project", "add", "/tmp/my-project", "-k", "custom-key"]);

		expect(mockRegisterProject).toHaveBeenCalledWith(expect.stringContaining("my-project"), "custom-key", "my-project");
	});
	...
	it("surfaces duplicate path collisions as errors", async () => {
		...
		await expect(program.parseAsync(["node", "ao", "project", "add", "/tmp/my-project"])).rejects.toThrow();
...
describe("ao project rm", () => {
	...
	it("exits with error when project not found", async () => {
		...
		await expect(program.parseAsync(["node", "ao", "project", "rm", "nonexistent"])).rejects.toThrow();
...
describe("ao project set-default", () => {
	...
	it("exits with error when project not found", async () => {
		...
		await expect(program.parseAsync(["node", "ao", "project", "set-default", "nonexistent"])).rejects.toThrow();

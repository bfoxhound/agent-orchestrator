...
function createHealthyPath(binDir: string): void {
	createFakeBinary(binDir, "node", 'if [ "$1" = "--version" ]; then\n  printf "v20.11.1\\n"\n  exit 0\nfi\nexit 0');
	createFakeBinary(
		binDir,
		"git",
		'if [ "$1" = "--version" ]; then\n  printf "git version 2.43.0\\n"\n  exit 0\nfi\nexit 0',
	);
	createFakeBinary(binDir, "pnpm", 'if [ "$1" = "--version" ]; then\n  printf "9.15.4\\n"\n  exit 0\nfi\nexit 0');
	createFakeBinary(binDir, "npm", 'if [ "$1" = "bin" ]; then\n  printf "/tmp/npm-bin\\n"\n  exit 0\nfi\nexit 0');
	createFakeBinary(
		binDir,
		"tmux",
...
describe.skipIf(process.platform === "win32")("ao-doctor.sh", () => {
	it("reports a healthy install as PASS", () => {
		...
		writeFileSync(configPath, [`dataDir: ${dataDir}`, `worktreeDir: ${worktreeDir}`, "projects: {}"].join("\n"));

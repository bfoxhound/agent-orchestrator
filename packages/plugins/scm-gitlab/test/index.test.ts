...
describe("scm-gitlab plugin", () => {
	...
	describe("verifyWebhook", () => {
		...
		it("rejects invalid token when secret env var is configured", async () => {
			...
			expect(result).toEqual(expect.objectContaining({ ok: false, reason: "Webhook token verification failed" }));
		});
	});
	...
	describe("detectPR", () => {
		...
		it("returns null and warns on glab CLI error", async () => {
			...
			const result = await scm.detectPR(makeSession(), project);
			expect(result).toBeNull();
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('detectPR: failed for branch "feat/my-feature"'));
		});
	...
	describe("mergePR", () => {
		...
		it("uses --rebase when specified", async () => {
			...
			expect(glabMock).toHaveBeenCalledWith("glab", expect.arrayContaining(["--rebase"]), expect.any(Object));
		});
	...
	describe("closePR", () => {
		it("calls glab mr close", async () => {
			...
			expect(glabMock).toHaveBeenCalledWith("glab", ["mr", "close", "42", "--repo", "acme/repo"], expect.any(Object));
		});
	});
	...
	describe("getCIChecks", () => {
		...
		it("correctly encodes project ID with subgroups in API path", async () => {
			...
			expect(firstCallArgs).toContain("projects/org%2Fsub-group%2Frepo/merge_requests/42/pipelines");
		});

		it("handles unknown job status as failed (fail-closed)", async () => {
			mockGlab([{ id: 100 }]);
			mockGlab([{ name: "mystery", status: "new_status", web_url: "", started_at: null, finished_at: null }]);
			const checks = await scm.getCIChecks(pr);
	...
	describe("getCISummary", () => {
		...
		it('returns "failing" and warns on error (fail-closed)', async () => {
			...
			expect(await scm.getCISummary(pr)).toBe("failing");
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("getCISummary: CI check fetch failed for MR !42"));
		...
		it('returns "none" when CI fetch fails but MR is merged', async () => {
			...
			mockGlab({ state: "merged" });
			expect(await scm.getCISummary(pr)).toBe("none");
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("getCISummary: CI check fetch failed"));
		});
		...
		it('returns "none" when all jobs are skipped', async () => {
			mockGlab([{ id: 1 }]);
			mockGlab([{ name: "a", status: "skipped", web_url: "", started_at: null, finished_at: null }]);
			expect(await scm.getCISummary(pr)).toBe("none");
		});
	});
	...
	describe("getReviews", () => {
		...
		it("returns approvals only and warns when discussions fetch fails", async () => {
			...
			expect(reviews).toHaveLength(1);
			expect(reviews[0]).toMatchObject({ author: "alice", state: "approved" });
			expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("getReviews: discussions fetch failed for MR !42"));
		});
	...
	describe("getMergeability", () => {
		...
		it("reports CI failures as blockers", async () => {
			...
			mockGlab([{ name: "build", status: "failed", web_url: "", started_at: null, finished_at: null }]); // jobs

...
function parseGitLabRepository(payload: Record<string, unknown>) {
	...
	const namespace = typeof projectRecord["namespace"] === "string" ? projectRecord["namespace"] : undefined;
...
function isGitLabTagRef(
	...
	return objectAttributes?.["tag"] === true || payload["ref_type"] === "tag" || payload["object_kind"] === "tag_push";
}
...
function parseGitLabWebhookEvent(
	...
	if (normalizedEventType === "note hook" || payload["object_kind"] === "note") {
		...
		return {
			...
			deliveryId,
			repository,
			prNumber: typeof mergeRequest["iid"] === "number" ? (mergeRequest["iid"] as number) : undefined,
			branch: parseWebhookBranchRef(mergeRequest["source_branch"]),
			sha:
				typeof mergeRequest["last_commit"] === "object" && mergeRequest["last_commit"]
					? ((mergeRequest["last_commit"] as Record<string, unknown>)["id"] as string | undefined)
					: undefined,
			timestamp: parseWebhookTimestamp(objectAttributes?.["updated_at"] ?? objectAttributes?.["created_at"]),
			data: payload,
		};
	}
	...
	return {
		...
		deliveryId,
		repository,
		timestamp: parseWebhookTimestamp(objectAttributes?.["updated_at"] ?? payload["event_created_at"]),
		data: payload,
	};
}
...
function createGitLabSCM(config?: Record<string, unknown>): SCM {
	...
	return {
		...
		async detectPR(session: Session, project: ProjectConfig): Promise<PRInfo | null> {
			...
			try {
				const raw = await glab(
					["mr", "list", "--source-branch", session.branch, "--repo", project.repo, "-F", "json", "-P", "1"],
					resolveHostname(),
				);
		...
		async getPRSummary(pr: PRInfo) {
			...
			const data = parseJSON<{ state: string; title: string }>(raw, `getPRSummary for MR !${pr.number}`);
			return {
		...
		async getCIChecks(pr: PRInfo): Promise<CICheck[]> {
			try {
				...
				const pipelines = parseJSON<Array<{ id: number }>>(pipelinesRaw, `getCIChecks pipelines for MR !${pr.number}`);
				if (pipelines.length === 0) return [];

				const latestPipelineId = pipelines[0].id;
				const projectId = encodeProjectId(pr.owner, pr.repo);

				const jobsRaw = await glab(["api", `projects/${projectId}/pipelines/${latestPipelineId}/jobs`], hostname);
		...
		async getReviews(pr: PRInfo): Promise<Review[]> {
			...
			try {
				const discussions = await fetchDiscussions(pr, hostname, `getReviews discussions for MR !${pr.number}`);
		...
		async getPendingComments(pr: PRInfo): Promise<ReviewComment[]> {
			const discussions = await fetchDiscussions(pr, resolveHostname(pr), `getPendingComments for MR !${pr.number}`);
		...
		async getReviewThreads(pr: PRInfo): Promise<ReviewThreadsResult> {
			const hostname = resolveHostname(pr);
			const discussions = await fetchDiscussions(pr, hostname, `getReviewThreads for MR !${pr.number}`);
		...
		async getMergeability(pr: PRInfo): Promise<MergeReadiness> {
			...
			const mrRaw = await glab(["mr", "view", String(pr.number), "--repo", repoFlag(pr), "-F", "json"], hostname);

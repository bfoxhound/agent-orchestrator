...
function isUnknownJsonFieldError(err: unknown, fieldName: string): boolean {
	...
	const unknownFieldSignals =
		text.includes("unknown json field") || text.includes("unknown field") || text.includes("invalid field");
...
async function ghIssueViewJson(identifier: string, project: ProjectConfig): Promise<string> {
	...
	try {
		...
		return gh(["issue", "view", identifier, "--repo", repo, "--json", "number,title,body,url,state,labels,assignees"]);
	}
}

async function ghIssueListJson(args: string[]): Promise<string> {
	const withStateReason = [...args, "--json", "number,title,body,url,state,stateReason,labels,assignees"];
	try {
...
function createGitHubTracker(): Tracker {
	...
	const tracker: Tracker = {
		...
		async generatePrompt(identifier: string, project: ProjectConfig): Promise<string> {
			...
			const lines = [`You are working on GitHub issue #${issue.id}: ${issue.title}`, `Issue URL: ${issue.url}`, ""];
		...
		async listIssues(filters: IssueFilters, project: ProjectConfig): Promise<Issue[]> {
			const args = ["issue", "list", "--repo", requireRepo(project), "--limit", String(filters.limit ?? 30)];
		...
		async updateIssue(identifier: string, update: IssueUpdate, project: ProjectConfig): Promise<void> {
			...
			if (update.removeLabels && update.removeLabels.length > 0) {
				await gh(["issue", "edit", identifier, "--repo", repo, "--remove-label", update.removeLabels.join(",")]);
			}

			// Handle label changes
			if (update.labels && update.labels.length > 0) {
				await gh(["issue", "edit", identifier, "--repo", repo, "--add-label", update.labels.join(",")]);
			}

...
import { _clearProcessCacheForTests, type PreflightContext, type ProjectConfig } from "@aoagents/ao-core";
...
describe("tracker-github plugin", () => {
	...
	describe("getIssue", () => {
		...
		it("falls back when gh does not support stateReason field", async () => {
			...
			expect(ghMock.mock.calls[0]?.[1]).toEqual(expect.arrayContaining([expect.stringContaining("state,stateReason")]));
		...
		it("does not swallow unrelated unknown-field errors", async () => {
			...
			await expect(tracker.getIssue("123", project)).rejects.toThrow('Unknown JSON field "milestone"');

...
describe("getLaunchCommand", () => {
	...
	it("combines all options", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ permissions: "permissionless", model: "o3", prompt: "Go" }));
		expect(cmd).toBe(
...
describe("getRestoreCommand", () => {
	...
	it("includes bypass flag when project config permissions=permissionless", async () => {
		const content = jsonl({ type: "session_meta", cwd: "/workspace/test", model: "gpt-4o" }, { threadId: "thread-1" });
		mockReaddir.mockResolvedValue(["sess.jsonl"]);
		setupMockOpen(content);
		setupMockStream(content);
		mockReadFile.mockResolvedValue(content);
	...
	it("falls back to session model when project config has no model", async () => {
		const content = jsonl({ type: "session_meta", cwd: "/workspace/test", model: "o4-mini" }, { threadId: "thread-1" });
		mockReaddir.mockResolvedValue(["sess.jsonl"]);
		setupMockOpen(content);
		setupMockStream(content);
		mockReadFile.mockResolvedValue(content);
...
describe("postLaunchSetup", () => {
	...
	it("runs setup when session has workspacePath", async () => {
		...
		await expect(agent.postLaunchSetup!(makeSession({ workspacePath: "/workspace/test" }))).resolves.toBeUndefined();
	});

...
// ---------------------------------------------------------------------------
const { mockReadLastActivityEntry, mockRecordTerminalActivity, mockSetupPathWrapperWorkspace, mockIsWindows } =
	vi.hoisted(() => ({
...
import { create, manifest, default as defaultExport, detect, _resetSessionMatchCache } from "./index.js";
...
describe("getLaunchCommand", () => {
	...
	it("concatenates systemPromptFile + prompt with separator when both set", () => {
		...
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ systemPromptFile: sysFile, prompt: "USER_TASK" }));
...
describe("detectActivity", () => {
	...
	it("does NOT match 'failed to connect' mid-sentence (false-positive guard)", () => {
		const narration = "Earlier I failed to connect on the first try, but the retry worked.\nGenerating code...";
		expect(agent.detectActivity(narration)).toBe("active");
	});
...
describe("getActivityState", () => {
	...
	it("first successful match writes the pin file (locks in the recency winner)", async () => {
		...
		const pin = JSON.parse(readFileSync(join(realWorkspace, ".ao", "kimi-session-id.json"), "utf-8"));
	...
	it("preLaunchSetup is write-once — restore preserves the original baseline", async () => {
		...
		writeFileSync(join(realWorkspace, ".ao", "kimi-baseline.json"), JSON.stringify(originalBaseline));
		...
		const baselineNow = JSON.parse(readFileSync(join(realWorkspace, ".ao", "kimi-baseline.json"), "utf-8"));
	...
	it("returns null when workspacePath is missing (no source of truth)", async () => {
		mockTmuxWithProcess("kimi");
		const result = await agent.getActivityState(makeSession({ runtimeHandle: makeTmuxHandle(), workspacePath: null }));
		expect(result).toBeNull();
	});
	...
	it("resolves symlinked workspace paths before hashing (kimi sees realpath as cwd)", async () => {
		...
		const result = await agent.getActivityState(makeSession({ runtimeHandle: makeTmuxHandle(), workspacePath: link }));
		expect(result?.state).toBe("active");
	});
...
describe("getRestoreCommand", () => {
	...
	it("returns null when workspacePath is missing", async () => {
		const result = await agent.getRestoreCommand!(makeSession({ workspacePath: null }), makeProject());
		expect(result).toBeNull();
	});

	it("returns null when no kimi session dir exists", async () => {
		const result = await agent.getRestoreCommand!(makeSession({ workspacePath: workspace }), makeProject());
		expect(result).toBeNull();
	});

...
function createOpenCodeAgent(): Agent {
	return {
		...
		getLaunchCommand(config: AgentLaunchConfig): string {
			...
			if (!existingSessionId) {
				const runOptions = ["--format", "json", "--title", shellEscape(`AO:${config.sessionId}`), ...sharedOptions];
				...
				const missingSessionError = shellEscape(`failed to discover OpenCode session ID for AO:${config.sessionId}`);
				return [

...
const { mockAppendActivityEntry, mockReadLastActivityEntry, mockRecordTerminalActivity, mockIsWindows } = vi.hoisted(
	() => ({
...
vi.mock("@aoagents/ao-core", async (importOriginal) => {
	...
	return {
		...
		buildAgentPath: (existing: string | undefined) => ["~/.ao/bin", existing ?? ""].filter(Boolean).join(":"),
	};
});
	...
	execFile: (...args: unknown[]) => {
		...
		if (typeof callback === "function") {
			...
			if (result && typeof result.then === "function") {
				result.then((r: { stdout: string; stderr: string }) => callback(null, r)).catch((e: Error) => callback(e));
			}
		}
	},
}));
...
describe("getLaunchCommand", () => {
	...
	it("combines prompt and model", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ prompt: "Go", model: "claude-sonnet-4-5-20250929" }));
		expect(cmd).toContain("opencode run --format json --title 'AO:sess-1' --model 'claude-sonnet-4-5-20250929'");
		expect(cmd).toContain("exec opencode --session \"$SES_ID\" --prompt 'Go' --model 'claude-sonnet-4-5-20250929'");
		expect(cmd).toContain("--model 'claude-sonnet-4-5-20250929'");
	});
	...
	it("generates command with agent and prompt", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ subagent: "sisyphus", prompt: "fix bug" }));
		expect(cmd).toContain("opencode run --format json --title 'AO:sess-1' --agent 'sisyphus'");
		expect(cmd).toContain("exec opencode --session \"$SES_ID\" --prompt 'fix bug' --agent 'sisyphus'");
		expect(cmd).toContain("--agent 'sisyphus'");
	});
	...
	it("shell-escapes sessionId in the discovery failure message", () => {
		...
		expect(cmd).toContain("echo 'failed to discover OpenCode session ID for AO:sess-1; rm -rf /' >&2");
	});
	...
	it("works with different agent names: oracle", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ subagent: "oracle", prompt: "review code" }));
		expect(cmd).toContain("--agent 'oracle'");
		expect(cmd).toContain("'review code'");
	});

	it("works with different agent names: librarian", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ subagent: "librarian", prompt: "find usages" }));
	...
	it("uses run bootstrap when systemPrompt is provided", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ systemPrompt: "You are an orchestrator" }));
		expect(cmd).toContain("opencode run --format json --title 'AO:sess-1'");
		expect(cmd).toContain('exec opencode --session "$SES_ID"');
	...
	it("escapes path in systemPromptFile", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ systemPromptFile: "/tmp/it's-prompt.md" }));
	...
	it("keeps subagent and prompt separate when systemPromptFile is set", () => {
		...
		expect(cmd).toContain("opencode run --format json --title 'AO:sess-1' --agent 'sisyphus'");
		expect(cmd).toContain(`exec opencode --session "$SES_ID" --prompt 'fix the bug' --agent 'sisyphus'`);
	});
	...
	it("handles prompt with special characters", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ prompt: "fix $PATH/to/file and `rm -rf /unquoted/path`" }));
...
describe("getSessionInfo", () => {
	...
	it("returns session info when matching session found by metadata ID", async () => {
		mockOpencodeSessionRows([{ id: "ses_abc123", title: "AO:test-1", updated: new Date().toISOString() }]);

		const info = await agent.getSessionInfo(makeSession({ metadata: { opencodeSessionId: "ses_abc123" } }));
	...
	it("returns session info when matching session found by title", async () => {
		mockOpencodeSessionRows([{ id: "ses_xyz789", title: "AO:test-1", updated: new Date().toISOString() }]);
	...
	it("returns null when no matching session", async () => {
		mockOpencodeSessionRows([{ id: "ses_other", title: "AO:different", updated: new Date().toISOString() }]);
...
describe("getRestoreCommand", () => {
	...
	it("returns restore command from metadata session ID", async () => {
		const cmd = await agent.getRestoreCommand!(makeSession({ metadata: { opencodeSessionId: "ses_abc123" } }), {
			name: "proj",
			repo: "o/r",
			path: "/p",
			defaultBranch: "main",
...
describe("getActivityState with activity JSONL", () => {
	...
	it("falls back to JSONL entry state when session list fails", async () => {
		...
		const result = await agent.getActivityState(makeSession({ runtimeHandle: makeTmuxHandle() }), 60_000);
		expect(result?.state).toBe("active");
	});

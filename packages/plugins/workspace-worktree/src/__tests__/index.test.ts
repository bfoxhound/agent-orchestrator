...
const mockExecFileAsync = (childProcess.execFile as any)[Symbol.for("nodejs.util.promisify.custom")] as ReturnType<
	typeof vi.fn
>;
...
describe("workspace.create()", () => {
	it("calls git fetch and git worktree add with correct args", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith("git", ["rev-parse", "--verify", "--quiet", "origin/main"], {
			cwd: "/repo/path",
			windowsHide: true,
			timeout: 30_000,
		});
	...
	it("finds an adoptable worktree in the legacy managed worktree directory", async () => {
		...
		mockGitSuccess(
			["worktree /mock-home/.worktrees/myproject/session-1", "HEAD deadbeef", "branch refs/heads/feat/TEST-1"].join(
				"\n",
			),
		);
		mockExistsSync.mockReturnValueOnce(true);
	...
	it("returns null when no managed worktree tracks the requested branch", async () => {
		...
		mockGitSuccess(
			["worktree /mock-home/.worktrees/myproject/session-2", "HEAD deadbeef", "branch refs/heads/feat/OTHER"].join(
				"\n",
			),
		);
	...
	it("throws when the matching branch is checked out outside AO-managed worktree directories", async () => {
		...
		mockGitSuccess(["worktree /tmp/manual-worktree", "HEAD deadbeef", "branch refs/heads/feat/TEST-1"].join("\n"));
		mockExistsSync.mockReturnValueOnce(true);
	...
	it("uses refs/heads/<defaultBranch> when origin is missing", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith("git", ["rev-parse", "--verify", "--quiet", "refs/heads/main"], {
			cwd: "/repo/path",
			windowsHide: true,
			timeout: 30_000,
		});
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith(
			"git",
			["worktree", "add", "-b", "feat/TEST-1", "/mock-home/.worktrees/myproject/session-1", "refs/heads/main"],
			{ cwd: "/repo/path", windowsHide: true, timeout: 30_000 },
		);
	});
...
describe("workspace.restore()", () => {
	it("prefers origin branch refs when origin exists", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith(
			"git",
			["worktree", "add", "-b", "feat/TEST-1", "/mock-home/.worktrees/myproject/session-1", "origin/feat/TEST-1"],
			{ cwd: "/repo/path", windowsHide: true, timeout: 30_000 },
		);
	...
	it("refuses to rmSync a still-registered worktree dir (data safety)", async () => {
		...
		await expect(ws.restore!(makeCreateConfig(), "/mock-home/.worktrees/myproject/session-1")).rejects.toThrow(
			/already exists and is still registered/,
		);
...
describe("workspace.destroy()", () => {
	it("removes worktree via git commands", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith("git", ["rev-parse", "--path-format=absolute", "--git-common-dir"], {
...
describe("workspace.list()", () => {
	...
	it("handles detached HEAD worktrees", async () => {
		...
		const porcelainOutput = ["worktree /mock-home/.worktrees/myproject/session-1", "HEAD abc1234", "detached"].join(
			"\n",
		);
...
describe("workspace.postCreate()", () => {
	...
	it("removes existing target before symlinking", async () => {
		...
		expect(mockRmSync).toHaveBeenCalledWith("/mock-home/.worktrees/myproject/session-1/node_modules", {
			recursive: true,
			force: true,
		});
	...
	it("uses Windows shell (pwsh) when getShell returns pwsh", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenCalledWith("pwsh", ["-NoLogo", "-NonInteractive", "-Command", "npm install"], {
			cwd: "/mock-home/.worktrees/myproject/session-1",
			windowsHide: true,
		});
	});

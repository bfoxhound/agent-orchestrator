...
vi.mock("node:child_process", () => {
	...
	(mockExecFile as unknown as Record<symbol, unknown>)[Symbol.for("nodejs.util.promisify.custom")] = vi.fn();
...
describe("workspace.post_create_failed (MUST emit)", () => {
	it("emits when a postCreate command fails, then rethrows", async () => {
		...
		await expect(ws.postCreate!(makeWorkspaceInfo(), project)).rejects.toThrow("Command failed: exit 127");
...
describe("workspace.branch_collision (SHOULD emit)", () => {
	it("emits when worktree add -b fails because branch already exists", async () => {
		...
		mockExecFileAsync.mockRejectedValueOnce(new Error("fatal: a branch named 'feat/TEST-1' already exists"));

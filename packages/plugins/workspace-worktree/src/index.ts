...
async function isRegisteredWorktree(repoPath: string, worktreePath: string): Promise<boolean> {
	try {
		...
		return output
			.split("\n")
			.some(
				(line) => line.startsWith("worktree ") && toComparablePath(resolve(line.slice("worktree ".length))) === target,
			);
	} catch {
		return false;
	}
 ...
 */
async function reattachExistingBranch(repoPath: string, workspacePath: string, branch: string): Promise<void> {
...
export function create(config?: Record<string, unknown>): Workspace {
	...
	return {
		...
		async create(cfg: WorkspaceCreateConfig): Promise<WorkspaceInfo> {
			...
			const projectWorktreeDir = cfg.worktreeDir ? effectiveBaseDir : join(effectiveBaseDir, cfg.projectId);
		...
		async list(projectId: string): Promise<WorkspaceInfo[]> {
			...
			const dirs = entries.filter((e) => e.isDirectory()).map((e) => join(projectWorktreeDir, e.name));

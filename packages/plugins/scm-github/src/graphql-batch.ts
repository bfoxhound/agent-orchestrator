 ...
 */
export function setExecGhAsync(fn: (args: string[], timeout: number, operation: string) => Promise<string>): void {
	execGhAsync = fn;
}
 ...
 * Uses LRU eviction to ensure bounded memory usage.
 */
const prMetadataCache = new LRUCache<string, { headSha: string | null; ciStatus: CIStatus }>(MAX_PR_METADATA);
 ...
 */
function updatePRMetadataCache(prKey: string, enrichment: PREnrichmentData, headSha: string | null): void {
 ...
 */
export function setPRMetadata(key: string, metadata: { headSha: string | null; ciStatus: CIStatus }): void {
 ...
 */
async function checkPRListETag(owner: string, repo: string, observer?: BatchObserver): Promise<boolean> {
...
function parseCheckContexts(contexts: unknown): CICheck[] {
	...
	for (const node of nodes) {
		...
		if (typeof n["name"] === "string" && typeof n["status"] === "string") {
			...
			const rawConclusion = typeof n["conclusion"] === "string" ? (n["conclusion"] as string).toUpperCase() : null;
...
function parseCIState(statusCheckRollup: unknown): CIStatus {
	...
	if (state === "TIMED_OUT" || state === "CANCELLED" || state === "ACTION_REQUIRED") return "failing";
 ...
 */
function extractPREnrichment(pullRequest: unknown): { data: PREnrichmentData; headSha: string | null } | null {
	...
	const headSha =
		typeof pr["headRefOid"] === "string" ? pr["headRefOid"] : typeof pr["headSha"] === "string" ? pr["headSha"] : null;
	...
	const commits = pr["commits"] as
		{ nodes?: Array<{ commit?: { statusCheckRollup?: Record<string, unknown> } }> } | undefined;

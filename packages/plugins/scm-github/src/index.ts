...
function extractActionRunReference(check: CICheck): { runId: string; jobId?: string; runUrl: string } | null {
...
async function getFailedJobLog(pr: PRInfo, runReference: { runId: string; jobId?: string }): Promise<string> {
	try {
		return await gh([
			"run",
			"view",
...
async function getCIChecksFromStatusRollup(pr: PRInfo): Promise<CICheck[]> {
	const raw = await gh(["pr", "view", String(pr.number), "--repo", repoFlag(pr), "--json", "statusCheckRollup"]);
	...
	return rollup
		.map((entry): CICheck | null => {
			...
			const name =
				(typeof row["name"] === "string" && row["name"]) || (typeof row["context"] === "string" && row["context"]);
			if (!name) return null;
...
function verifyGitHubSignature(body: string | Uint8Array, secret: string, signatureHeader: string): boolean {
...
function parseGitHubRepository(payload: Record<string, unknown>) {
	...
	const ownerLogin =
		ownerValue && typeof ownerValue === "object" ? (ownerValue as Record<string, unknown>)["login"] : undefined;
...
function parseGitHubWebhookEvent(
		...
		return {
			...
			timestamp:
				rawEventType === "pull_request_review"
					? parseWebhookTimestamp((payload["review"] as Record<string, unknown> | undefined)?.["submitted_at"])
					: parseWebhookTimestamp(
		...
		return {
			...
			branch:
				typeof check?.["head_branch"] === "string"
					...
					: typeof (check?.["check_suite"] as Record<string, unknown> | undefined)?.["head_branch"] === "string"
	...
	if (rawEventType === "status") {
		const branches = Array.isArray(payload["branches"]) ? (payload["branches"] as Array<Record<string, unknown>>) : [];
		return {
			provider: "github",
			kind: "ci",
...
function createGitHubSCM(): SCM {
	...
	return {
		...
		async verifyWebhook(request: SCMWebhookRequest, project: ProjectConfig): Promise<SCMWebhookVerificationResult> {
			...
			if (config.maxBodyBytes !== undefined && Buffer.byteLength(request.body, "utf8") > config.maxBodyBytes) {
		...
		async parseWebhook(request: SCMWebhookRequest, project: ProjectConfig): Promise<SCMWebhookEvent | null> {
			const config = getGitHubWebhookConfig(project);
		...
		async getPRState(pr: PRInfo): Promise<PRState> {
			...
			return withPRCache(pr.owner, pr.repo, String(pr.number), "getPRState", async () => {
				const raw = await gh(["pr", "view", String(pr.number), "--repo", repoFlag(pr), "--json", "state"]);
		...
		async getCIFailureSummary(pr: PRInfo, providedFailedChecks?: CICheck[]): Promise<CIFailureSummary | null> {
			try {
		...
		async getReviews(pr: PRInfo): Promise<Review[]> {
			...
			return withPRCache(pr.owner, pr.repo, String(pr.number), "getReviews", async () => {
				const raw = await gh(["pr", "view", String(pr.number), "--repo", repoFlag(pr), "--json", "reviews"]);
				const data: {
					reviews: Array<{
						author: { login: string };
						state: string;
		...
		async getReviewDecision(pr: PRInfo): Promise<ReviewDecision> {
			...
			return withPRCache(pr.owner, pr.repo, String(pr.number), "getReviewDecision", async () => {
				const raw = await gh(["pr", "view", String(pr.number), "--repo", repoFlag(pr), "--json", "reviewDecision"]);

...
describe("scm-github plugin", () => {
	...
	describe("verifyWebhook", () => {
		...
		it("verifies a valid HMAC signature", async () => {
			...
			const signature = await import("node:crypto").then(
				({ createHmac }) => `sha256=${createHmac("sha256", "topsecret").update(body).digest("hex")}`,
			);
		...
		it("rejects an invalid HMAC signature", async () => {
			...
			expect(result).toEqual(expect.objectContaining({ ok: false, reason: "Webhook signature verification failed" }));
		});
	});
	...
	describe("parseWebhook", () => {
		...
		it("omits repository when owner.login is not a string", async () => {
			...
			expect(event).toEqual(expect.objectContaining({ kind: "pull_request", repository: undefined }));
		});
		...
		it("parses issue_comment events on pull requests as comment events", async () => {
			...
			expect(event).toEqual(expect.objectContaining({ provider: "github", kind: "comment", prNumber: 42 }));
		});

...
describe("Claude Code Activity Detection", () => {
	describe("toClaudeProjectPath", () => {
		...
		it("handles paths with multiple dot-directories", () => {
			expect(toClaudeProjectPath("/Users/dev/.config/.local/share")).toBe("-Users-dev--config--local-share");
		});
	});
	...
	describe("getActivityState", () => {
		...
		it("returns 'exited' when no runtimeHandle", async () => {
			expect((await agent.getActivityState(makeSession({ runtimeHandle: undefined })))?.state).toBe("exited");
		});
		...
		it("recordActivity is a no-op when workspacePath is null", async () => {
			await agent.recordActivity?.(makeSession({ workspacePath: null }), "Do you want to proceed?\n(Y)es / (N)o");

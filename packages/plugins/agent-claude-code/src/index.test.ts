...
function mockJsonlFiles(jsonlContent: string, files = ["session-abc123.jsonl"], mtime = new Date(1700000000000)) {
...
describe("toClaudeProjectPath", () => {
	...
	it("collapses dot directories like .worktrees into a leading double dash", () => {
		expect(toClaudeProjectPath("/Users/dev/.worktrees/ao/ao-3")).toBe("-Users-dev--worktrees-ao-ao-3");
	});
	...
	it("normalizes underscores to dashes (issue #1611)", () => {
		...
		).toBe("-Users-dev--agent-orchestrator-projects-graph-isomorphism-d185b44d56-worktrees-gi-orchestrator");
	});
...
describe("getLaunchCommand", () => {
	...
	it("includes --append-system-prompt and prompt as positional arg after --", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ systemPrompt: "You are a helper", prompt: "Do the task" }));
	...
	it("uses systemPromptFile via shell substitution with prompt after --", () => {
		const cmd = agent.getLaunchCommand(makeLaunchConfig({ systemPromptFile: "/tmp/prompt.md", prompt: "Do the task" }));
...
describe("getSessionInfo", () => {
	...
	describe("cost estimation", () => {
		...
		it("uses costUSD field when present", async () => {
			const jsonl = ['{"type":"user","message":{"content":"hi"}}', '{"costUSD":0.05}', '{"costUSD":0.03}'].join("\n");
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
		...
		it("prefers costUSD over estimatedCostUsd to avoid double-counting", async () => {
			const jsonl = ['{"type":"user","message":{"content":"hi"}}', '{"costUSD":0.10,"estimatedCostUsd":0.10}'].join(
				"\n",
			);
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
		...
		it("falls back to estimatedCostUsd when costUSD is absent", async () => {
			const jsonl = ['{"type":"user","message":{"content":"hi"}}', '{"estimatedCostUsd":0.12}'].join("\n");
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
		...
		it("uses direct inputTokens/outputTokens fields", async () => {
			const jsonl = ['{"type":"user","message":{"content":"hi"}}', '{"inputTokens":5000,"outputTokens":1000}'].join(
				"\n",
			);
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
	...
	describe("malformed JSONL handling", () => {
		it("skips malformed lines and parses valid ones", async () => {
			const jsonl = ["not valid json", '{"type":"summary","summary":"Good summary"}', "{truncated", ""].join("\n");
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
		...
		it("skips JSON null, array, and primitive values", async () => {
			const jsonl = ["null", "42", '"just a string"', "[1,2,3]", '{"type":"summary","summary":"Valid object"}'].join(
				"\n",
			);
			mockJsonlFiles(jsonl);
			const result = await agent.getSessionInfo(makeSession());
...
describe("hook setup — relative path (symlink-safe)", () => {
	...
	it("setupWorkspaceHooks writes a relative hook command (not absolute)", async () => {
		await agent.setupWorkspaceHooks!("/Users/equinox/.worktrees/integrator/integrator-5", {} as WorkspaceHooksConfig);
	...
	it("postLaunchSetup is a no-op (hooks installed pre-launch via setupWorkspaceHooks)", async () => {
		...
		await agent.postLaunchSetup!(makeSession({ workspacePath: "/Users/equinox/.worktrees/integrator/integrator-10" }));
	...
	it("different worktree paths produce identical settings.json content", async () => {
		...
		await agent.setupWorkspaceHooks!("/Users/equinox/.worktrees/integrator/integrator-10", {} as WorkspaceHooksConfig);
					...
					PostToolUse: [
						{
							...
							hooks: [
								{
									type: "command",
									command: "/Users/equinox/.worktrees/integrator/integrator-5/.claude/metadata-updater.sh",
									timeout: 5000,
								},
							],
						},
...
describe("setupWorkspaceHooks on win32", () => {
	...
	it("does not add duplicate hook entry when called twice on Windows", async () => {
		...
		const metadataHooks = hookEntries.flatMap((e) => e.hooks).filter((h) => h.command.includes("metadata-updater"));

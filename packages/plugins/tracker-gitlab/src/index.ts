...
function createGitLabTracker(config?: Record<string, unknown>): Tracker {
	...
	return {
		...
		async generatePrompt(identifier: string, project: ProjectConfig): Promise<string> {
			...
			const lines = [`You are working on GitLab issue #${issue.id}: ${issue.title}`, `Issue URL: ${issue.url}`, ""];
		...
		async listIssues(filters: IssueFilters, project: ProjectConfig): Promise<Issue[]> {
			const args = ["issue", "list", "--repo", requireRepo(project), "-O", "json", "-P", String(filters.limit ?? 30)];

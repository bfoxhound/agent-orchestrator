...
describe("tracker-gitlab plugin", () => {
	...
	describe("issueUrl", () => {
		...
		it("uses custom host when configured", () => {
			...
			expect(customTracker.issueUrl("42", project)).toBe("https://gitlab.example.com/acme/repo/-/issues/42");
		});

		it("infers host from project.repo when config.host is unset", () => {
			const selfHostedProject = { ...project, repo: "gitlab.corp.com/org/repo" };
			expect(tracker.issueUrl("42", selfHostedProject)).toBe("https://gitlab.corp.com/org/repo/-/issues/42");
		});

		it("strips hostname from project.repo with custom host", () => {
			const customTracker = create({ host: "gitlab.corp.com" });
			const selfHostedProject = { ...project, repo: "gitlab.corp.com/org/repo" };
			expect(customTracker.issueUrl("42", selfHostedProject)).toBe("https://gitlab.corp.com/org/repo/-/issues/42");
		});

		it("does not treat dotted group name as hostname (only 2 segments)", () => {
			const dottedGroupProject = { ...project, repo: "my.company/repo" };
			expect(tracker.issueUrl("42", dottedGroupProject)).toBe("https://gitlab.com/my.company/repo/-/issues/42");
		});
	});
	...
	describe("listIssues", () => {
		...
		it("passes --closed filter for closed issues", async () => {
			...
			expect(glabMock).toHaveBeenCalledWith("glab", expect.arrayContaining(["--closed"]), expect.any(Object));
		});

		it("passes --all filter for all issues", async () => {
			mockGlab([]);
			await tracker.listIssues!({ state: "all" }, project);
			expect(glabMock).toHaveBeenCalledWith("glab", expect.arrayContaining(["--all"]), expect.any(Object));
		});
		...
		it("respects custom limit", async () => {
			...
			expect(glabMock).toHaveBeenCalledWith("glab", expect.arrayContaining(["-P", "5"]), expect.any(Object));
		});
	});
	...
	describe("createIssue", () => {
		...
		it("throws when URL cannot be parsed from glab output", async () => {
			...
			await expect(tracker.createIssue!({ title: "Test", description: "" }, project)).rejects.toThrow(
				"Failed to parse issue URL",
			);
		});
	});
});

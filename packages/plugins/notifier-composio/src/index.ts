...
export function create(config?: Record<string, unknown>): Notifier {
	const apiKey =
		(typeof config?.composioApiKey === "string" ? config.composioApiKey : undefined) ?? process.env.COMPOSIO_API_KEY;
	...
	async function executeWithTimeout(
		...
		if (!result.successful) {
			throw new Error(`[notifier-composio] Composio action ${action} failed: ${result.error ?? "unknown error"}`);
		}
	}

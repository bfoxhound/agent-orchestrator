...
describe("workspace.create()", () => {
	...
	it("creates feature branch via checkout -b", async () => {
		...
		expect(mockExecFileAsync).toHaveBeenNthCalledWith(3, "git", ["checkout", "-b", "feat/new-branch"], {
	...
	it("cleans up partial clone on clone failure", async () => {
		...
		(fs.existsSync as ReturnType<typeof vi.fn>).mockReturnValueOnce(false).mockReturnValueOnce(true);
		// git clone fails
...
describe("workspace.list()", () => {
	...
	it("emits corrupt_clone_skipped only once per clone path", async () => {
		...
		(fs.readdirSync as ReturnType<typeof vi.fn>).mockReturnValue([{ name: "corrupt-repeat", isDirectory: () => true }]);

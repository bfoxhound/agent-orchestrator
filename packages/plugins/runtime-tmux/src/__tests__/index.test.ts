...
const mockExecFileCustom = (childProcess.execFile as any)[Symbol.for("nodejs.util.promisify.custom")] as ReturnType<
	typeof vi.fn
>;
...
describe("runtime.create()", () => {
	...
	it("keeps the keep-alive tail in the temp script for long launch commands", async () => {
		...
		const writeCall = (fs.writeFileSync as unknown as { mock: { calls: unknown[][] } }).mock.calls[0];
...
describe("runtime.sendMessage()", () => {
	...
	it("uses load-buffer + paste-buffer for long text (> 200 chars)", async () => {
		...
		expect(mockExecFileCustom).toHaveBeenNthCalledWith(
			2,
			"tmux",
			["load-buffer", "-b", "ao-test-uuid-1234", expect.stringContaining("ao-send-test-uuid-1234.txt")],
			expectedTmuxOptions,
		);
		...
		expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining("ao-send-test-uuid-1234.txt"), longText, {
			encoding: "utf-8",
			mode: 0o600,
		});

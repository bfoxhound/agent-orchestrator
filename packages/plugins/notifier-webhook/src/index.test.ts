...
describe("notifier-webhook", () => {
	...
	describe("retry logic", () => {
		...
		it("does NOT retry on 403 Forbidden", async () => {
			const fetchMock = vi.fn().mockResolvedValue({ ok: false, status: 403, text: () => Promise.resolve("forbidden") });
			vi.stubGlobal("fetch", fetchMock);
		...
		it("throws after all retries exhausted on 5xx", async () => {
			...
			await expect(notifier.notify(makeEvent())).rejects.toThrow("Webhook POST failed (500): error");
			// 1 initial + 2 retries = 3
			expect(fetchMock).toHaveBeenCalledTimes(3);
		});

		it("retries on network errors", async () => {
			const fetchMock = vi.fn().mockRejectedValueOnce(new Error("ECONNREFUSED")).mockResolvedValueOnce({ ok: true });
			vi.stubGlobal("fetch", fetchMock);
		...
		it("respects retries=0 (no retries)", async () => {
			const fetchMock = vi.fn().mockResolvedValue({ ok: false, status: 500, text: () => Promise.resolve("fail") });
			vi.stubGlobal("fetch", fetchMock);

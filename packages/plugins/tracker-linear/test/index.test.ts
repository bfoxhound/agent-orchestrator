...
describe("tracker-linear plugin", () => {
	...
	describe("getIssue", () => {
		...
		it("propagates API errors", async () => {
			...
			await expect(tracker.getIssue("INT-999", project)).rejects.toThrow("Linear API error: Issue not found");
		});
		...
		it("throws on HTTP errors", async () => {
			...
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Linear API returned HTTP 400");
		});
	...
	describe("updateIssue", () => {
		...
		it("updates labels additively (merges with existing)", async () => {
			...
			expect(body.variables.labelIds).toEqual(expect.arrayContaining(["label-existing", "label-1", "label-2"]));
	...
	describe("createIssue", () => {
		it("creates a basic issue", async () => {
			...
			const issue = await tracker.createIssue!({ title: "Fix login bug", description: "Desc" }, project);
		...
		it("resolves assignee by display name after creation", async () => {
			...
			const issue = await tracker.createIssue!({ title: "Bug", description: "", assignee: "Alice" }, project);
		...
		it("skips assignee when user not found", async () => {
			...
			const issue = await tracker.createIssue!({ title: "Bug", description: "", assignee: "Unknown" }, project);
			expect(issue.assignee).toBeUndefined();
		...
		it("adds labels after creation", async () => {
			...
			const issue = await tracker.createIssue!({ title: "Bug", description: "", labels: ["bug", "urgent"] }, project);
		...
		it("throws when teamId is missing from config", async () => {
			...
			await expect(tracker.createIssue!({ title: "Bug", description: "" }, noTeam)).rejects.toThrow("teamId");
		});
		...
		it("handles label error gracefully (best-effort)", async () => {
			...
			const issue = await tracker.createIssue!({ title: "Bug", description: "", labels: ["bug"] }, project);
	...
	describe("linearQuery error handling", () => {
		...
		it("throws on GraphQL errors", async () => {
			...
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Linear API error: You do not have access");
		});

		it("throws on HTTP error status", async () => {
			mockHTTPError(401, "Unauthorized");
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Linear API returned HTTP 401");
		});
		...
		it("throws on empty data response", async () => {
			...
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Linear API returned no data");
		});
	});
});

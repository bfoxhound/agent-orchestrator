...
describe("tracker-linear Composio transport", () => {
	...
	describe("error handling", () => {
		...
		it("throws with 'unknown error' when error field is missing", async () => {
			...
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Composio Linear API error: unknown error");
		});
		...
		it("throws when response has no data", async () => {
			...
			await expect(tracker.getIssue("INT-123", project)).rejects.toThrow("Composio Linear API returned no data");
		});

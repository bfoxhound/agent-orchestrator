		...
		const execute = (): Promise<T> =>
			new Promise<T>((resolve, reject) => {
				...
				const req = request(
					...
					(res) => {
						const chunks: Buffer[] = [];
						res.on("error", (err: Error) => settle(() => reject(new LinearNetworkError(err.message))));
...
function createLinearTracker(query: GraphQLTransport): Tracker {
	return {
		...
		async generatePrompt(identifier: string, project: ProjectConfig): Promise<string> {
			...
			const lines = [`You are working on Linear ticket ${issue.id}: ${issue.title}`, `Issue URL: ${issue.url}`, ""];
		...
		async updateIssue(identifier: string, update: IssueUpdate, _project: ProjectConfig): Promise<void> {
          ...
          issue(id: $id) {
            ...
            workflowStates(filter: { team: { id: { eq: $teamId } } }) {
              nodes { id name type }
				...
				const targetType = update.state === "closed" ? "completed" : update.state === "open" ? "unstarted" : "started";

...
describe("getLaunchCommand (integration)", () => {
	...
	it("generates correct command with subagent", () => {
		...
		expect(cmd).toContain("opencode run --format json --title 'AO:test-1' --agent 'sisyphus' --command true");
	...
	it("uses prompt with systemPromptFile for orchestrator-style launch", () => {
		...
		expect(cmd).toContain("opencode run --format json --title 'AO:test-orchestrator' --command true");

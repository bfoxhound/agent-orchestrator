...
import { readLastJsonlEntry, type ActivityDetection, type AgentSessionInfo } from "@aoagents/ao-core";
...
describe.skipIf(!canRun)("agent-claude-code (integration)", () => {
	...
	beforeAll(async () => {
		...
		exitedRunning = await pollUntilEqual(async () => (await agent.isProcessRunning(handle)) === true, false, {
			timeoutMs: 90_000,
			intervalMs: 2_000,
		});
	...
	it("getActivityState → returns valid non-exited state while agent is alive", () => {
		...
		expect([null, "active", "ready", "idle", "waiting_input", "blocked"]).toContain(aliveActivityState?.state ?? null);
	});

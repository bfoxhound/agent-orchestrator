...
describe("notifier-webhook integration", () => {
	...
	describe("non-retryable errors", () => {
		it.each([400, 401, 403, 404])("HTTP %d -> single fetch call, immediate rejection", async (status) => {
			...
			await expect(notifier.notify(makeEvent())).rejects.toThrow(`Webhook POST failed (${status})`);
			expect(fetchMock).toHaveBeenCalledTimes(1);
		});
	});

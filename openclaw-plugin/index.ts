...
function resolveAoConfigPath(config: PluginConfig): string | null {
	...
	while (true) {
		candidates.push(join(currentDir, "agent-orchestrator.yaml"), join(currentDir, "agent-orchestrator.yml"));
...
export function fetchIssues(
	...
	const dedupedIssues = issues
		...
		.filter(
			(issue, index, allIssues) =>
				allIssues.findIndex((candidate) => getIssueIdentity(candidate) === getIssueIdentity(issue)) === index,
		);
	...
	return {
		...
		scannedRepos: [
			...new Set(scannedRepos.length > 0 ? scannedRepos : inferredRepos.length > 0 ? inferredRepos : repos),
		],
		warnings,
	};
}
...
export default function (api: PluginApi) {
	...
	const onMessageReceived = async (event: PluginEvent) => {
		const message = event?.message?.text || event?.message?.content || event?.text || event?.content || "";
	...
	api.registerCommand({
		name: "ao",
		description: "Agent Orchestrator — /ao sessions | status | spawn | issues | batch-spawn | retry | kill | doctor",
		acceptsArgs: true,
		requireAuth: true,
		handler: async (ctx: CommandContext) => {
			...
			switch (subcommand) {
				...
				case "setup": {
					...
					if (runSetup("openclaw", ["config", "set", "tools.profile", "full"])) steps.push("✅ tools.profile → full");
					else steps.push("❌ Failed to set tools.profile");

					// 2. Allow plugin tools
					const mergedToolsAllow = mergeStringLists(readOpenClawStringArraySetting("tools.allow", ["tools", "allow"]), [
						"group:plugins",
					]);
					if (runSetup("openclaw", ["config", "set", "tools.allow", JSON.stringify(mergedToolsAllow)])) {
					...
					if (runSetup("openclaw", ["config", "set", "plugins.allow", JSON.stringify(mergedPluginsAllow)])) {
	...
	api.registerTool({
		...
		async execute(_toolCallId: string, params: { issue?: string; agent?: string; claimPr?: string }) {
			const args = ["spawn"];
			if (params.issue) {
	...
	api.registerTool({
		...
		async execute(_toolCallId: string, params: { issues: string[] }) {
			...
			const checkStatus = (label: string) => {
				...
				try {
					api.runtime?.sendMessageToDefaultSession?.(`${label}:\n\n${msg}\n\nNeed me to do anything?`);
				} catch {
	...
	api.registerTool({
		name: "ao_kill",
		description: "Kill an Agent Orchestrator session. Always confirm with the user before calling this.",
		parameters: {
			type: "object",
			properties: {
		...
		async execute(_toolCallId: string, params: { sessionId: string }) {
			...
			return {
				content: [{ type: "text", text: `Session ${params.sessionId} killed and worktree cleaned up.` }],
			};
		},
	});
	...
	api.registerTool({
		...
		async execute(_toolCallId: string, params: { pr: string; sessionId?: string; assignOnGithub?: boolean }) {
		...
		start: async () => {
			...
			const scan = () => {
				try {
					...
					const newIssues = issuesResult.issues.filter((issue) => !lastKnownIssueIds.has(getIssueIdentity(issue)));
